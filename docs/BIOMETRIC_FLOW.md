...
```
================================================================================
                        🧬 FIM DO FLUXO BIOMIMÉTICO — INÍCIO DA SEÇÃO SOCIAL
================================================================================

---

## 🫀 SISTEMA CIRCULATÓRIO E ENDÓCRINO — SocialRegistry

A partir de Julho/2026, a iaglobal ganhou um **sistema circulatório hormonal**.

Cada agente secreta `Advertisement` no `AcetylcholineBus` (corrente sanguínea),
e o `SocialRegistry` mantém a tabela de roteamento metabólica:

```
┌──────────────────────────────────────────────────────────────────┐
│  SocialRegistry (singleton)                                      │
│                                                                  │
│  publish(adv)   → upserta skills + load_factor                  │
│  heartbeat(id)  → atualiza last_seen (TTL=120s)                 │
│  withdraw(id)   → remove agente (apoptose voluntária)           │
│  query(domain)  → lista viva ordenada por proficiency descendente│
│  get(id)        → retorna Advertisement ou None (stale vira None)│
│                                                                  │
│  Canais AcetylcholineBus:                                        │
...
### O que muda no fluxo da pipeline

Antes (vertical): Agente → Critic → Bandit → Provider

```
Agente A ──solicita──→ Critic ──avalia──→ Bandit ──seleciona──→ Provider
```

Depois (vertical + horizontal):
...
Agente A ──solicita──→ Critic ──avalia──→ Bandit ──seleciona──→ Provider
      │                                                              │
      └── query("test", min=0.8) → [agent-c] ──delega──→────────────┘
                 (cooperação horizontal sem passar pelo pipeline)
```

### Advertisement — O DNA Social do Agente

```python
@dataclass
class Advertisement:
    agent_id: str
    skills: dict[str, Capability]   # dominio -> proficiencia + latencia
    load_factor: float              # 0.0 (ocioso) a 1.0 (saturado)
    last_seen: float                # heartbeat TTL (120s)
```

O IVM de cada agente realimenta o proficiency no Capability, criando um ciclo
virtuoso: agente com alto IVM em um dominio e mais requisitado -> mais cooperacao ->
mais IVM (o "C" do IVM e cooperacao).

### Como adicionar um novo agente a sociedade

```python
from iaglobal.agents.social import social_registry, Advertisement, Capability
from iaglobal.graphs.comms.acetylcholine_bus import bus, AgentMessage

adv = Advertisement(
    agent_id="meu_agente",
    skills={"code": Capability("code", proficiency=0.9, latency_p50_ms=120)},
    load_factor=0.0,
)
social_registry.publish(adv)
```
...
|---------|-----------------|
| iaglobal/agents/social.py | SocialRegistry, Advertisement, Capability, integracao AcetylcholineBus |
| iaglobal/tests/test_social.py | 19 testes de cobertura (publish, query, stale, thread-safety, heartbeat, withdraw) |
