...
class TestServer:
    ...
    @pytest.mark.asyncio
    async def test_task_request_schema(self):
    ...
    @pytest.mark.asyncio
    async def test_metrics_response_returns_dict(self):
        from iaglobal.server.server import app
        for route in app.routes:
    ...
    @pytest.mark.asyncio
    async def test_lifespan_is_callable(self):
        from iaglobal.server.server import lifespan
        assert callable(lifespan)

    @pytest.mark.asyncio
    async def test_routes_use_expected_http_methods(self):
        from iaglobal.server.server import app
        route_map = {}
        for r in app.routes:
    ...
    @pytest.mark.asyncio
    async def test_replay_engine_has_diff(self):
    ...
    @pytest.mark.asyncio
    async def test_dashboard_json_returns_dict(self):
        from iaglobal.server.server import app
        for route in app.routes:
...
class TestCpuAffinity:
    ...
    @pytest.mark.asyncio
    async def test_import_cpu_affinity(self):
    ...
    @pytest.mark.asyncio
    async def test_detect_cores(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        assert mgr._total_cores >= 1

    @pytest.mark.asyncio
    async def test_assign_core_deterministic(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        core = await mgr.assign_core_deterministic('agent-123')
        assert isinstance(core, int)
        assert 0 <= core < mgr._total_cores

    @pytest.mark.asyncio
    async def test_assign_core_consistent(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        c1 = await mgr.assign_core_deterministic('consistent-agent')
        c2 = await mgr.assign_core_deterministic('consistent-agent')
        assert c1 == c2

    @pytest.mark.asyncio
    async def test_assign_core_different_agents(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        c1 = await mgr.assign_core_deterministic('agent-alpha')
        c2 = await mgr.assign_core_deterministic('agent-beta')
        assert 0 <= c1 < mgr._total_cores
        assert 0 <= c2 < mgr._total_cores

    @pytest.mark.asyncio
    async def test_pin_to_hash_returns_core(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        core = await mgr.pin_to_hash('agent-pin')
        assert isinstance(core, int)

    @pytest.mark.asyncio
    async def test_pin_to_hash_hex_id(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        hex_id = 'a1b2c3d4e5f6'
        core = await mgr.pin_to_hash(hex_id)
        assert isinstance(core, int)

    @pytest.mark.asyncio
    async def test_map_balanced(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        agents = [f'agent-{i}' for i in range(10)]
        mapping = await mgr.map_balanced(agents)
        assert len(mapping) == 10
        assert all((a in mapping for a in agents))
        assert all((0 <= c < mgr._total_cores for c in mapping.values()))

    @pytest.mark.asyncio
    async def test_dispersion_report(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        agents = [f'agent-{i}' for i in range(8)]
        await mgr.map_balanced(agents)
        report = await mgr.dispersion_report()
    ...
    @pytest.mark.asyncio
    async def test_rebalance_if_needed(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        agents = [f'agent-{i}' for i in range(5)]
        await mgr.map_balanced(agents)
        result = await mgr.rebalance_if_needed()
        assert isinstance(result, bool)

    @pytest.mark.asyncio
    async def test_pin_current(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.pin_current('test-agent')
        assert True
    ...
    @pytest.mark.asyncio
    async def test_set_and_get_budget(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.set_cpu_budget('agent-budget', 0.15)
        assert await mgr.get_cpu_budget('agent-budget') == 0.15

    @pytest.mark.asyncio
    async def test_budget_capped_at_max(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager, BUDGET_PADRAO
        mgr = CpuAffinityManager()
        await mgr.set_cpu_budget('greedy', 0.99)
        assert await mgr.get_cpu_budget('greedy') <= BUDGET_PADRAO

    @pytest.mark.asyncio
    async def test_survival_mode(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager, BUDGET_SOBREVIVENCIA
        mgr = CpuAffinityManager()
        await mgr.survival_mode('nomade')
        assert await mgr.get_cpu_budget('nomade') == BUDGET_SOBREVIVENCIA
        metrics = await mgr.get_metrics('nomade')
        assert metrics['em_modo_sobrevivencia'] is True

    @pytest.mark.asyncio
    async def test_restore_budget(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager, BUDGET_PADRAO
        mgr = CpuAffinityManager()
        await mgr.survival_mode('restore-test')
        await mgr.restore_budget('restore-test')
        assert await mgr.get_cpu_budget('restore-test') == BUDGET_PADRAO
        metrics = await mgr.get_metrics('restore-test')
        assert metrics['em_modo_sobrevivencia'] is False

    @pytest.mark.asyncio
    async def test_calcular_ivm_alto(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        ivm = await mgr.calcular_ivm('agent-ivm', produtividade=0.9, cpu_usage=0.05, obsidian_notes=8)
        assert ivm > 0.7

    @pytest.mark.asyncio
    async def test_calcular_ivm_baixo(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        ivm = await mgr.calcular_ivm('agent-ivm-bad', produtividade=0.1, cpu_usage=0.5, obsidian_notes=0)
        assert ivm < 0.5

    @pytest.mark.asyncio
    async def test_monitorar_metabolismo_apoptose(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.reportar_uso_cpu('agent-morto', 0.8)
        await mgr.registrar_tarefa('agent-morto', sucesso=False)
        await mgr.registrar_tarefa('agent-morto', sucesso=False)
        await mgr.calcular_ivm('agent-morto', obsidian_notes=0)
        result = await mgr.monitorar_metabolismo('agent-morto')
        assert result['acao'] == 'apoptose'

    @pytest.mark.asyncio
    async def test_monitorar_metabolismo_mitose(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.reportar_uso_cpu('agent-vivo', 0.02)
        await mgr.registrar_tarefa('agent-vivo', sucesso=True)
        await mgr.registrar_tarefa('agent-vivo', sucesso=True)
        await mgr.calcular_ivm('agent-vivo', obsidian_notes=10)
        result = await mgr.monitorar_metabolismo('agent-vivo')
        assert result['acao'] in ('mitose', 'monitorar')

    @pytest.mark.asyncio
    async def test_update_fitness_default(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        score = await mgr.update_fitness('agent-fit')
        assert 0 <= score <= 1

    @pytest.mark.asyncio
    async def test_update_fitness_eficiencia(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        score = await mgr.update_fitness('agent-eff', trabalho_realizado=10, custo_cpu=2)
        assert 0 <= score <= 1

    @pytest.mark.asyncio
    async def test_get_fitness_default(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        assert await mgr.get_fitness('unknown') == 0.5

    @pytest.mark.asyncio
    async def test_auto_critica_diagnostics(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.reportar_uso_cpu('agent-crit', 0.5)
        await mgr.registrar_tarefa('agent-crit', sucesso=False)
        await mgr.registrar_tarefa('agent-crit', sucesso=False)
        await mgr.registrar_tarefa('agent-crit', sucesso=False)
        await mgr.calcular_ivm('agent-crit', obsidian_notes=0)
        critica = await mgr.auto_critica('agent-crit')
        assert 'diagnosticos' in critica
        assert 'recommendacao' in critica

    @pytest.mark.asyncio
    async def test_registrar_tarefa_sucesso(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.registrar_tarefa('agent-task', sucesso=True)
        metrics = await mgr.get_metrics('agent-task')
        assert metrics['tasks_completadas'] == 1

    @pytest.mark.asyncio
    async def test_registrar_tarefa_falha(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.registrar_tarefa('agent-task2', sucesso=False)
        metrics = await mgr.get_metrics('agent-task2')
        assert metrics['tasks_falhas'] == 1

    @pytest.mark.asyncio
    async def test_resource_manager_alocar(self):
        from iaglobal.execution.cpu_affinity import ResourceManager
        rm = ResourceManager()
        budgets = await rm.alocar([('a', 0.9), ('b', 0.5), ('c', 0.1)])
        assert len(budgets) == 3
        assert all((b >= 0.05 for b in budgets.values()))
        assert all((b <= 0.25 for b in budgets.values()))

    @pytest.mark.asyncio
    async def test_resource_manager_budget_proporcional(self):
        from iaglobal.execution.cpu_affinity import ResourceManager
        rm = ResourceManager()
        budgets = await rm.alocar([('alta', 1.0), ('baixa', 0.1)])
    ...
    async def test_get_all_metrics(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.assign_core_deterministic('agent-m1')
        await mgr.assign_core_deterministic('agent-m2')
        all_m = await mgr.get_all_metrics()
        assert len(all_m) >= 2

    @pytest.mark.asyncio
    async def test_reportar_uso_cpu(self):
        from iaglobal.execution.cpu_affinity import CpuAffinityManager
        mgr = CpuAffinityManager()
        await mgr.reportar_uso_cpu('agent-cpu', 0.3)
        metrics = await mgr.get_metrics('agent-cpu')
...
class TestObsidianVault:
    ...
    @pytest.mark.asyncio
    async def test_vault_structure_can_be_created(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_markdown_note_with_frontmatter(self, tmp_path):
        vault = tmp_path / 'obsidian_vault'
...
class TestSubconsciousAPI:
    ...
    @pytest.mark.asyncio
    async def test_create_with_default_vault(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
    ...
    @pytest.mark.asyncio
    async def test_escrever_nota_instinto(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_escrever_curto_prazo(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        caminho = await api.escrever_curto_prazo('log_execucao', 'saida do agente')
        assert caminho.exists()
        assert 'saida do agente' in caminho.read_text()

    @pytest.mark.asyncio
    async def test_escrever_longo_prazo(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        caminho = await api.escrever_longo_prazo('estrategia_x', 'insight consolidado', fitness=0.95)
        assert caminho.exists()
        texto = caminho.read_text()
    ...
    @pytest.mark.asyncio
    async def test_registrar_erro(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        caminho = await api.registrar_erro('agente_x', 'tarefa y', 'traceback simulado')
        assert caminho.exists()
        texto = caminho.read_text()
    ...
    @pytest.mark.asyncio
    async def test_sussurrar_intuicao_sem_memoria(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        sussurro = await api.sussurrar_intuicao([])
        assert isinstance(sussurro, str)
        assert len(sussurro) > 0

    @pytest.mark.asyncio
    async def test_sussurrar_intuicao_com_tags(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.escrever_longo_prazo('topico_a', 'memoria sobre A', fitness=0.9, tags=['#tagA'])
        sussurro = await api.sussurrar_intuicao(['#tagA'])
        assert isinstance(sussurro, str)

    @pytest.mark.asyncio
    async def test_obter_insight_subconsciente_vazio(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        insight = await api.obter_insight_subconsciente(['#inexistente'])
        assert isinstance(insight, str)

    @pytest.mark.asyncio
    async def test_obter_insight_subconsciente_com_dados(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.escrever_longo_prazo('estrategia_b', 'conteudo relevante', fitness=0.8, tags=['#otimizacao'])
        await api.atualizar_mapa_conexoes()
        insight = await api.obter_insight_subconsciente(['#otimizacao'])
        assert 'conteudo relevante' in insight or 'estrat' in insight.lower()

    @pytest.mark.asyncio
    async def test_atualizar_mapa_conexoes(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.atualizar_mapa_conexoes()
    ...
    @pytest.mark.asyncio
    async def test_exportar_nota_agente(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        caminho = await api.exportar_nota_agente('agent_alpha', 'explore', 0.85)
        assert caminho.exists()
        texto = caminho.read_text()
        assert 'id:' in texto
    ...
    @pytest.mark.asyncio
    async def test_ler_nota(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        escrita = await api.escrever_instinto('lei_base', 'nao mentir')
        leitura = await api.ler_nota(escrita.stem)
        assert leitura is not None
        assert 'nao mentir' in leitura

    @pytest.mark.asyncio
    async def test_listar_notas(self, tmp_path):
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.escrever_instinto('inst1', 'conteudo 1')
        await api.escrever_instinto('inst2', 'conteudo 2')
        notas = await api.listar_notas(diretorio=api.instincts_dir)
        assert len(notas) >= 2
...
class TestREMSleepEngine:
    ...
    @pytest.mark.asyncio
    async def test_iniciar_fase_rem_vazia(self, tmp_path):
        from iaglobal.obsidian.consolidation import REMSleepEngine
        engine = REMSleepEngine(tmp_path / 'vault')
        resultado = await engine.iniciar_fase_rem()
        assert isinstance(resultado, dict)
        assert 'memorias_consolidadas' in resultado
        assert resultado['memorias_consolidadas'] == 0

    @pytest.mark.asyncio
    async def test_iniciar_fase_rem_com_memorias(self, tmp_path):
        from iaglobal.obsidian.consolidation import REMSleepEngine
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.escrever_curto_prazo('mem1', 'dado bruto 1')
        await api.escrever_curto_prazo('mem2', 'dado bruto 2')
        engine = REMSleepEngine(tmp_path / 'vault')
        resultado = await engine.iniciar_fase_rem()
        assert resultado['memorias_consolidadas'] >= 1

    @pytest.mark.asyncio
    async def test_iniciar_fase_rem_apaga_curto_prazo(self, tmp_path):
        from iaglobal.obsidian.consolidation import REMSleepEngine
        from iaglobal.obsidian.subconsciousapi import SubconsciousAPI
        api = SubconsciousAPI(tmp_path / 'vault')
        await api.escrever_curto_prazo('temp_mem', 'dado volatil')
        engine = REMSleepEngine(tmp_path / 'vault')
        await engine.iniciar_fase_rem()
...
class TestLearningSystem:
    ...
    async def test_preparar_prompt_com_intuicao(self, tmp_path):
        ...
        prompt = await wrapper.preparar_prompt_com_intuicao('tarefa urgente', ['#urgente'])
...
class TestErrorCapture:
    ...
    @pytest.mark.asyncio
    async def test_capturar_excecao(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_capturar_cria_arquivo(self, tmp_path):
        from iaglobal.obsidian.error_capture import ErrorCapture
        capture = ErrorCapture(vault_path=tmp_path / 'vault', agente='test_agent')
    ...
    @pytest.mark.asyncio
    async def test_context_manager_captura_erro(self, tmp_path):
        from iaglobal.obsidian.error_capture import ErrorCapture
        try:
...
class TestShortTermMemory:
    ...
    @pytest.mark.asyncio
    async def test_add_and_get_recent(self, tmp_path):
        from iaglobal.memory.term_short import ShortTermMemory
    ...
    @pytest.mark.asyncio
    async def test_max_size(self, tmp_path):
        from iaglobal.memory.term_short import ShortTermMemory
    ...
    @pytest.mark.asyncio
    async def test_swap_status(self, tmp_path):
        from iaglobal.memory.term_short import ShortTermMemory
    ...
    @pytest.mark.asyncio
    async def test_is_full(self, tmp_path):
        from iaglobal.memory.term_short import ShortTermMemory
    ...
    @pytest.mark.asyncio
    async def test_get_recent_with_metadata(self, tmp_path):
        from iaglobal.memory.term_short import ShortTermMemory
...
class TestLongTermMemory:
    ...
    @pytest.mark.asyncio
    async def test_consolidate_new(self, tmp_path):
        from iaglobal.memory.term_long import LongTermMemory
    ...
    @pytest.mark.asyncio
    async def test_consolidate_merge(self, tmp_path):
        from iaglobal.memory.term_long import LongTermMemory
    ...
    @pytest.mark.asyncio
    async def test_importance_increment_on_consolidate(self, tmp_path):
        from iaglobal.memory.term_long import LongTermMemory
...
class TestMemoryStorage:
    ...
    @pytest.mark.asyncio
    async def test_store_success_function(self):
...
class TestCache:
    ...
    @pytest.mark.asyncio
    async def test_get_set_module_level(self):
    ...
    @pytest.mark.asyncio
    async def test_cache_class(self):
    ...
    @pytest.mark.asyncio
    async def test_hash_prompt_deterministic(self):
...
class TestCognitiveCache:
    ...
    @pytest.mark.asyncio
    async def test_set_with_critic_approval(self):
        from iaglobal.memory.cognitive_cache import CognitiveCache
        from iaglobal.memory.cache import Cache
    ...
    @pytest.mark.asyncio
    async def test_set_with_critic_rejection(self):
        from iaglobal.memory.cognitive_cache import CognitiveCache
        from iaglobal.memory.cache import Cache
...
class TestConsolidationEngine:
    ...
    @pytest.mark.asyncio
    async def test_consolidate_single_item(self):
        from iaglobal.memory.consolidation import ConsolidationEngine
    ...
    @pytest.mark.asyncio
    async def test_consolidate_with_cluster(self):
        from iaglobal.memory.consolidation import ConsolidationEngine
        engine = ConsolidationEngine(min_cluster_size=2)
        items = [{'content': 'aprendizado de maquina supervisionado', 'source': 'web'}, {'content': 'aprendizado de maquina nao supervisionado', 'source': 'local'}]
        result = engine.consolidate(items)
        assert len(result) >= 1
        assert 'Insight consolidado' in result[0]['content']

    @pytest.mark.asyncio
    async def test_consolidate_web_knowledge(self):
        from iaglobal.memory.consolidation import ConsolidationEngine
        engine = ConsolidationEngine(min_cluster_size=2)
    ...
    @pytest.mark.asyncio
    async def test_normalize_items(self):
        from iaglobal.memory.consolidation import ConsolidationEngine
        engine = ConsolidationEngine()
        items = engine._normalize([{'text': 'corpo', 'title': 'titulo'}, 'string pura'], 'test')
        assert len(items) == 2
...
class TestCognitiveRanking:
    ...
    @pytest.mark.asyncio
    async def test_score_defaults(self):
        from iaglobal.memory.ranking import CognitiveRanking
    ...
    @pytest.mark.asyncio
    async def test_score_with_all_fields(self):
        from iaglobal.memory.ranking import CognitiveRanking
        ranker = CognitiveRanking()
        item = {'relevance': 0.9, 'usage_count': 5, 'timestamp': datetime.now(timezone.utc).isoformat(), 'web_frequency': 0.7}
    ...
    @pytest.mark.asyncio
    async def test_detect_no_conflict_identical(self):
        from iaglobal.memory.ranking import CognitiveRanking
        ranker = CognitiveRanking()
...
class TestFusionEngine:
    ...
    @pytest.mark.asyncio
    async def test_import(self):
        from iaglobal.memory.fusion_engine import FusionEngine, WebCacheInteligente, AntiRedundanciaGlobal, FakeNoiseDetector, KnowledgeGraph, AtualizacaoIncremental
    ...
    @pytest.mark.asyncio
    async def test_webcache_set_get(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_antiredundancia_is_duplicate_exact(self):
    ...
    @pytest.mark.asyncio
    async def test_antiredundancia_is_duplicate_similar(self):
        from iaglobal.memory.fusion_engine import AntiRedundanciaGlobal
        ar = AntiRedundanciaGlobal(similarity_threshold=0.8)
        is_dup, match = ar.is_duplicate('aprendizado supervisionado com redes neurais', [{'content': 'aprendizado supervisionado com redes neurais profundas'}])
        assert isinstance(is_dup, bool)

    @pytest.mark.asyncio
    async def test_antiredundancia_not_duplicate(self):
        from iaglobal.memory.fusion_engine import AntiRedundanciaGlobal
        ar = AntiRedundanciaGlobal()
    ...
    @pytest.mark.asyncio
    async def test_antiredundancia_dedup_list(self):
        from iaglobal.memory.fusion_engine import AntiRedundanciaGlobal
        ar = AntiRedundanciaGlobal()
    ...
    @pytest.mark.asyncio
    async def test_antiredundancia_merge_similar(self):
        from iaglobal.memory.fusion_engine import AntiRedundanciaGlobal
        ar = AntiRedundanciaGlobal()
        items = [{'content': 'Python é uma linguagem', 'source': 'a'}, {'content': 'Python é uma linguagem de programacao', 'source': 'b'}]
        merged = ar.merge_similar(items, merge_threshold=0.5)
        assert len(merged) <= len(items)

    @pytest.mark.asyncio
    async def test_antiredundancia_clear(self):
        from iaglobal.memory.fusion_engine import AntiRedundanciaGlobal
        ar = AntiRedundanciaGlobal()
    ...
    @pytest.mark.asyncio
    async def test_fakenoise_score_confidence(self):
    ...
    @pytest.mark.asyncio
    async def test_fakenoise_is_noise_short(self):
        from iaglobal.memory.fusion_engine import FakeNoiseDetector
        nd = FakeNoiseDetector()
        assert nd.is_noise({'content': 'curto'})

    @pytest.mark.asyncio
    async def test_fakenoise_is_not_noise(self):
        from iaglobal.memory.fusion_engine import FakeNoiseDetector
        nd = FakeNoiseDetector()
        assert not nd.is_noise({'content': 'Um texto longo o suficiente para nao ser considerado ruido pelo detector'})

    @pytest.mark.asyncio
    async def test_fakenoise_detect_contradiction(self):
        from iaglobal.memory.fusion_engine import FakeNoiseDetector
        nd = FakeNoiseDetector()
        existing = [{'content': 'Python é uma linguagem excelente para machine learning'}]
        result = nd.detect_contradiction({'content': 'Python não é adequado para machine learning'}, existing)
        assert result is not None
        assert 'jaccard' in result

    @pytest.mark.asyncio
    async def test_fakenoise_filter_noise(self):
        from iaglobal.memory.fusion_engine import FakeNoiseDetector
        nd = FakeNoiseDetector()
        items = [{'content': 'texto valido com conteudo significativo para teste'}, {'content': 'curto'}]
        filtered = nd.filter_noise(items)
        assert len(filtered) == 1

    @pytest.mark.asyncio
    async def test_fakenoise_clickbait_detection(self):
        from iaglobal.memory.fusion_engine import FakeNoiseDetector
        nd = FakeNoiseDetector()
    ...
    @pytest.mark.asyncio
    async def test_knowledgegraph_extract_and_store(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_knowledgegraph_get_related(self, tmp_path):
        from iaglobal.memory.fusion_engine import KnowledgeGraph
    ...
    @pytest.mark.asyncio
    async def test_knowledgegraph_get_top_concepts(self, tmp_path):
        from iaglobal.memory.fusion_engine import KnowledgeGraph
    ...
    @pytest.mark.asyncio
    async def test_knowledgegraph_search(self, tmp_path):
        from iaglobal.memory.fusion_engine import KnowledgeGraph
    ...
    @pytest.mark.asyncio
    async def test_knowledgegraph_stats(self, tmp_path):
        from iaglobal.memory.fusion_engine import KnowledgeGraph
    ...
    @pytest.mark.asyncio
    async def test_atualizacao_incorporate_new(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_atualizacao_incorporate_noise(self, tmp_path):
        from iaglobal.memory.fusion_engine import AtualizacaoIncremental, KnowledgeGraph, AntiRedundanciaGlobal
    ...
    @pytest.mark.asyncio
    async def test_atualizacao_batch_incorporate(self, tmp_path):
        from iaglobal.memory.fusion_engine import AtualizacaoIncremental, KnowledgeGraph, AntiRedundanciaGlobal
        kg = KnowledgeGraph(db_path=tmp_path / 'ai_batch.db')
        dedup = AntiRedundanciaGlobal()
        ai = AtualizacaoIncremental(kg, dedup)
        items = [{'content': 'Primeiro conceito importante', 'source': 'web'}, {'content': 'Segundo conceito relevante', 'source': 'local'}]
        results = ai.batch_incorporate(items)
        assert len(results) == 2

    @pytest.mark.asyncio
    async def test_fusion_engine_process_web_result(self, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_fusion_engine_process_knowledge_batch(self, tmp_path):
        from iaglobal.memory.fusion_engine import FusionEngine
        fe = FusionEngine(db_path=tmp_path / 'fe_batch.db')
        items = [{'content': 'Item um sobre machine learning', 'source': 'web'}, {'content': 'Item dois sobre deep learning', 'source': 'web'}]
        result = fe.process_knowledge_batch(items)
        assert 'items' in result
        assert 'stats' in result

    @pytest.mark.asyncio
    async def test_fusion_engine_get_knowledge_context(self, tmp_path):
        from iaglobal.memory.fusion_engine import FusionEngine
    ...
    @pytest.mark.asyncio
    async def test_fusion_engine_stats(self, tmp_path):
        from iaglobal.memory.fusion_engine import FusionEngine
...
class TestDatabaseManager:
    ...
    @pytest.mark.asyncio
    async def test_insert_and_get_insights(self):
    ...
    @pytest.mark.asyncio
    async def test_get_insights_with_min_score(self):
        from iaglobal.memory.db_manager import db
    ...
    @pytest.mark.asyncio
    async def test_count_insights(self):
        from iaglobal.memory.db_manager import db
    ...
    @pytest.mark.asyncio
    async def test_init_execution_and_checkpoint(self):
        from iaglobal.memory.db_manager import db
        eid = 'exec-test-001'
        nodes = ['planner', 'coder']
    ...
    @pytest.mark.asyncio
    async def test_reset_failed_node(self):
        from iaglobal.memory.db_manager import db
        eid = 'exec-reset-001'
    ...
    @pytest.mark.asyncio
    async def test_clear_execution(self):
        from iaglobal.memory.db_manager import db
        eid = 'exec-clear-001'
    ...
    @pytest.mark.asyncio
    async def test_decision_events(self):
        from iaglobal.memory.db_manager import db
    ...
    @pytest.mark.asyncio
    async def test_count_decision_events(self):
        from iaglobal.memory.db_manager import db
...
class TestPersistence:
    ...
    @pytest.mark.asyncio
    async def test_validar_integridade_memoria(self):
        from iaglobal.memory.persistence import Persistence
...
class TestMemoryManager:
    ...
    @pytest.mark.asyncio
    async def test_force_emergency_backup(self, tmp_path):
        from iaglobal.memory.backup_manager import MemoryManager
...
class TestMemoryError:
    ...
    @pytest.mark.asyncio
    async def test_import(self):
        from iaglobal.memory.memory_error import load_errors, store_error, query_relevant_errors, format_errors_for_prompt, record_error
        assert load_errors is not None

    @pytest.mark.asyncio
    async def test_store_and_load_errors(self):
    ...
    @pytest.mark.asyncio
    async def test_query_relevant_errors(self):
    ...
    @pytest.mark.asyncio
    async def test_format_errors_for_prompt(self):
        from iaglobal.memory.memory_error import format_errors_for_prompt
        errors = [{'error_type': 'TypeError', 'prompt': 'p', 'response_errada': 'r', 'critica_sandbox': 'c', 'codigo_corrigido': 'f'}]
        formatted = format_errors_for_prompt(errors)
        assert 'TypeError' in formatted
        assert 'HISTÓRICO DE ERROS' in formatted

    @pytest.mark.asyncio
    async def test_record_runtime_error(self):
    ...
    @pytest.mark.asyncio
    async def test_format_empty_errors(self):
        from iaglobal.memory.memory_error import format_errors_for_prompt
...
class TestSemanticCache:
    ...
    @pytest.mark.asyncio
    async def test_get_miss(self, tmp_path):
        from iaglobal.memory.semantic_cache import SemanticCache
...
class TestMemoryModule:
    ...
    async def test_all_exports(self):
        ...
        expected = ['Cache', 'Memory', 'MemoryStorage', 'MemoryVector', 'Persistence', 'ShortTermMemory', 'LongTermMemory', 'ConsolidationEngine', 'CognitiveRanking', 'FusionEngine', 'WebCacheInteligente', 'AntiRedundanciaGlobal', 'FakeNoiseDetector', 'KnowledgeGraph', 'AtualizacaoIncremental', 'SemanticCache']
        for name in expected:
            assert name in exported, f'{name} faltando em __all__'

    @pytest.mark.asyncio
    async def test_all_importable(self):
        from iaglobal.memory import Cache, Memory, MemoryStorage, MemoryVector, Persistence, ShortTermMemory, LongTermMemory, ConsolidationEngine, CognitiveRanking, FusionEngine, WebCacheInteligente, AntiRedundanciaGlobal, FakeNoiseDetector, KnowledgeGraph, AtualizacaoIncremental, SemanticCache
        assert Cache is not None
...
class TestMemoryClass:
    ...
    @pytest.mark.asyncio
    async def test_memory_save_and_append(self):
        from iaglobal.memory.memory import Memory
        m = Memory()
