...
class Snapshotter:
    ...
    def __init__(
        self, db_path: Optional[Path] = None, snapshots_dir: Optional[Path] = None
    ):
    ...
    def _init_index(self):
        ...
        try:
            with conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_snapshots_fp ON snapshots(task_fingerprint)"
                )
    ...
    def create_snapshot(self, state_data: Dict[str, Any]) -> Optional[str]:
        ...
        self._record_index(
            ...
            str(path),
            len(nodes),
            len(state_data.get("compressed", {}))
            if isinstance(state_data, dict)
            else 0,
        )

        logger.info(
            f"[SNAPSHOTTER] Snapshot criado: {snapshot_id} "
    ...
    def rollback(self, snapshot_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if not snap_id:
            logger.warning(
                "[SNAPSHOTTER] Rollback impossível: nenhum snapshot disponível"
            )
            return None
        ...
        logger.info(
            f"[SNAPSHOTTER] Rollback para snapshot {snap_id}: "
    ...
    def prune_old_snapshots(self, keep_last: int = 50):
        ...
        try:
            with conn:
                for snap in to_remove:
                    ...
                    if path.exists():
                        path.unlink()
                        logger.debug(
                            f"[SNAPSHOTTER] Removido snapshot antigo: {path.name}"
                        )
                    conn.execute(
            ...
            logger.info(
                f"[SNAPSHOTTER] Prune concluído: {len(to_remove)} snapshots removidos"
            )
        except sqlite3.Error as e:
