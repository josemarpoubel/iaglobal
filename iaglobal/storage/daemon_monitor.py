...
PASTA_MONITORADA = os.path.join(DATA_DIR, "storage")
...
class MonitorHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            dados_normalizados = bridge.processar_sugestao(
                event.src_path
            )
            ...
            if "task" not in dados_normalizados:
                logger.warning(
                    f"⚠️ Nenhuma task encontrada em {nome_arquivo}"
                )
                return
            ...
            logger.info(
                f"🚀 Invocando Multi-Agent Resolver "
                f"para task: '{task_text[:60]}...'"
            )

            # -------------------------------------------------
            # 2. Execução multi-agente
            # -------------------------------------------------
            codigo_final = resolver(
                task_text,
                max_iters=3
            )
            ...
            logger.error(
                f"❌ Erro crítico ao processar {nome_arquivo}: {str(e)}",
                exc_info=True
            )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"👁️ Monitorando diretório: {PASTA_MONITORADA}"
    )
