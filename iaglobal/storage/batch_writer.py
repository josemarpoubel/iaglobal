...
class BatchWriter:
    ...
    def __init__(
        self, db_path: Optional[Path] = None, flush_ms: int = 2000, max_batch: int = 100
    ):
    ...
    def _init_table(self):
        ...
        try:
            with conn:
                ...
                """)
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_events_fingerprint ON events(task_fingerprint)"
                )
                conn.execute(
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self._loop, daemon=True, name="batch-writer"
        )
        self._thread.start()
        logger.info(
            f"[BATCH_WRITER] Iniciado: flush_ms={self.flush_ms} max_batch={self.max_batch}"
        )
    ...
    def _write_batch(self, events: List[Event]):
        ...
        try:
            ...
            logger.error(
                f"[BATCH_WRITER] Erro ao gravar lote ({len(events)} eventos): {exc}"
            )
        finally:
            conn.close()
