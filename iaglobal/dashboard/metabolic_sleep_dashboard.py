...
class MetabolicSleepDashboard:
    ...
    async def coletar_metricas(self) -> Dict[str, Dict]:
        ...
        tarefas_processing = len([
            t for t in self.fugue._background_tasks.values() 
    ...
    async def exibir_dashboard(self) -> None:
        ...
        while self.running:
            ...
            print(f"Última atualização: {time.ctime(metricas['eficiencia']['ultima_sincronia'])}")
            print("=" * 60)
            
            # Seção FugueCompartment
            print("\n🔮 COMPARTIMENTO DE FUGA")
            print(f"  Tarefas ativas: {metricas['fugue']['total_tasks']} ",
