...
"""
    ...
    async def sussurrar_intuicao(self, tags_tarefa: List[str]) -> str:
        ...
        return (
            "\n\n".join(insights)
            if insights
            else ("Subconsciente: Sem memórias diretas correlacionadas a este padrão.")
        )
    ...
    async def ler_nota(
        self, nome: str, diretorio: Optional[Path] = None
    ) -> Optional[str]:
    ...
    async def buscar_notas(self, query: str) -> List[Dict[str, Any]]:
        ...
        if query:
            ...
            notas = [
                n
                for n in notas
                if query_lower in n.get("conteudo", "").lower()
    ...
    async def atualizar_nota(
        ...
        def _atualizar():
            ...
            if frontmatch:
                ...
                corpo = (
                    texto[frontmatch.end() :].strip() if conteudo is None else conteudo
                )
                novo_texto = f"---\n{novo_front}\n---\n\n{corpo}\n"
