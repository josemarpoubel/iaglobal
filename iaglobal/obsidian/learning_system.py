...
class IAGlobalAgentWrapper:
    ...
    def preparar_prompt_com_intuicao(self, prompt_original_usuario: str,
                                     tags_da_tarefa: List[str]) -> str:
        """Injeta memória de longo prazo no prompt antes de enviar ao LLM."""
        sussurro = self.subconscious.sussurrar_intuicao(tags_da_tarefa)
...
class LearningSystem:
    ...
    def processar_requisicao_agente(self, agente_nome: str,
                                    tarefa_texto: str,
                                    tags_contexto: Optional[List[str]] = None) -> str:
        """Processa requisição de um agente com infusão de memória subconsciente."""
        tags = tags_contexto or []

        insights = self._subconscious.obter_insight_subconsciente(tags)
