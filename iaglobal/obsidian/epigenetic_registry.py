...
class EpigeneticRegistry:
    ...
    def _epigenetic_id(
        self, agent_id: str, task_hash: str, error_type: str, event_index: int = None
    ) -> str:
    ...
    async def record_failure(
        ...
        epigenetic_id = self._epigenetic_id(
            agent_id, task_hash, error_type, event_index=event_index
        )
        ...
        logger.info(
            "🧬 [EPIGENETIC] Falha registrada: %s | %s | IVM=%.2f",
            epigenetic_id,
            error_type,
            ivm_score or 0.0,
        )
    ...
    async def record_success(
        ...
        await asyncio.to_thread(_write)
        logger.info(
            "✅ [EPIGENETIC] Sucesso registrado: %s | IVM=%.2f | Reward=%.2f",
            epigenetic_id,
            ivm_score or 0.0,
            reward_value or 0.0,
        )
            ...
            for file in self.base_path.glob("*.cbor"):
                try:
                    ...
                    if (
                        ...
                        and data.get("task_hash") == task_hash
                    ):
                        ...
                        if "timeout" in error_type:
                            ...
                            result["retry_delay"] = min(
                                result["retry_delay"] * 0.9, 1.0
                            )
    ...
    async def apply_bandit_reward(
        self, agent_id: str, task_hash: str, reward: float, ivm: float
    ) -> None:
