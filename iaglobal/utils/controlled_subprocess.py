        ...
        for arg in args[1:]:
            if not arg.startswith("-"):
                if not any(arg.startswith(p) for p in _ALLOWED_READ_PREFIXES):
                    raise CommandBlockedError(f"Path não permitido para leitura: {arg}")
        ...
        for arg in args[1:]:
            if not arg.startswith("-"):
                if not any(arg.startswith(p) for p in _ALLOWED_WRITE_PREFIXES):
                    raise CommandBlockedError(f"Path não permitido para escrita: {arg}")
            ...
            if idx + 1 < len(args):
                ...
                if not git_dir.startswith("/tmp/"):
                    raise CommandBlockedError(
                        f"Git não permitido fora de /tmp/: {git_dir}"
                    )
...
async def run_controlled(
    ...
    logger.info(
        "[CTRL-SUB] Executando: %s (timeout=%.1fs)", shlex.join(args), effective_timeout
    )
    ...
    try:
        ...
        logger.error(
            "[CTRL-SUB] Timeout (%ds) para: %s", effective_timeout, shlex.join(args)
        )
        raise
