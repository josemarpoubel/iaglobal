...
def check_installed_integrity():
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                if module_target not in valid_modules:
                    issues.append(f"Módulo ausente: '{module_target}' (chamado por {file_path.name})")
                    continue
                ...
                if target_path.is_file():
                    try:
                        with open(target_path, "r", encoding="utf-8") as tf:
                            target_tree = ast.parse(tf.read(), filename=str(target_path))
                        
                        defined_names = set()
                        for t_node in ast.walk(target_tree):
                            if isinstance(t_node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                        ...
                        for alias in node.names:
                            ...
                            if alias.name not in defined_names:
                                if f"{module_target}.{alias.name}" not in valid_modules:
                                    issues.append(f"Assinatura ausente: '{alias.name}' em '{module_target}'")
                    except Exception:
                        pass
    ...
    if issues:
        print("\n" + "!"*70, file=sys.stderr)
        print("🚨 ERRO DE INTEGRIDADE DETECTADO NA INSTALAÇÃO DO IAGLOBAL 🚨", file=sys.stderr)
        print("!"*70, file=sys.stderr)
        for issue in issues:
            print(f" -> {issue}", file=sys.stderr)
        print("!"*70 + "\n", file=sys.stderr)
        raise ImportError("A instalação do pacote 'iaglobal' está corrompida ou incompleta no ambiente atual.")
