...
class BanditPolicyEvolution:
    ...
    async def _executar_ciclo_autonomo(self) -> dict:
        ...
        providers_ruins = [
            pid
            for pid, rec in self._base.fitness_records.items()
            if rec.fitness_score < self._base.min_fitness_for_selection
            and rec.total_uses >= 5
        ]
        ...
        base.update(
            {
                ...
                "ultimo_web_learn": self._last_web_learn.isoformat()
                if self._last_web_learn
                else None,
