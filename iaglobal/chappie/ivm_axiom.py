...
class IVMMetrics:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "IVMMetrics":
        ...
        metrics = cls(
            ...
            cpu_usage_atual=cpu.get("usage_atual", 0.0)
            if (cpu := data.get("cpu", {}))
            else 0.0,
            cpu_budget=cpu.get("budget", 0.25)
            if (cpu := data.get("cpu", {}))
            else 0.25,
            cpu_efficiency_score=cpu.get("efficiency_score", 0.0)
            if (cpu := data.get("cpu", {}))
            else 0.0,
            cpu_signal_ativo=cpu.get("signal_ativo", False)
            if (cpu := data.get("cpu", {}))
            else False,
...
class IVMAxiom:
    ...
    def _iniciar_memoria_curto_prazo(self) -> None:
        ...
        try:
            ...
            logger.info(
                "[IVMAxiom] Memória de curto prazo inicializada | db=%s", self.db_path
            )
            ...
            logger.error(
                "[IVMAxiom] Falha ao inicializar memória de curto prazo: %s", e
            )
            self._stm = None
    ...
    async def atualizar_metricas(
        ...
        if tasks_completed > 0 and total_latency_ms > 0:
            ...
            metricas.avg_latency_ms = (
                metricas.total_latency_ms / metricas.tasks_completed
            )
            latency_ratio = metricas.avg_latency_ms / self.latency_baseline_ms
            metricas.energy_efficiency_score = (
                min(1.0, 1.0 / latency_ratio)
                if latency_ratio > 0
                else metricas.energy_efficiency_score
            )
        ...
        if cpu_usage is not None:
            ...
            metricas.cpu_efficiency_score = max(
                0.0, 1.0 - (metricas.cpu_usage_atual / budget) * 0.5
            )
        ...
        metricas.cooperation_score = (
            skills_score + metricas.mhc_validation_score
        ) / 2.0
        ...
        homocysteine_alert = self._verificar_homocisteina(
    ...
    def _persistir_apos_atualizacao(self, agent_name: str) -> None:
        ...
        try:
            ...
            metricas_data = {
                agent: metricas.to_dict() for agent, metricas in self._metricas.items()
            }

            # Prepara dados de histórico
            historico_data = {
                agent: record.to_dict() for agent, record in self._historico.items()
            }
            ...
            if metricas:
                ...
                chave_entrada = (
                    f"ivm_update_{agent_name}_{datetime.now(UTC).timestamp()}"
                )
                self._stm.add(
                    entrada_individual,
                    metadata={
    ...
    def get_todas_metricas(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {agent: metricas.to_dict() for agent, metricas in self._metricas.items()}
    ...
    def _ajustar_pesos_epigeneticos(self, agent_name: str) -> None:
        ...
        latency_ratio = (
            metrics.avg_latency_ms / self.latency_baseline_ms
    ...
    def _verificar_homocisteina(
        ...
        """
        pool = self._homocysteine_pool.setdefault(
            agent_name,
            {
        ...
        if (
            pool["consecutive_failures"] >= self.HOMOCYSTEINE_MIN_RECENT_TASKS
    ...
    async def _registrar_homocisteina_obsidian(
        self, agent_name: str, alert_data: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            logger.info(
                "[IVMAxiom] Alerta de homocisteína registrado no Obsidian | agent=%s",
                agent_name,
            )
        except Exception as e:
            logger.warning(
                "[IVMAxiom] Falha ao registrar homocisteína no Obsidian: %s", e
            )
    ...
    def get_homocysteine_status(self) -> Dict[str, Any]:
        ...
        agents_em_homocisteina = sum(
            1
            for p in self._homocysteine_pool.values()
