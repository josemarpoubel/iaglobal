...
from iaglobal.evolution.skills.native.skill_executor import skill_executor
...
class EvolutionEngine:
    ...
    def __init__(
        ...
        meta_designer: Optional[
            MetaAgentDesigner
        ] = None,  # ⬅️ ADICIONADO PARA TESTABILIDADE
    ):
        self.graph = graph
        self.adversarial = darwin.DynamicAdversarialEnvironment(
        ...
        self.strategies = strategies or ["coding", "research", "fast", "explore"]
    ...
    def register_lineage(
        self, node, event_type: str, parent_name: str, parent_fitness: float = 0.0
    ):
    ...
    def _finalize_cycle(self):
        ...
        logger.info(
            "🏁 Ciclo de evolução %d finalizado com sucesso.", self.generation - 1
        )
    ...
    def _perform_canonicalize(self):
        ...
        logger.info(
            "[EVO] Grafo canonicalizado: hash=%s",
            self.graph._graph_hash[:16] if self.graph._graph_hash else "N/A",
        )
    ...
    def _finalize_evolution_step(self):
        ...
        logger.info(
            "[EVO] Passo finalizado: %d nós evo ativos, geração %d",
            evo_count,
            self.generation,
        )
    ...
    def _crossover(self, parent_a: Node, parent_b: Node) -> Tuple[Node, dict]:
        ...
        child_name = _short_name(
            parent_a.name, "x", parent_b.name, str(self.generation)
        )
        child.name = child_name
        child.node_type = "hybrid"
        child.seed_id = ""
        child.mutation_id = _short_name(
            "cx", str(self.generation), secrets.token_hex(4)
        )

        # Estratégia: herda do pai mais fit (ou aleatório se sem fitness)
        fa = (
            parent_a.fitness()
            if hasattr(parent_a, "fitness") and callable(parent_a.fitness)
            else 0.0
        )
        fb = (
            parent_b.fitness()
            if hasattr(parent_b, "fitness") and callable(parent_b.fitness)
            else 0.0
        )
        ...
        child.mutation_rate = (
            getattr(parent_a, "mutation_rate", 0.1)
            + getattr(parent_b, "mutation_rate", 0.1)
        ) / 2

        # Linhagem: registra ambos os pais
        parent_a_lid = parent_a.lineage[-1].lineage_id if parent_a.lineage else ""
        parent_a_marker = (
            parent_a.lineage[-1].lineage_marker if parent_a.lineage else ""
        )
        parent_b_lid = parent_b.lineage[-1].lineage_id if parent_b.lineage else ""
        parent_b_marker = (
            parent_b.lineage[-1].lineage_marker if parent_b.lineage else ""
        )
    ...
    async def _evolve_impl(self, strategy: Optional[EvolutionStrategy] = None):
        ...
        logger.info(
            "🧬 [ASYNC] Iniciando ciclo de evolução: gen=%d, strategy=%s",
            self.generation,
            strategy.name,
        )
        ...
        logger.info(
            "✅ [ASYNC] Ciclo evolutivo concluído com sucesso (strategy=%s).",
            strategy.name,
        )
    ...
    def _is_evo_cloneable(self, name: str) -> bool:
        ...
        if not node:
            logger.warning(
                "[EVO] Tentativa de checar clone de nó inexistente: %s", name
            )
            return False
    ...
    async def seed_evo_population_async(self):
        ...
        from iaglobal.evolution.skills.utils.skill_quarantine import quarantine
        ...
        for name, node in candidates:
            ...
            # Validações rápidas
            if (
                quarantine.is_quarantined(skill_name)
                or not skill_executor.can_execute(skill_name)
                or not self._is_evo_cloneable(name)
            ):
                continue
    ...
    def _initialize_seed_workdir(self, name: str, skill: str):
        ...
        try:
            ...
            logger.error(
                "[EVO] Falha ao inicializar WorkDir para '%s': %s", name, str(e)
            )
    ...
    async def _create_synthetic_evo_seeds_async(self):
        ...
        for src_name, strategy in base_nodes:
            ...
            parent_lid = (
                src_node.lineage[-1].lineage_id if src_node and src_node.lineage else ""
            )
            parent_marker = (
                src_node.lineage[-1].lineage_marker
                if src_node and src_node.lineage
                else ""
            )
            await asyncio.to_thread(
                self._record_lineage,
            ...
            # 3. I/O Isolado
            await asyncio.to_thread(
                self._initialize_seed_workdir, seed_name, f"sintetico_{strategy}"
            )
    ...
    def _record_lineage(
        ...
        *,
        ...
        parent_lid = parent_lineage_id or (
            node.lineage[-1].lineage_id if node.lineage else ""
        )
        parent_marker = parent_lineage_marker or (
            node.lineage[-1].lineage_marker if node.lineage else ""
        )
        ...
        entry = LineageEntry(
            ...
            fitness_delta=node.fitness() - parent_fitness
            if parent_fitness > 0
            else 0.0,
            timestamp=time.time(),
        ...
        logger.debug(
            "[LINEAGE] Registrado: %s → %s (marker=%s)",
            node.name,
            event_type,
            lineage_marker[:8],
        )
    ...
    def _select_survivors(self, pressure: Optional[float] = None):
        ...
        logger.info(
            "🧬 Seleção (pressure=%.1f): %d candidatos | %d sobreviventes | %d eliminados.",
            pressure,
            len(evo_nodes),
            len(survivors),
            len(eliminated),
    ...
    async def mutate_nodes_async(self):
        ...
        mutated = {}
        evo_targets = [
            n for n in self.graph.nodes.values() if n.name not in self.CORE_NODE_NAMES
        ]

        logger.info(
            "🔀 Iniciando mutação adaptativa para %d agentes...", len(evo_targets)
        )
        ...
        for node in evo_targets:
            ...
            if (
                not (stats.get("strategy_shifted") or stats.get("model_drifted"))
                and len(evo_targets) <= 2
            ):
                new_node.strategy = secrets.choice(
                    ["coding", "research", "fast", "explore", "general"]
                )
            ...
            if stats.get("strategy_shifted") or stats.get("model_drifted"):
                ...
                logger.info(
                    "🔀 Variação criada: '%s' (estratégia: %s)",
                    new_node.name,
                    new_node.strategy,
                )
    ...
    def _mutate_node(self, node: Node) -> tuple:
        ...
        if secrets.randbelow(1000000) < int(base_rate * 1000000):
            candidates = (
                list(self.task_strategies) if self.task_strategies else self.strategies
            )
            new_node.strategy = secrets.choice(candidates)
            stats["strategy_shifted"] = True

        # 2. Model Drift (usa self.mutation_rate que foi ajustado pela estratégia)
        effective_rate = self.strategy_mutation_rates.get(
            node.strategy, self.mutation_rate
        )
        ...
        for name, node in self.graph.nodes.items():
            ...
            for entry in node.lineage:
                if entry.event_type in ("seed", "mutation"):
                    ...
                    p_list = [
                        p.strip() for p in entry.parent_name.split(" x ") if p.strip()
                    ]
