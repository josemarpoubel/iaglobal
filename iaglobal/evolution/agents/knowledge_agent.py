...
class KnowledgeAgent:
    ...
    def _load(self) -> None:
        try:
            if KNOWLEDGE_FILE.exists():
                ...
                logger.info(
                    "[KNOWLEDGE] %d entradas carregadas de %s",
                    len(self._cache),
                    KNOWLEDGE_FILE,
                )
            else:
    ...
    def _is_duplicate(self, title: str, content: str, threshold: float = 0.85) -> bool:
        for e in self._cache:
            ...
            if e.get("title") == title.strip():
                ...
                if (
                    SequenceMatcher(None, e.get("content", ""), content.strip()).ratio()
                    > threshold
                ):
                    return True
        return False
    ...
    def extract_and_store(self, task: str, source_data: str) -> None:
        ...
        if not snippets:
            snippets = [
                s.strip() for s in source_data.split("\n") if len(s.strip()) > 50
            ][:5]
        for snippet in snippets[:5]:
            title = snippet[:60].strip()
            self.store(
                "best_practice", title, snippet, tags=["auto_extracted"], source=task
            )
    ...
    def store(
        ...
        if self._is_duplicate(title, content):
            logger.debug(
                "[KNOWLEDGE] Duplicata ignorada: [%s] %s", category, title[:60]
            )
            return
    ...
    def retrieve_relevant(
        self, task: str, max_results: int = 3
    ) -> List[Dict[str, Any]]:
        """
