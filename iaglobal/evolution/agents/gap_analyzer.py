...
class GapAnalyzer:
    ...
    SPECIALTY_TRIGGERS: Dict[str, Set[str]] = {
        "security_specialist": {
            "login", "autenticacao", "authentication", "senha", "password",
            "api", "payment", "pagamento", "formulario", "form",
            "jwt", "oauth", "token", "csrf", "xss", "sql injection",
        },
        "ux_reviewer": {
            "ux", "user experience", "acessibilidade", "accessibility",
            "mobile", "responsivo", "responsive", "fluxo", "flow",
            "usabilidade", "usability", "clareza", "clear",
        },
        "architecture_reviewer": {
            "arquitetura", "architecture", "escalabilidade", "scalability",
            "acoplamento", "coupling", "design pattern", "solid",
            "microservicos", "microservices", "clean architecture",
        },
        "performance_auditor": {
            "performance", "performance", "otimizacao", "optimization",
            "cache", "lazy loading", "slow", "lento", "carregamento",
        },
    }
    ...
    def detect_gaps(cls, task: str, graph: ExecutionGraph) -> List[Dict]:
        ...
        for specialty in mandatory:
            if not cls._specialty_exists(graph, specialty):
                gaps.append(cls._build_recommendation(specialty, task_type, strategies, prioridade="alta"))
        ...
        for specialty, triggers in cls.SPECIALTY_TRIGGERS.items():
            ...
            if any(trigger in task_lower for trigger in triggers):
                if not cls._specialty_exists(graph, specialty):
                    gaps.append(cls._build_recommendation(specialty, task_type, strategies, prioridade="media"))

        if gaps:
            logger.info(
                "[GAP] %d lacuna(s) detectada(s) para task type=%s: %s",
                len(gaps), task_type, [g["nome"] for g in gaps]
            )
    ...
    @classmethod
    def _build_recommendation(cls, specialty: str, task_type: str,
                              strategies: Set[str], prioridade: str = "media") -> Dict:
