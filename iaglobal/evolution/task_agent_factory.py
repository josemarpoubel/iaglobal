...
class TaskAgentFactory:
    ...
    def inject_specialists(
        cls, graph: ExecutionGraph, prompt: str, task_type: str = "default"
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"[TASK_AGENT] Injetando {len(agentes_specs)} especialistas para o cenário '{task_type}'"
            )

            # Coleta os nós existentes para calcular dependências dinâmicas reais e evitar nós órfãos
            existing_node_names = (
                set(graph.get_all_node_names())
                if hasattr(graph, "get_all_node_names")
                else set()
            )
            ...
            logger.critical(
                f"[TASK_AGENT] Falha crítica na injeção de topologia dinêmica: {e}",
                exc_info=True,
            )
    ...
    def _criar_agente(
        ...
        from iaglobal.evolution.skills.native.skill import SKILL_CODER
        ...
        if not skill_obj:
            logger.warning(
                f"[TASK_AGENT] Skill base '{skill_name_real}' não encontrada no registry. Usando CODER como fallback."
            )
        ...
        if skill_name_real in ["critic", "security", "performance"]:
            coders_ativos = [
                ...
                if "developer" in n or "coder" in n or "generalist" in n
            ]
            if coders_ativos:
        ...
        node = Node(
            name=node_name,
            run=skill_obj.execute
            if hasattr(skill_obj, "execute")
            else skill_obj.run_fn,  # Vincula a execução real!
