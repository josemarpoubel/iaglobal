...
class EvolutionRuntime:
    ...
    def __init__(self, evolver=None, interval: int = 60):
        ...
        logging.getLogger("iaglobal.evolution").info(
            "🚀 EvolutionRuntime inicializado."
        )
    ...
    def set_strategy(self, strategy) -> None:
        ...
        logger.info(
            f"[RUNTIME] Estratégia alterada para {strategy.__class__.__name__} (intervalo={strategy.interval}s)"
        )
        ...
        try:
            while self._running:
                ...
                try:
                    ...
                    result = await self.evolver.evolve_async(
                    ...
                    mut_count = (
                        result.get("mutations_count", 0)
                        if isinstance(result, dict)
                        else 0
                    )
                    logger.info(
                        f"[CICLO #{self.cycles}] Sucesso via {strategy_name}. Mutações: {mut_count}"
                    )

                    # Backoff dinâmico descendente se estiver estável
                    if self._consecutive_stable >= 3:
                        self.interval = max(
                            self.base_interval, int(self.interval * 0.8)
                        )
