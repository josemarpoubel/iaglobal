...
class CollapseReport:
    ...
    def summary(self) -> str:
        ...
        lines.append(f"  Overall: {'⚠️  COLLAPSED' if self.collapsed else '✅  Healthy'} (score={self.overall_score:.3f})")
        lines.append("")
        for ind in self.indicators:
            icon = "❌" if ind.collapsed else "✅"
            lines.append(f"  {icon} {ind.name:25s} score={ind.score:.3f} (threshold={ind.threshold}) | {ind.message}")
        if self.warnings:
            lines.append("")
...
class CollapseDetector:
    ...
    def detect(self, graph, engine, metrics=None) -> CollapseReport:
        evo_nodes = [
            ...
            if n.name not in engine.CORE_NODE_NAMES
        ]
        ...
        if metrics is not None and len(metrics.snapshots) >= 2:
            ...
            indicators.append(CollapseIndicator(
                name="fitness_stagnation", score=1.0,
                threshold=self.thresholds["fitness_stagnation"],
                value=0, message="not enough data",
            ))
        ...
        if metrics is not None and len(metrics.snapshots) >= 2:
            ...
            indicators.append(CollapseIndicator(
                name="premature_convergence", score=1.0,
        ...
        overall = sum(
            ind.score * weights.get(ind.name, 0.15)
        ...
        if report.collapsed:
            ...
            for ind in report.collapsed_indicators:
                logger.warning("  ❌ %s: score=%.3f (threshold=%.3f)", ind.name, ind.score, ind.threshold)
    ...
    def _check_strategy_entropy(self, evo_nodes: list) -> CollapseIndicator:
        if not evo_nodes:
            return CollapseIndicator(
                name="strategy_entropy", score=0.0,
                threshold=self.thresholds["strategy_entropy"],
                value=0, message="no evo nodes",
            )
        counts: Dict[str, int] = {}
        for n in evo_nodes:
        ...
        return CollapseIndicator(
            name="strategy_entropy", score=normalized,
            threshold=self.thresholds["strategy_entropy"],
            value=entropy, message=msg,
        )

    def _check_fitness_variance(self, evo_nodes: list) -> CollapseIndicator:
        if len(evo_nodes) < 2:
            return CollapseIndicator(
                name="fitness_variance", score=1.0,
                threshold=self.thresholds["fitness_variance"],
                value=0, message="too few nodes",
            )
        ...
        return CollapseIndicator(
            name="fitness_variance", score=score,
            threshold=self.thresholds["fitness_variance"],
            value=variance, message=msg,
        )
    ...
    def _check_fitness_stagnation(self, metrics) -> CollapseIndicator:
        ...
        return CollapseIndicator(
            name="fitness_stagnation", score=score,
            threshold=1.0 / (max_stagnant + 1),
            value=stagnant, message=msg,
        )
    ...
    def _check_genetic_diversity(self, evo_nodes: list) -> CollapseIndicator:
        ...
        return CollapseIndicator(
            name="genetic_diversity", score=ratio,
            threshold=self.thresholds["genetic_diversity"],
            value=ratio, message=msg,
        )

    def _check_premature_convergence(self, metrics, evo_count: int) -> Tuple[CollapseIndicator, Optional[str]]:
        ...
        return (
            CollapseIndicator(
                name="premature_convergence", score=score,
                threshold=0.5,
                value=drop_ratio, message=msg,
            ),
            warn,
        )
