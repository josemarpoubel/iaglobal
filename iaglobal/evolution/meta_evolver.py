...
MAX_TRIALS_WINDOW = 200  # Limite máximo de histórico para conter estouro de memória e lentidão de I/O
...
class MetaEvolver:
    ...
    def __init__(self, path: Optional[Path] = None, classifier_memory: Optional[ClassifierMemory] = None):
            ...
            try:
                if self.path.exists() and self.path.stat().st_size > 0:
                    with open(self.path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        self.current_params = EvolutionParams.from_dict(data.get("current_params", {}))
                        
                        raw_trials = data.get("trials", [])
                        # Garante a importação cortando o excesso se o arquivo antigo estivesse inflado
                        self.trials = [MetaTrial.from_dict(t) for t in raw_trials[-MAX_TRIALS_WINDOW:]]
                        
                        if self.trials:
                            self._best_improvement = max(t.improvement for t in self.trials)
            except json.JSONDecodeError:
                logger.error("[META] Arquivo meta_evolution.json corrompido. Inicializando novos parâmetros de fábrica.")
    ...
    def _save(self):
        ...
        try:
            ...
            logger.error("[META-CRITICAL] Falha catastrófica ao persistir metadados evolutivos: %s", e)

    def record_trial(self, params: EvolutionParams, improvement: float, task_type: str = "general"):
        """Regista o resultado de um ciclo de evolução, aplicando a janela deslizante de RAM/Disk."""
        with self._lock:
            trial = MetaTrial(params=params, improvement=improvement, task_type=task_type)
    ...
    def adapt_params(self, last_improvement: float):
        ...
        if avg_improvement < 5.0:
            ...
            self.current_params.mutation_rate = max(0.05, self.current_params.mutation_rate * 0.8)
            self.current_params.crossover_rate = min(0.5, self.current_params.crossover_rate * 1.2)
            logger.info("[META] Convergência estagnada (Melhoria média: %.2f) — Ajustando crossover=%.3f mutation=%.3f",
                        avg_improvement, self.current_params.crossover_rate, self.current_params.mutation_rate)
        else:
            # Ótimo ritmo evolutivo: Minimiza taxas para consolidar o código estável gerado
            self.current_params.mutation_rate = max(0.05, self.current_params.mutation_rate * 0.7)
            self.current_params.exploration_rate = max(0.05, self.current_params.exploration_rate * 0.8)
            logger.info("[META] Ritmo evolutivo saudável (Melhoria média: %.2f) — Consolidando: mutation=%.3f exploration=%.3f",
                        avg_improvement, self.current_params.mutation_rate, self.current_params.exploration_rate)

    def get_stats(self) -> Dict[str, Any]:
        with self._lock:
            avg = (sum(t.improvement for t in self.trials) / len(self.trials)) if self.trials else 0.0
            return {
                "current_params": self.current_params.to_dict(),
