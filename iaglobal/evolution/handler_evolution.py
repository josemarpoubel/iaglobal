...
class _SourceTupleMutator(_BaseMutator):
    ...
    ALL_SOURCES = [
        "multi_coder", "coder", "debug_coder",
        "backend_builder", "frontend_builder", "api_builder",
        "database_builder", "test_generator",
        "search", "knowledge", "architect",
    ]
...
class _LogLevelMutator(_BaseMutator):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> ast.AST:
        node = self.generic_visit(node)
        if isinstance(node.value, ast.Name) and node.value.id == "logger" and node.attr in self.LEVELS:
            if self._maybe(0.25):
        ...
        for kw in node.keywords:
            if kw.arg and self.INT_PARAM_NAMES.search(kw.arg):
                if isinstance(kw.value, ast.Constant) and isinstance(kw.value.value, int):
                    if self._maybe(0.3):
                        kw.value = ast.Constant(value=self._jitter(kw.value.value, fraction=0.3))
                        self._mutated = True
        return node
...
def _validate_import(name: str) -> bool:
    ...
    try:
        ...
        _log.warning("[HANDLER-EVO] Falha ao importar no_%s:\n%s", name, traceback.format_exc())
        return False
...
class HandlerEvolver:
    ...
    def __init__(self, engine: Any, generation: int = 0):
        ...
        self._core_names = getattr(engine, "CORE_NODE_NAMES", []) if hasattr(engine, "CORE_NODE_NAMES") else []
        self._rng = secrets.SystemRandom()
    ...
    def _mutate_handler(self, name: str, source: str) -> Optional[str]:
        ...
        mutators = self._rng.sample(self.MUTATORS, min(MAX_MUTATIONS_PER_HANDLER, len(self.MUTATORS)))
        modified = tree
        applied: List[str] = []
    ...
    def _crossover_handlers(self, parent_a: Tuple[str, str], parent_b: Tuple[str, str]) -> Optional[Tuple[str, str]]:
        ...
        _log.info("[HANDLER-EVO] Crossover %s x %s → %s [%s]", name_a, name_b, new_name, operator)
    ...
    def _register_handler(self, name: str, source: str) -> bool:
        ...
        dynamic_registry.register_dynamic(skill, template_type="evolved", template_prompt="")
    ...
    def evolve(self) -> Dict[str, Any]:
        ...
        _log.info(
            ...
            stats["eligible"], stats["parents"],
            stats["mutations"], stats["crossovers"],
            stats["registered"], stats["errors"],
        )
        return stats
