...
@dataclass(
    frozen=True
)  # Tornar imutável previne mutações colaterais e dispensa deepcopy profundo
class ReplayNode:
    name: str
    strategy: str
    fitness: float
...
class EvolutionReplay:
    ...
    def reconstruct_snapshots(
        ...
        history: List[Any] = (
            self.engine.get_lineage_history()
            if hasattr(self.engine, "get_lineage_history")
            else []
        )
        ...
        for gen in range(1, target_generation + 1):
            ...
            avg_fit = (
                (sum(n.fitness for n in all_nodes) / total_count)
                if total_count > 0
                else 0.0
            )
