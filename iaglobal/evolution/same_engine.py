    ...
    def _load(self):
        with self._io_lock:
            try:
                ...
                logger.error("[SAME] Arquivo de pool corrompido detectado. Reinicializando banco local.")
...
class MethylationInhibitor:
    ...
    def can_mutate(self, agent_name: str, cost: int, critical: bool = False) -> bool:
        ...
        if current_balance < self.threshold:
            logger.info("[SAME-INHIBITOR] SAMe baixo (%d < %d) — mutação não-crítica bloqueada para '%s'",
                       current_balance, self.threshold, agent_name)
            return False
        return self.pool.can_afford(agent_name, cost)
...
class SAMeBudgetTracker:
    ...
    def _check_cycle(self, agent_name: str):
        now = time.time()
        if agent_name not in self._cycle_start or (now - self._cycle_start[agent_name]) > self.CYCLE_WINDOW:
...
def rewrite_prompt(agent_name: str, current_prompt: str, error_history: str = "") -> Optional[str]:
    ...
    if not same_pool.spend(agent_name, COST_FINE_TUNE):
        logger.warning("[SAME-MUTATE] SAMe insuficiente para rewrite_prompt em '%s'", agent_name)
        return None
