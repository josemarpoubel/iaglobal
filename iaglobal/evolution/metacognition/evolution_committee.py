...
class EvolutionCommittee:
    ...
    @classmethod
    async def _register_to_omnimind(
        cls, evaluations: List, task_context: str, decision_text: str
    ) -> None:
            ...
            for e in evaluations:
                if e["approved"]:
                    await subconscious.escrever_longo_prazo(
                        f"evolucao_{e['skill_name']}_{int(time.time())}",
    ...
    async def _persist_to_memory(cls, evaluations: List, task_context: str) -> None:
        ...
        try:
            ...
            await add_ltm(
                "evolution_committee",
                {"evaluations": evaluations, "task_snippet": task_context},
            )
            for e in evaluations:
                await add_stm(
                    f"skill_eval:{e['skill_name']}", {"approved": e["approved"]}
                )
        except Exception as e:
        ...
        try:
            for e in evaluations:
                ...
                if skill:
                    ...
                    skill.metadata["evolution_status"] = (
                        "approved" if e["approved"] else "rejected"
                    )
                    skill.metadata["committee_evaluated_at"] = str(time.time())
                    logger.info(
                        "[EVOLUTION_COMMITTEE] Skill %s %s",
                        skill_name,
                        "aprovada" if e["approved"] else "rejeitada",
                    )
        except Exception as e:
    ...
    def _check_compatibility(cls, skill_name: str, skill: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "rationale": "Conflito de nome detectado"
            if name_conflict
            else "Compatível",
        }
    ...
    @classmethod
    def _analyze_cost(
        cls, skill_name: str, severity: str, skill: Any
    ) -> Dict[str, Any]:
        score = 3
        if severity == "high":
            score = 5
...
async def _run_evolution_committee(ctx: dict) -> dict:
    logger.info(
        "[LIFE-SIGNAL] _run_evolution_committee invoked | ctx_keys=%s",
        sorted(ctx.keys()),
    )
