...
class PipelineUpdater:
    ...
    def _add_to_graph(cls, graph, updates: list) -> int:
        ...
        for upd in updates:
            ...
            if skill is None:
                logger.warning("[PIPELINE] Skill '%s' não encontrada no registry", skill_name)
                continue
            run_fn = skill.run_fn if skill.run_fn and callable(skill.run_fn) else (
...
async def _run_pipeline_updater(ctx: dict) -> dict:
    logger.info("[LIFE-SIGNAL] _run_pipeline_updater invoked | ctx_keys=%s", sorted(ctx.keys()))
