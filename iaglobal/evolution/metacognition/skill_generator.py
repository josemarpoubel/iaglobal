...
class MetaSkillGenerator:
    ...
    async def generate(cls, ctx: dict) -> Dict[str, Any]:
        ...
        skill_gaps = gap_result.get("skill_gaps", gaps) if isinstance(gap_result, dict) else gaps
        ...
        for gap in skill_gaps:
            ...
            if gap_type != "recurrent_error" and gap_type != "low_score":
                skipped.append({"description": gap_desc[:80], "reason": "tipo não elegível"})
                continue
            ...
            try:
                ...
                logger.warning("[SKILL-GEN] Falha ao registrar skill '%s': %s", skill_name, e)
