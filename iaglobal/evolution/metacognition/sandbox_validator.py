...
class SandboxValidator:
    ...
    async def validate(cls, ctx: dict) -> Dict[str, Any]:
        ...
        generated = (
            skill_result.get("generated_skills", [])
        ...
        for g in generated:
            ...
            valid = cls._validate_skill_metadata(
                skill.to_dict() if hasattr(skill, "to_dict") else {}
            )
            sandbox_result = None
...
"""
            ...
            sandbox_valid = sandbox_result is None or sandbox_result.get(
                "sucesso", False
            )
            ...
            results.append(
                {
                    ...
                    if passed
                        ...
                        valid.get("error", "")
                        or (
                            sandbox_result.get("erro", "unknown")
                            if sandbox_result
                            else "unknown"
                        )
                    ),
...
async def _run_sandbox_validator(ctx: dict) -> dict:
    logger.info(
        "[LIFE-SIGNAL] _run_sandbox_validator invoked | ctx_keys=%s", sorted(ctx.keys())
    )
