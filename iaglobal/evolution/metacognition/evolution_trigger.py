...
class EvolutionTrigger:
    ...
    async def trigger(cls, ctx: dict) -> Dict[str, Any]:
        ...
        same_used = 0
        if triggered:
            if not same_inhibitor.can_mutate(
                "evolution_trigger", COST_CREATE_SKILL, critical=True
            ):
                triggered = False
                ...
                try:
                    ...
                    await engine.evolve()
                    logger.info(
                        "[EVO-TRIGGER] Ciclo evolutivo disparado: %s (SAMe restante: %d)",
                        reason,
                        same_pool.balance("evolution_trigger"),
                    )
                    from iaglobal.evolution.metacognition.evaluator import (
                        PipelineEvaluator,
                    )

                    PipelineEvaluator._evolution_count += 1
                    from iaglobal.evolution.meta_evolver import (
                        meta_evolver,
                        EvolutionParams,
                    )
        ...
        try:
            outcome_tracker.record(
                ExecutionOutcome(
                    ...
                    fingerprint=str(
                        ctx.get(
                            "execution_id", ctx.get("input", {}).get("task", "unknown")
                        )
                    ),
                    latency_ms=0,
                    token_cost=0,
