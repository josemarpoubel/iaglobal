...
class MetabolicLifecycleManager:
    ...
    async def process_agent_viability(self, agent: EvoAgent):
        ...
        if acao == "apoptose":
            logger.warning(
                "[%s] 💀 Gatilho de Apoptose ativado por IVM crítico.", agent.name
            )
            await agent.apoptose(reason=f"metabolic_collapse_{motivo}")

        elif acao == "mitose":
            logger.info(
                "[%s] ✨ Gatilho de Mitose ativado por IVM de excelência.", agent.name
            )
            try:
                # Replicação baseada em sucesso
                child = await agent.replicate(
                    mutation_hint="metabolic_excellence_clone"
                )
                logger.info(
                    "[%s] Mitose concluída: Filho [%s] gerado.", agent.name, child.name
                )
