...
GENOME_PATH = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "best_genome.json"
TELEMETRY_PATH = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "ga_telemetry.json"
...
def _save_genome(data: dict):
    ...
    GENOME_PATH.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
...
def _save_telemetry(data: dict):
    ...
    TELEMETRY_PATH.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
...
async def _persist_biological_memory_async(
    ...
    try:
        ...
        if prev_fitness is not None and best_fitness < prev_fitness * 0.85:
            ...
            logger.info("[GA] Vacina registrada para genoma %s (queda de fitness)", genome_id)
        ...
        if loop.is_running():
            asyncio.ensure_future(
                _persist_biological_memory_async(
                    generation, best_weights, best_fitness,
                    avg_fitness, prev_genome_id, prev_fitness,
                )
            )
        else:
...
class GARunner:
        ...
        if not self.initialized:
            ...
            if genome and "weights" in genome:
                self.population.individuals = [
                    Individual(weights=genome["weights"])
                ]
                ...
                logger.info("[GA] População carregada do genoma salvo (gen=%d)", self.population.generation)
            else:
                self.population.initialize()
                logger.info("[GA] População inicializada com %d indivíduos", self.population.size)
    ...
    def step(self) -> dict[str, Any]:
        ...
        logger.info(
            ...
            self.population.generation, best.fitness, avg_fit, best.to_dict(),
        )
