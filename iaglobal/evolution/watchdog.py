...
class EvolutionaryWatchdog:
    ...
    def analyze(self) -> List[Dict[str, Any]]:
        ...
        if not self.ancestry_path.exists():
            logger.debug("[WATCHDOG] ancestry_tree nao encontrado: %s", self.ancestry_path)
            return []
        ...
        if patterns:
            ...
            for p in patterns:
                logger.info("[WATCHDOG]   task_hash=%s count=%d conf=%.2f sum='%s'",
                           p["task_hash"], p["count"], p["confidence"], p["task_summary"][:50])
    ...
    def register_tool_from_pattern(self, pattern: Dict[str, Any], code: str) -> Optional[str]:
        ...
        try:
            ...
            if name:
                logger.info("[WATCHDOG] Tool registrada do padrao evolutivo: %s (task_hash=%s)",
                           name, pattern["task_hash"])
