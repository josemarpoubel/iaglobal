...
class SkillExecutor:
    ...
    async def execute(
        ...
        skill = self.registry.get_version(skill_name, version) if version else self.registry.get(skill_name)
        ...
        if not skill.run_fn:
            logger.warning("[SKILL] '%s' sem run_fn. Redirecionando para barramento genérico da instância.", skill_name)
            ...
            try:
                ...
                return {"status": "delegated_to_node", "skill": skill_name, "context": contract_ctx}
            except Exception as e:
                raise SkillExecutionError(f"Erro no barramento de fallback para '{skill_name}': {e}")
    ...
    async def execute_with_fallback(
        ...
        try:
            ...
            alternatives = self.registry.find_alternatives(skill_name, available, _visited)
