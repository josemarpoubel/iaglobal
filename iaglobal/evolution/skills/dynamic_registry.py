...
class DynamicSkillRegistry:
    ...
    def __init__(self, registry: SkillRegistry = _global_registry, db_path: Union[str, Path] = CORE_DB):
        ...
        self._db_lock = threading.Lock()  # Protege as transações de escrita do SQLite contra concorrência externa
        self._init_tables()
        self._loaded = False
    ...
    def register_dynamic(self, skill: Skill, template_type: str = "llm", template_prompt: str = "") -> bool:
    ...
    def _load_dynamic_skills(self):
        with self._db_lock:
            ...
            try:
                ...
                if rows:
                    logger.info(f"[DYNAMIC-SKILL] Sincronizadas {len(rows)} skills dinâmicas do SQLite.")
            finally:
                conn.close()
