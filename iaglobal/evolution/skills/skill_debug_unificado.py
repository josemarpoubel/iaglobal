...
class SkillDebugUnificado(Skill):
    ...
    async def execute(self, task: Task) -> str:
        ...
        logger.info(
            ...
            len(code), len(lsp_errors), task_desc[:50] if task_desc else "N/A",
        )
        ...
        logger.info(
            ...
            result.success, result.attempts, elapsed,
        )
    ...
    def build_prompt_com_lsp(
        ...
        if "syntax" in erro_principal.lower() or "'('" in erro_principal:
            tipo = "sintaxe"
            instrucao = "Corrija a sintaxe Python. Verifique parênteses, colchetes, indentação."
