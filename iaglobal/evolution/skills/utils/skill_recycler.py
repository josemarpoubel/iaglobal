...
class SkillRecycler:
    ...
    @classmethod
    def recycle(
        cls, max_generations: int = MAX_GENERATIONS_WITHOUT_IMPROVEMENT
    ) -> Dict[str, Any]:
        recycled = []
        total = 0
        ...
        for skill in skills:
            ...
            if skill.description:
                fragments.append(
                    {"type": "description", "content": skill.description[:200]}
                )
            if skill.name:
            ...
            skill_registry.unregister(skill.name) if hasattr(
                skill_registry, "unregister"
            ) else None

        return {
            "recycled": recycled,
            "count": total,
            "total_fragments": sum(r["fragments"] for r in recycled),
        }
