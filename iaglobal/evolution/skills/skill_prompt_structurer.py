...
async def run_prompt_structurer(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[PROMPT_STRUCTURER] Prompt enriquecido com CoT + JSON constraint (%d chars -> %d chars)",
                result["original_length"], result["structured_length"])
    ...
    if provider_output:
        try:
            ...
            logger.warning("[PROMPT_STRUCTURER] Provider output não é JSON válido. Acionando autocorreção.")
