...
logger = get_logger("iaglobal.agents.skill_generator_agent")
...
class SkillGeneratorAgent(AgentBase):
    ...
    def analyze_and_generate(self) -> List[Dict]:
        ...
        logger.info(
            f"[SKILL-GEN] Iniciando análise: {kb_count} skills dinâmicas existentes"
        )
        ...
        logger.info(
            f"[SKILL-GEN] Análise concluída: {len(generated)} novas skills "
    ...
    def generate_skill_from_pattern(
        ...
        run_fn = (
            fn_factory()
            if fn_factory
            else make_dynamic_run_fn(name, "llm", description)
        )
        ...
        registered = dynamic_registry.register_dynamic(
            skill, template_type="llm", template_prompt=description
        )
    ...
    def _generate_from_kb_entries(self) -> List[Dict]:
        ...
        for entry_type, count in type_counts.items():
            if count < 2:
                logger.debug(
                    f"[SKILL-GEN] {entry_type}: apenas {count} entry(s), mínimo 2"
                )
                continue
            ...
            logger.info(
                f"[SKILL-GEN] Gerando skill para {entry_type} ({count} entries)"
            )
    ...
    def _generate_from_concepts(self) -> List[Dict]:
        ...
        logger.debug(
            f"[SKILL-GEN] Conceitos frequentes (>=3): {len(high_freq)} de {len(top)}"
        )
        for concept in high_freq[:5]:
            ...
            logger.info(
                f"[SKILL-GEN] Gerando skill para conceito: {concept['name']} "
    ...
    def _generate_from_faqs(self) -> List[Dict]:
        ...
        for q, a, freq in faqs:
            ...
            logger.info(
                f"[SKILL-GEN] Gerando skill para FAQ: {q[:40]}... (freq={freq})"
            )
            skill = self.generate_skill_from_pattern(
                name=skill_name,
