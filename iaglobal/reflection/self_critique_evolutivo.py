...
class SelfCritiqueEvolutivo(SelfCritique):
    ...
    def evaluate(self, output: str, contexto: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            tipo, critique["score"], critique["line_count"],
        )
    ...
    def _avaliar_codigo(
        ...
        erros_import = [e for e in analise["erros"] if "import" in e.get("msg", "").lower()]
        if not erros_import:
    ...
    def _avaliar_testes(
        ...
        if codigo_original and any(
            fn in testes for fn in ["def ", "test_", "Test"]
        ):
    ...
    def gerar_sugestoes_refinamento(self, critique: Dict[str, Any]) -> List[str]:
        ...
        if "imports_problematicos" in fraquezas:
            sugestoes.append("Verifique imports: remova não utilizados ou corrija nomes")
