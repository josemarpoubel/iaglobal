...
from iaglobal.observability.registry import ServiceInstance, registry as default_registry
from iaglobal.observability.load_balancer import PhospholipidLoadBalancer, load_balancer as default_lb
...
class PhospholipidSearchBridge:
    ...
    def register_search_service(
        ...
        """
        full_name = name if name.startswith(SEARCH_SERVICE_PREFIX) else f"{SEARCH_SERVICE_PREFIX}-{name}"
        return self.registry.register(full_name, endpoint, weight=weight, capacity=capacity)
    ...
    def report_failure(self, service_name: str, error: Exception):
        ...
        if instance:
            ...
            logger.warning(
                "[PHOSPHOLIPID_SEARCH] Instância não encontrada: %s", service_name,
            )
    ...
    async def search_with_lb(
        ...
        if not self.has_search_services():
            logger.info("[PHOSPHOLIPID_SEARCH] Sem search services registrados → ddgs local")
            try:
        ...
        if instance is None:
            logger.info("[PHOSPHOLIPID_SEARCH] Nenhum search service saudável → ddgs local")
            return SearchTools.search_and_fetch_raw(query, max_results=max_results)
        ...
        try:
            ...
            logger.warning(
                "[PHOSPHOLIPID_SEARCH] %s falhou, tentando fallback...", instance.name,
            )
    ...
    async def search_and_learn_with_lb(
        ...
        if not results:
            logger.warning(
                "[PHOSPHOLIPID_SEARCH] Nenhum resultado para '%s'", termo_busca,
            )
            return False
