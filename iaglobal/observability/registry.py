...
class PhospholipidRegistry:
    ...
    def register(self, name: str, endpoint: str, weight: float = 1.0, capacity: int = 10) -> ServiceInstance:
        ...
        logger.info("[PHOSPHOLIPID] Serviço registrado: %s → %s (peso=%.2f, cap=%d)", name, endpoint, weight, capacity)
        self._audit("service_registered", name, {"endpoint": endpoint, "weight": weight, "capacity": capacity})
        return instance
    ...
    def heartbeat(self, name: str):
        ...
        if not instance.health and instance.consecutive_heartbeats_ok >= RECOVERY_THRESHOLD:
            instance.health = True
            instance.weight = 0.5
            instance.consecutive_failures = 0
            logger.info("[PHOSPHOLIPID] Serviço recuperado: %s (peso=%.2f)", name, instance.weight)
    ...
    def report_health(self, name: str, latency_ms: float = 0.0, error: bool = False):
        ...
        if instance is None:
            return
        instance.latency_p95_ms = latency_ms if latency_ms > 0 else instance.latency_p95_ms

        if error:
            instance.consecutive_failures += 1
            instance.error_rate = min(1.0, instance.error_rate + 0.1)
            instance.weight = max(0.1, instance.weight * 0.8)
            self._audit("failure_detected", name, {
                "consecutive_failures": instance.consecutive_failures,
                "new_weight": round(instance.weight, 4),
            })
            if instance.weight < 0.2:
                instance.health = False
                logger.warning("[PHOSPHOLIPID] Serviço unhealthy: %s (peso=%.4f)", name, instance.weight)
                self._audit("health_changed", name, {"health": False, "weight": instance.weight})
        else:
            instance.consecutive_failures = 0
    ...
    def check_timeouts(self):
        ...
        for instance in self._services.values():
            if instance.health and (now - instance.last_heartbeat) > HEARTBEAT_TIMEOUT:
                instance.health = False
                logger.warning("[PHOSPHOLIPID] Heartbeat timeout: %s (%.1fs sem beat)", instance.name, now - instance.last_heartbeat)
                self._audit("health_changed", instance.name, {"health": False, "reason": "heartbeat_timeout"})
