...
class PhospholipidBanditBridge:
    ...
    def report_success(
        self, service_name: str, latency_ms: float = 0.0
    ):
        ...
        if self.has_registered_services(provider):
            ...
            if instance:
                logger.info(
                    ...
                    model_name, instance.name, instance.weight, instance.health,
                )
                try:
                ...
                if instance2 and instance2.name != instance.name:
                    logger.info(
                        "[PHOSPHOLIPID_BRIDGE] Fallback para %s", instance2.name,
                    )
                    try:
                        result = await async_route_generate(
                            model=model_name,
    ...
    def auto_report(self, model_name: str, success: bool, latency_ms: float):
        ...
        for svc in list(self.registry._services.values()):
            if svc.name.startswith(provider):
                self.registry.report_health(svc.name, latency_ms=latency_ms, error=not success)
