...
from iaglobal.events import store
...
class DecisionReplaySystem:
    ...
    def batch_compare_what_if(
        execution_ids: List[str], alternative_model: str
    ) -> Dict[str, Any]:
        """
        ...
        avg_delta = (
            sum(r.get("score_delta", 0) for r in results.values()) / len(results)
            if results
            else 0
        )
    ...
    def explain(
        ...
        for row in rows:
            ...
            tag = "; ".join(
                ...
                if k
                not in ("step", "timestamp", "execution_id", "metadata", "candidates")
                and v is not None
            )
