...
_ENVIRONMENT_DETAILS_RE = re.compile(
...
async def _broadcast_execution(
    execution_id: str, status: str, message: str = "", error: str = ""
):
...
async def _run_pipeline_background(execution_id: str, task: str):
    ...
    try:
        await _broadcast_execution(
            execution_id, "running", "Inicializando orquestrador..."
        )
...
def AgentDashboard():
    ...
    return html.div(
        ...
        html.h1(
            {"style": {"color": DARK_THEME["accent"]}}, "🧠 IAGLOBAL Agent Dashboard"
        ),
        html.div(
...
async def api_create_task(request: Request):
    ...
    try:
        ...
        if not task or len(task.strip()) < 3:
            return JSONResponse(
                status_code=400, content={"success": False, "error": "Tarefa inválida"}
            )

        execution_id = f"ui_{uuid.uuid4().hex}"
        asyncio.create_task(_run_pipeline_background(execution_id, task))
        return JSONResponse(
            content={"success": True, "execution_id": execution_id, "status": "pending"}
        )
    except Exception:
...
async def api_get_task(execution_id: str):
    ...
    try:
        ...
        match = next(
            (item for item in executions if item.get("id") == execution_id), None
        )
        if not match:
            return JSONResponse(
...
async def api_metabolismo():
    ...
    try:
        ...
        return JSONResponse(content={"success": True, **metabolic})
    except Exception:
...
async def api_epigenetica(agent_id: str = None):
    ...
    try:
        ...
        return JSONResponse(content={"success": True, **markers})
    except Exception:
...
async def ws_progress(websocket: WebSocket, execution_id: str):
    ...
    try:
        await websocket.send_json(
            {"type": "status", "status": "connected", "execution_id": execution_id}
        )
        while True:
