...
from iaglobal.ui.git_workspace import GitWorkspace
...
class WorkspaceRunner:
    ...
    async def run_task(
        self, task_description: str, execution_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if not task_description or len(task_description.strip()) < 3:
            raise WorkspaceRunnerError(
                "task_description deve ter pelo menos 3 caracteres"
            )
        ...
        try:
            ...
            logger.info(
                "[RUNNER] Workspace criado para %s: %s", execution_id, workspace_path
            )
            ...
            try:
                result = await asyncio.wait_for(
                    graph.async_run(input_data, execution_id=execution_id), timeout=300
                )
            ...
            logger.info(
                "[RUNNER] ✅ Tarefa %s concluída em %.2fs", execution_id, duration
            )
