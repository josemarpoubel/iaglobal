...
def _get_embedder():
    ...
    if _embed_model is None:
        try:
            ...
            logger.debug("[TOOL_LIBRARY] fastembed nao disponivel — semantica desativada")
            _embed_model = False
...
class ToolLibrary:
    ...
    def _save(self):
        ...
        try:
            ...
            logger.debug("[TOOL_LIBRARY] Registry persistido: %s (%d tools)", path, len(serializable))
        except Exception as e:
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info("[TOOL_LIBRARY] Carregados %d tools do disco", len(serializable))
        except Exception as e:
            logger.debug("[TOOL_LIBRARY] Falha ao carregar: %s", e)

    def register(self, name: str, fn: Callable, tags: List[str], description: str = "") -> None:
        ...
        self._dirty = True
        self._save()
        logger.info("[TOOL_LIBRARY] Registrada: %s | tags=%s | emb=%s", name, tags, "sim" if embedding else "nao")
    ...
    def match(self, task: str, tags: Optional[List[str]] = None) -> Tuple[Optional[ToolEntry], float]:
        task_lower = task.lower()
    ...
    @staticmethod
    def _entry_tag_score(entry: ToolEntry, task_lower: str, prompt_tags: List[str]) -> float:
        score = 0.0
        for tag in prompt_tags:
            if tag in entry.tags:
                score += 0.3
        if score > 0:
            ratio = difflib.SequenceMatcher(None, task_lower, " ".join(entry.tags)).ratio()
            score += ratio * 0.4
        if entry.description:
            desc_score = difflib.SequenceMatcher(None, task_lower, entry.description.lower()).ratio()
