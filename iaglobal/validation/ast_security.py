...
class RealASTValidator:
    ...
    def _check_undefined_vars(self, tree: ast.AST) -> None:
        ...
        builtins = {
            ...
            "staticmethod", "classmethod", "enumerate", "zip", "map",
            "filter", "sorted", "reversed", "min", "max", "sum", "abs",
            "round", "any", "all", "hash", "id", "repr", "ord", "chr",
            "bytes", "bytearray", "memoryview", "iter", "next", "input",
            "format", "vars", "dir", "help", "__import__", "ValueError",
            "TypeError", "KeyError", "IndexError", "Exception", "BaseException",
        }
                    ...
                    for child in ast.walk(node):
                        if isinstance(child, ast.Call):
                            if isinstance(child.func, ast.Attribute):
                                if child.func.attr in ("append", "extend", "insert", "pop", "remove", "__setitem__", "__delitem__"):
                ...
                if isinstance(node.value, str):
                    ...
                    for pattern in self.forbidden_string_patterns:
                        if pattern in lowered:
                            violations.append(
                                f"Suspicious string pattern: {pattern}"
                            )
                            break
