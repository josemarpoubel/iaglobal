...
def detect_lang(code: str) -> Optional[str]:
    ...
    has_typescript = bool(
        re.search(r":\s*(string|number|boolean|void|any|never|unknown)\s*[=(,;]", code)
    )
...
def _get_parse_options(code: str) -> Tuple[str, Dict[str, Any]]:
    ...
    has_jsx = bool(re.search(r"<[A-Za-z][A-Za-z0-9]*[\s>]", code)) or bool(
        re.search(r'from\s+[\'"]react[\'"]', code)
    )
    ...
    def validate(self, code: str) -> JSValidationResult:
        ...
        if lang is None:
            return JSValidationResult(
                valid=False,
                code=None,
                errors=["Código não parece ser JavaScript/JSX"],
                lang="unknown",
            )
