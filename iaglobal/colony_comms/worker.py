...
class ColonyWorker:
    ...
    async def start(self):
        ...
        logger.info(
            "[WORKER %s] Iniciado com skills: %s", self.organism_id, self.skills
        )
    ...
    async def _on_message(self, message: AgentMessage):
        if message.message_type == "task_offer":
            ...
            if self.can_handle(task_type):
                ...
                logger.debug(
                    "[WORKER %s] Recusou task_offer %s (skills: %s, precisa: %s)",
                    self.organism_id,
    ...
    async def _execute_task(self, msg: AgentMessage) -> dict:
        ...
        logger.info(
            "[WORKER %s] Executando %s (%s)", self.organism_id, task_id, task_type
        )
    ...
    async def _handle_code_review(self, code_hint: str) -> dict:
        return {
            "success": True,
            "output": f"Revisão concluída para: {code_hint}",
            "issues": [],
        }
