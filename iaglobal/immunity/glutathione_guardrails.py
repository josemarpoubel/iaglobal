...
class GlutathioneGuardrails:
    ...
    def validate(cls, code: str, agent_name: str = "unknown") -> Dict[str, Any]:
        ...
        sam_cost = COST_VALIDATION_DEEP if threat_level == "critical" else COST_VALIDATION_BASIC
        sam_status = cls._check_sam_budget(agent_name, sam_cost, threat_level == "critical")
    ...
    @classmethod
    def _check_sam_budget(cls, agent_name: str, cost: int, critical: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"has_budget": False, "cost": cost, "reason": "SAMe insuficiente para validação imunológica"}
        except Exception as e:
            logger.warning("[GLUTATHIONE-SAME] Falha ao verificar budget SAMe: %s", e)
            return {"has_budget": True, "cost": 0, "error": str(e)}  # Allow if SAMe unavailable

    @classmethod
    def defend_and_correct(cls, code: str, agent_name: str = "unknown") -> Dict[str, Any]:
        ...
        sam_status = cls._check_sam_budget(agent_name, COST_AUTO_CORRECTION, critical=True)
        
        if not sam_status.get("has_budget"):
            logger.warning("[GLUTATHIONE-SAME] Ameaça detectada mas SAMe insuficiente para correção automática")
            return {
                **validation,
    ...
    def _apply_auto_correction(cls, code: str, issues: List[Dict]) -> str:
        ...
        for issue in issues:
            ...
            if "eval()" in msg:
                corrected = re.sub(r'\beval\s*\([^)]*\)', '# eval() BLOCKED - segurança', corrected)
            elif "exec(" in msg:
                corrected = re.sub(r'\bexec\s*\([^)]*\)', '# exec() BLOCKED - segurança', corrected)
            elif "subprocess" in msg:
                corrected = re.sub(r'\bsubprocess\.(call|Popen|run)\s*\([^)]*\)', '# subprocess BLOCKED - segurança', corrected)
            elif "os.system" in msg:
                corrected = re.sub(r'\bos\.system\s*\([^)]*\)', '# os.system BLOCKED - segurança', corrected)
            elif "shutil.rmtree" in msg:
                corrected = re.sub(r'\bshutil\.rmtree\s*\([^)]*\)', '# shutil.rmtree BLOCKED - segurança', corrected)
