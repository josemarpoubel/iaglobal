...
SUSPICIOUS_PATTERNS = [
    ...
    (
        r"(function|método|biblioteca|lib) (não encontrad[oa]|inexistente)",
        "reconhecimento_falha",
    ),
...
class HallucinationDetector:
    ...
    def check(cls, text: str) -> Dict[str, Any]:
        ...
        for pattern, label in SUSPICIOUS_PATTERNS:
            ...
            if matches:
                findings.append(
                    {"pattern": label, "matches": len(matches), "detail": matches[:3]}
                )
        ...
        if fake_found:
            findings.append(
                {
                    "pattern": "lib_inventada",
                    "matches": len(fake_found),
                    "detail": list(fake_found),
                }
            )
