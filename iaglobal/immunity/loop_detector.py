...
class LoopDetector:
    ...
    def __init__(
        ...
        reflexion_fn: Optional[Callable] = None,
    ):
    ...
    def check_and_repair(
        self, node_name: str, success: bool, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        if in_loop and self._reflexion_fn and context:
            logger.warning(
                f"[LOOP-REPAIR] Loop detectado no nó '{node_name}' ({count} tentativas) - acionando ReflexionEngine"
            )
            try:
                ...
                self._execution_count[node_name] = 0
                logger.info(
                    f"[LOOP-REPAIR] Reparação concluída: {repair_result.get('status', 'unknown')}"
                )
