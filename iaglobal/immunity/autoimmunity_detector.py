...
class AutoimmunityDetector:
    ...
    def __init__(self):
        ...
        self.trigger_history: Dict[str, List[Tuple[float, bool]]] = defaultdict(
            list
        )  # timestamp, was_false_positive
    ...
    def register_component(self, component_id: str, initial_sensitivity: float = 0.5):
        ...
        if component_id not in self.markers:
            self.markers[component_id] = AutoimmunityMarker(
                component_id=component_id, sensitivity_level=initial_sensitivity
            )
            ...
            self.autoimmune_events.append(
                {
                    ...
                    "old_sensitivity": marker.sensitivity_level
                    + self.sensitivity_reduction_step,
    ...
    def should_block_component(self, component_id: str) -> Tuple[bool, str]:
        ...
        if risk > 0.8 and marker.health_score < 0.3:
            return (
                True,
                f"Autoimunidade crítica (risco={risk:.2f}, saúde={marker.health_score:.2f})",
            )
    ...
    def get_system_autoimmunity_report(self) -> Dict:
        ...
        avg_health = (
            statistics.mean([m.health_score for m in self.markers.values()])
            if self.markers
            else 1.0
        )
        avg_risk = (
            statistics.mean(
                [m.calculate_autoimmunity_risk() for m in self.markers.values()]
            )
            if self.markers
            else 0.0
        )
        ...
        return {
            ...
            else ("WARNING" if warning_components else "HEALTHY"),
    ...
    def integrate_with_immune_orchestrator(self, orchestrator) -> None:
        ...
        for component_status in report.get("critical_components", []) + report.get(
            "warning_components", []
        ):
            ...
            if hasattr(orchestrator, "adjust_detector_sensitivity"):
                orchestrator.adjust_detector_sensitivity(
                    component_id, suppression_factor
                )
...
if __name__ == "__main__":
    ...
    print(
        f"   Status: {status['status']}, Saúde: {status['health_score']:.2f}, Risco: {status['autoimmunity_risk']:.2f}"
    )
    ...
    for i in range(15):
        detector.record_trigger(
            "apoptosis_engine", was_false_positive=(i < 12)
        )  # 80% FPs
