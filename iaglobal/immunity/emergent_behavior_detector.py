...
class EmergentBehaviorDetector:
    ...
    def check_dependencies(self, node_name: str, depends_on: List[str]) -> Dict[str, Any]:
        issues = []
        ...
        for dep in depends_on:
            ...
            if node_name in chain:
                issues.append({"type": "circular_dependency", "chain": f"{node_name} → {dep} → ... → {node_name}"})
                self._circular_count += 1
    ...
    def check_skill_name(self, skill_name: str) -> Dict[str, Any]:
        suspicious = ["malware", "trojan", "backdoor", "hack", "exploit", "ransomware", "keylogger"]
