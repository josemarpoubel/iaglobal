...
class MetabolicMetrics:
    ...
    async def _get_active_agents(self) -> int:
        ...
        Valor determinístico baseado em timestamp e hostname.
        """
        unique_seed = f"agents-{int(time.time() // 3600)}-{os.uname().nodename}".encode()
    ...
    def _generate_suggestions(self, invariants: Dict, system_metrics: Dict) -> List[str]:
        ...
        for inv_name, data in invariants.items():
            if data["status"] == "AVISO" and "alert" in data:
                ...
                suggestions.append(f"Ação recomendada: Executar `iaglobal autocorrect` para corrigir {inv_name}")
        
        # Sugestões para métricas do sistema
        if system_metrics["cpu_usage"]["value"] > "80%":
            suggestions.append("Pressão de CPU alta. Considerar otimização ou escalamento.")
        
        if system_metrics["memory_pressure"]["value"] > "85%":
            suggestions.append("Pressão de memória alta. Verificar leaks ou aumentar quota.")

        # Sugestões baseadas em padrões
        if invariants["vault"]["value"] > "85%" and invariants["toxins"]["value"] > "20%":
            suggestions.append("Padrão detectado: Vault e toxinas altos. Aumentar limpeza DeltaSleep.")
        
        if invariants["ivm"]["value"] < "0.6":
            suggestions.append("IVM baixo. Priorizar agentes de alta eficiência (ver `iaglobal status`).")
