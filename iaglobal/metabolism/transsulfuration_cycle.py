...
# iaglobal/metabolism/transsulfuration_cycle.py
    ...
    async def run(self, candidate: CandidateSkill) -> bool:
        ...
        try:
            errors = await _run_maybe_async(
                ...
                candidate.source_gap or candidate.skill.name,
                top_k=5,
            )
            max_freq = max((e.get("count", 1) for e in errors), default=0)
        except Exception as e:
            logger.warning(
                "[TRANSSULFURATION] Erro ao consultar memória de falhas: %s", e
            )
            max_freq = 0

        if max_freq >= self.frequency_threshold:
            success = await _run_maybe_async(
                homocysteine_pool.route_to_guardrail, candidate
            )
            if success:
                logger.info(
