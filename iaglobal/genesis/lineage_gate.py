...
def revoke_node(node_name: str, reason: str = "", duration_hours: Optional[int] = None) -> bool:
    """
    ...
    """
    if _REVOCATION_FILE is None:
        logger.warning("[LINEAGE_GATE] CRL não configurada — defina set_revocation_file() primeiro")
        return False
    ...
    try:
        ...
        _REVOCATION_FILE.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")

        logger.warning("[LINEAGE_GATE] 🔒 Nó revogado: %s | motivo: %s", node_name, reason)
        return True
...
def unrevoke_node(node_name: str, reason: str = "") -> bool:
    ...
    try:
        ...
        _REVOCATION_FILE.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
        logger.info("[LINEAGE_GATE] 🔓 Nó readmitido: %s | motivo: %s", node_name, reason)
        return True
...
def verify_lineage_token(
    ...
    """
    if _OPEN_MODE:
        logger.debug("[LINEAGE_GATE] Open mode — %s autorizado sem verificação", node_name)
        return True
