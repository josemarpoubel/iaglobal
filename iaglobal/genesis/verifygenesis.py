...
class VerifyGenesis:
    ...
    def __init__(self):
        ...
        self.blueprint_path = (
            PACKAGE_DIR / "genesis" / "data" / "webhidden_genesis_blueprint.cbor"
        )
        self.evolutive_path = (
            PACKAGE_DIR / "genesis" / "data" / "webhidden_genesis_evolutive.cbor"
        )
    ...
    def verify_frozen_authority(self) -> bool:
        ...
        if not self.blueprint_path.exists() or not self.evolutive_path.exists():
            ...
            logger.critical(
                "🚨 [DNA AUSENTE] Gênese não localizada. O nó não pode nascer."
            )
            return False
        ...
        try:
            ...
            if not expected_hash:
                ...
                logger.error(
                    "🚨 [BLUEPRINT INVÁLIDO] Chave 'hash' ausente no certificado."
                )
                return False
            ...
            if manifesto and not isinstance(manifesto, dict):
                ...
                logger.error(
                    "🚨 [BLUEPRINT INVÁLIDO] Estrutura de manifesto corrompida."
                )
                return False
            ...
            logger.info(
                "✅ [SOBERANIA CONFIRMADA] DNA validado. Nó autorizado a ingressar na malha."
            )
            return True
