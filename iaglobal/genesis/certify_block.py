...
def verify_genesis_integrity():
    ...
    base_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "genesis", "data")
    ...
    try:
        ...
        if not os.path.exists(blueprint_path):
            logging.error("🚨 CRITICAL: Blueprint original não encontrado! O nó está órfão.")
            return False
        ...
        if not blueprint_hash:
            logging.warning("⚠️ Blueprint não contém um campo 'hash'. Tentando calcular hash do conteúdo do blueprint...")
        ...
        if not verify_genesis_hash(evolutive_path, blueprint_hash):
            logging.error("❌ VIOLAÇÃO DE REALIDADE: O arquivo físico não corresponde ao hash esperado!")
            return False

        # 4. O Veredito
        if calculated_hash == blueprint_hash:
            logging.info("✅ CONSENSO: O Genesis evolutivo é legítimo. Realidade confirmada.")
            return True
        else:
            logging.error("❌ VIOLAÇÃO DE REALIDADE: O hash lógico não bate com o Blueprint!")
