...
class FusionEngine:
    ...
    async def verificar_ressonancia(self, dna_a: GenomaMetabolico, dna_b: GenomaMetabolico) -> Tuple[bool, float]:
        """
        ...
        if dna_a.linhagem != dna_b.linhagem:
            logger.debug(f"[FusionEngine] 🚫 Divergência de linhagem: {dna_a.linhagem} != {dna_b.linhagem}")
            return False, 0.0
    ...
    async def fundir_agentes(self, dna_a: GenomaMetabolico, dna_b: GenomaMetabolico) -> GenomaMetabolico:
        """
        Sintetiza um Agente Híbrido a partir de dois genomas ressonantes.
        """
        logger.info(f"[FusionEngine] 🧬 INICIANDO FUSÃO: {dna_a.node_id[:8]} + {dna_b.node_id[:8]}")
        ...
        logger.info(f"[FusionEngine] ✅ FUSÃO CONCLUÍDA: Híbrido {novo_id[:8]} criado (G{nova_geracao})")
        return hibrido
