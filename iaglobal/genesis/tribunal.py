...
class GenesisTribunal:
    ...
    async def executar(self) -> list[Veredito]:
        ...
        for v in resultados[1:]:
            s = "✅" if v.dna_valido else "❌"
            print(f"  {s} {v.arquivo:<50} {v.phonetic_name if v.dna_valido else v.razao}")
            if not v.dna_valido:
                inv_mostrados += 1

        print(f"{linha}")
        print(f"  Genesis File: {'✅ CONFORME' if resultados[0].dna_valido else '❌ VIOLADO'}")
        print(f"  Agentes: {total - 1} total | {validos - (1 if resultados[0].dna_valido else 0)} validos | {inv_mostrados} invalidados")
        print(f"{linha}")
