...
class PipelineEngine:
        ...
        try:
            ...
            if not state.graph.nodes:
                ...
                try:
                    ...
                    logger.warning("[PIPELINE] Antes build - registry size=%d", len(skill_registry._skills))
                    
                    state.graph = build_graph_from_skills(self.orchestrator)
                    
                    logger.warning("[PIPELINE] Depois build - graph nodes=%d", len(state.graph.nodes))
                    roots = [n for n, nd in state.graph.nodes.items() if not nd.depends_on]
                    logger.warning("[PIPELINE] Nós raiz (sem deps): %s", roots)
                    logger.warning("[PIPELINE] Nós do grafo: %s", list(state.graph.nodes.keys()))
                except Exception as exc:
                    logger.exception("❌ [PIPELINE] Falha ao construir grafo automático: %s", exc)
        ...
        if cached:
            ...
            if score >= 0.6 and len(codigo) > 100 and ("def " in codigo or "class " in codigo):
                ...
                if metadata.get("script_path") or metadata.get("path"):
                    logger.info(
                        "⚡ já aprendi isso antes! (score=%.2f, %d chars)", score, len(codigo)
                    )
                    return PipelineResult(
                        success=True,
                        response=codigo,
            ...
            logger.info(
                "📖 aprendizado anterior não se aplica (score=%.2f, %d chars)", score, len(codigo)
            )
    ...
    async def _async_generation_stage(
        ...
        logger.warning("[PIPELINE] ⚙️ INICIANDO GERAÇÃO | prompt=%s | parallel=%s", state.prompt[:50], parallel)
            ...
            if detected:
                ...
                if result["file_count"] > 0:
                    ...
                    logger.info(
                        ...
                        result["file_count"], result["total_chars"],
                    )
        except Exception as e:
        ...
        if not chosen_model:
            ...
            logger.warning("🎯 chosen_model vazio — fallback hardcoded: %s", chosen_model)
            ...
            if isinstance(node_result, dict):
                ...
                if output and len(str(output)) > 10:
                    ...
                    logger.info(
                        ...
                        node_name, len(state.generated_code),
                    )
                    return
    ...
    def _extract_fenced_code(self, code: str) -> str:
        ...
        if m:
            ...
            if extracted:
                logger.info(
                    "[EXTRACT] Código extraído de fence ```%s (%d chars)", lang, len(extracted)
                )
