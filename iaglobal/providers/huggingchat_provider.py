...
async def _ensure_browser():
    ...
    async with _BROWSER_LOCK:
        ...
        logger.info("[HUGGINGCHAT] Inicializando navegador")
        ...
        logger.info("[HUGGINGCHAT] Navegador inicializado")
...
async def async_generate(
    ...
    try:
        ...
        input_el = await page.query_selector('textarea[placeholder="Ask anything"]')

        if not input_el:
            input_el = await page.query_selector("textarea")

        if not input_el:
            logger.warning("[HUGGINGCHAT] Campo de entrada não encontrado")
        ...
        for _ in range(max_cycles):
            ...
            texts = [t.strip() for t in texts if t and len(t.strip()) > 10]
            ...
            if candidate.startswith(prompt.strip()):
                candidate = candidate[len(prompt.strip()) :].strip()
        ...
        logger.warning("[HUGGINGCHAT] Timeout aguardando resposta")

        return ""

    except asyncio.CancelledError:
        logger.debug("[HUGGINGCHAT] Task cancelada")
