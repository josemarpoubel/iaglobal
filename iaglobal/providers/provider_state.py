...
class ProviderStats:
    ...
    def avg_latency(self) -> float:
        ...
        return self.success_latency_total / self.success_latency_count
...
class ProviderState:
    ...
    def __init__(
        ...
        self.providers: Dict[str, ProviderStats] = {
            provider: ProviderStats() for provider in self.DEFAULT_PROVIDERS
        }
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            for provider, values in data.items():
                ...
                self.providers[provider] = ProviderStats(**values)

            logger.info("[STATE] restored provider statistics")
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            data = {
                provider: asdict(stats) for provider, stats in self.providers.items()
            }
    ...
    def _calculate_cooldown(
        ...
        if error_code in (429, 503, 504):
            return min(300, 5 * (2 ** min(failures - 1, 6)))
...
provider_state = ProviderState(persist_file=str(DATA_ROOT / "provider_state.json"))
