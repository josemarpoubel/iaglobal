...
class ProviderMetrics:
    ...
    def record(
        ...
        logger.info(
            "[METRICS] record provider=%s model=%s success=%s latency_ms=%.0f tokens=%d cost=%.6f task=%s",
            provider,
            model,
            success,
            latency_ms,
...
def format_metrics_report(
    stats: Dict[str, Dict[str, Any]], title: str = "Provider Metrics"
) -> str:
    ...
    lines.append(f"  {'─' * 72}")
    lines.append(
        f"  {'Provider/Model':<45} {'Calls':>6} {'Succ%':>7} {'Avg ms':>8} {'Tokens':>8} {'Cost $':>10}"
    )
    lines.append(f"  {'─' * 72}")
