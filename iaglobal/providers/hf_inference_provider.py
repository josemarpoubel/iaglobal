...
async def async_generate(
    ...
    if not client:
        logger.debug("[HFInference] API key não configurada")

        return ""

    model = model.replace("hf_inference/", "", 1).strip()

    if not model:
        model = "openai/gpt-oss-20b:groq"

    max_tokens = _estimate_max_tokens(prompt)
    ...
    try:
        ...
        result = _extract_result(completion)
