...
def block_provider(provider: str, status: int):
    ...
    if status in (401, 402):
        ...
        logger.warning(
            "[CIRCUIT-BREAKER] Provider %s bloqueado por status %d por %ds",
            provider,
            status,
            block_time,
        )
        ...
        _BLOCKED_PROVIDERS[provider] = time.time() + block_time
        logger.warning(
            "[CIRCUIT-BREAKER] Provider %s bloqueado por rate-limit (429) por %ds",
            provider,
            block_time,
        )
...
async def async_post(
    ...
    if domain in _BANNED_DOMAINS:
        logger.warning(
            "[CIRCUIT-BREAKER] Abortando request para domínio banido: %s", domain
        )
    ...
    if provider and is_provider_blocked(provider):
        logger.warning(
            "[ASYNC-HTTP] Provider %s está bloqueado (circuit breaker)", provider
        )
        return ""
