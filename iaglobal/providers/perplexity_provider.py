...
def generate(prompt: str, model: str = "perplexity/default", timeout: int = 60, token_collector: Optional[callable] = None) -> str:
...
async def async_generate(
    ...
    try:
        ...
        if not input_el:
            ...
            logger.warning(
                "[PERPLEXITY] Campo de entrada não encontrado"
            )
        ...
        for _ in range(max_cycles):
            ...
            if (
                ...
                or
                "exclusive access to web app generation" in body_lower
            ):

                logger.warning(
                    "[PERPLEXITY] Login obrigatório"
                )
            ...
            if (
                "sources" not in body_lower
                and
                "follow-up" not in body_lower
            ):
                continue
        ...
        logger.warning(
            "[PERPLEXITY] Timeout aguardando resposta"
        )
