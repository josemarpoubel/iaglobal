...
def get_api_key() -> str:
    ...
    if not key:
        logger.warning(
            "⚠️ [HF_Router] Chave de API ausente (HUGGINGFACE_API_KEY ou HF_TOKEN)."
        )
    return key or ""
        ...
        with _client_lock:
            if not _async_client:
                _async_client = AsyncOpenAI(
                    api_key=get_api_key(), base_url="https://router.huggingface.co/v1"
                )
...
def generate(
    ...
    **kwargs,
    ...
    return run_async_safe(
        async_generate, prompt, model, timeout, token_collector, **kwargs
    )
...
async def async_generate(
    ...
    **kwargs,  # Absorve parâmetros extras do Router sem quebrar
    ...
    try:
        ...
        logger.debug(
            f"[HF_Router ASYNC] {clean_model} respondeu em {latency:.2f}s ({len(result)} chars)"
        )
        return result.strip()
