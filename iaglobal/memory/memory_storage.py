...
class MemoryStorage:
    ...
    def store(
        self, task: str, codigo: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def retrieve(self, task: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            with self._get_connection() as conn:
                cursor = conn.execute(
                    "SELECT data FROM success_registry WHERE task_hash = ?",
                    (task_hash,),
                )
                row = cursor.fetchone()
    ...
    def delete(self, task: str) -> None:
        ...
        with self._lock:
            try:
                with self._get_connection() as conn:
                    conn.execute(
                        "DELETE FROM success_registry WHERE task_hash = ?", (task_hash,)
                    )
                    conn.commit()
            except Exception as e:
