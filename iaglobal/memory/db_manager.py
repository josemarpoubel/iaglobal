...
class DatabaseManager:
    ...
    def _init_tables(self):
        ...
        try:
            with conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_vector_metadata ON vector_store(metadata)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_execution_node ON execution_states (execution_id, status)"
                )
                ...
                conn.execute("""
                    ...
                    """)
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_synthesis_confidence ON synthesis_cache(confidence DESC)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_synthesis_agent ON synthesis_cache(agent_id)"
                )
                ...
                conn.execute("""
                    ...
                    """)
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_decision_execution ON decision_events(execution_id)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_decision_step ON decision_events(step)"
                )
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_decision_created ON decision_events(created_at)"
                )
    ...
    def count_insights(
        ...
        try:
            ...
            logger.error(f"❌ Erro ao contar insights: {e}")
            return 0
    ...
    def insert_decision_event(
        self, execution_id: str, step: str, timestamp: str, event_data: str
    ) -> None:
        conn = self._get_conn()
        try:
            with conn:
            ...
            logger.error(
                f"❌ Erro ao inserir decision_event {execution_id}/{step}: {e}"
            )
        finally:
            conn.close()
    ...
    def count_decision_events(
        ...
        try:
            ...
            # Safe: where_clause é construído com colunas fixas, valores usam parameter binding
            cursor = conn.execute(
                f"SELECT COUNT(*) FROM decision_events {where_clause}", params
            )
            row = cursor.fetchone()
            return row[0] if row else 0
    ...
    def insert_synthesis(
        ...
        """
        INSERT OR IGNORE na tabela synthesis_cache.

        Retorna True se inseriu, False se já existia (duplicado).
        """
        conn = self._get_conn()
        try:
            with conn:
            ...
            logger.error(f"❌ Erro ao insert_synthesis: {e}")
            return False
        finally:
            conn.close()

    def get_synthesis(self, cache_key: str) -> Optional[dict]:
        """Retorna síntese por chave, atualizando accessed_at."""
        conn = self._get_conn()
        try:
            row = conn.execute(
                "SELECT cache_key, summary, confidence, score, agent_id, created_at "
                "FROM synthesis_cache WHERE cache_key = ?",
                (cache_key,),
            ).fetchone()
            if row:
                conn.execute(
                    "UPDATE synthesis_cache SET accessed_at = ? WHERE cache_key = ?",
                    (_now_iso(), cache_key),
                )
                conn.commit()
                return {
            ...
            logger.error(f"❌ Erro ao get_synthesis: {e}")
            return None
        finally:
            conn.close()

    def get_useful_syntheses(
        self,
        agent_id: Optional[str] = None,
        min_confidence: float = 0.7,
        limit: int = 50,
    ) -> list[dict]:
        """Retorna sínteses acima do limiar de confiança."""
        conn = self._get_conn()
        try:
            ...
            cursor = conn.execute(
                f"SELECT cache_key, summary, confidence, score, agent_id, created_at "
                f"FROM synthesis_cache {where} "
            ...
            logger.error(f"❌ Erro ao get_useful_syntheses: {e}")
            return []
        finally:
            conn.close()

    def prune_old_syntheses(self, max_age_hours: int = 48) -> int:
        """Remove sínteses mais antigas que max_age_hours. Retorna quantidade removida."""
        conn = self._get_conn()
        try:
            with conn:
                cursor = conn.execute(
                    "DELETE FROM synthesis_cache WHERE datetime(created_at) < "
                    "datetime('now', '-' || ? || ' hours')",
                    (max_age_hours,),
                )
            return cursor.rowcount
        except sqlite3.Error as e:
            logger.error(f"❌ Erro ao prune_old_syntheses: {e}")
            return 0
        finally:
            conn.close()
