...
class ConsolidationEngine:
    ...
    def consolidate_web_knowledge(self, web_results: List[Dict], local_memories: List[Dict]) -> List[Dict]:
        """Consolida conhecimento web + local em insights."""
        all_items = self._normalize(web_results, "web") + self._normalize(local_memories, "local")
    ...
    def _summarize(self, cluster: List[Dict]) -> Optional[Dict]:
        ...
        titles = [item.get("title", "") or item.get("type", "") for item in cluster if item.get("title") or item.get("type")]
        ...
        key_topics = list(set(re.findall(r"\b[A-Z][a-z]+(?:\s[A-Z][a-z]+)*\b", combined)))
