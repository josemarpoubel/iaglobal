...
class AntiRedundanciaGlobal:
    ...
    def is_duplicate(
        self, content: str, existing_items: List[Dict]
    ...
    def merge_similar(
        self, items: List[Dict], merge_threshold: float = 0.7
    ) -> List[Dict]:
...
# =========================================================================
# CONSTANTES DE AVALIAÇÃO DE QUALIDADE (fontes e thresholds)
# =========================================================================

_SOURCE_AUTHORITY_MAP: dict[str, float] = {
    "wikipedia": 0.9,
    "duckduckgo": 0.6,
    "web": 0.5,
    "rss": 0.5,
...
# Regex pré-compilados — executados uma única vez no módulo
...
_STRUCTURED_DATA_PATTERNS: list[re.Pattern] = [
    re.compile(r"\d{4}"),
    re.compile(r"\b(segundo|according|fonte|source)\b", re.IGNORECASE),
]

# Limites avaliáveis (evita magic numbers)
_MIN_CONTENT_LENGTH: int = 20
_MIN_WORD_COUNT: int = 5
_MIN_CONFIDENCE_TO_KEEP: float = 0.5
_MIN_JACCARD_OVERLAP: float = 0.2
_MAX_JACCARD_OVERLAP: float = 0.7
_MIN_SNIPPET_LENGTH_FOR_CONTRADICTION: int = 30


class FakeNoiseDetector:
    """
    Avalia qualidade, ruído e contradições em conteúdo de busca/síntese.

    Usado como **gatekeeper de persistência**: decide o que é worth keeping
    no banco de dados vs. o que deve ser descartado para não poluir a
    memória do organismo.
    """

    @staticmethod
    def _normalize_source(source: str | None) -> str:
        """Normaliza nome de fonte para chave canônica de _SOURCE_AUTHORITY_MAP."""
        raw = (source or "web").strip().lower()
        return _SOURCE_ALIASES.get(raw, raw)

    @classmethod
    def score_confidence(cls, item: Dict) -> float:
        """
        Score de confiança 0-1.

        Componentes:
            - Autoridade da fonte normalizada: 0.0–0.9 × 0.3
            - Tamanho do conteúdo:              0.05–0.3
            - Ausência de clickbait:            0.2
            - Dados estruturados:               0.0–0.1
              (ano + referência textual)
        """
        source = cls._normalize_source(item.get("source"))
        authority = _SOURCE_AUTHORITY_MAP.get(source, 0.4)
        ...
        # Autoridade da fonte (peso 30%)
        scores.append(authority * 0.3)

        # Tamanho do conteúdo (peso até 30%)
        if length > 500:
            scores.append(0.3)
        elif length > 100:
            scores.append(0.2)
        ...
        # Ausência de clickbait (peso 20%)
        if not cls._is_clickbait(content):
            scores.append(0.2)

        # Dados estruturados (peso até 10%)
        if any(p.search(content) for p in _STRUCTURED_DATA_PATTERNS):
            scores.append(0.1)

        return min(1.0, sum(scores))

    @classmethod
    def is_noise(cls, item: Dict) -> bool:
        """Retorna True se o item é ruído — não vale a pena persistir."""
        content = (item.get("content") or item.get("text") or "").strip()
    ...
    @classmethod
    def should_keep(
        cls, item: Dict, min_confidence: float = _MIN_CONFIDENCE_TO_KEEP
    ) -> bool:
        """
        Decisão de persistência: retorna True se o item tem qualidade suficiente.

        Combina is_noise (filtro duro) + score_confidence (filtro mole).
        Usado pelo sweeper para evitar estourar a RAM com lixo.
        """
        if cls.is_noise(item):
            return False
        return cls.score_confidence(item) >= min_confidence
    ...
    def detect_contradiction(
        ...
        if not new_text or len(new_text) < _MIN_SNIPPET_LENGTH_FOR_CONTRADICTION:
            return None
        ...
        for existing in existing_items:
            existing_text = (
                existing.get("content") or existing.get("text") or ""
            ).lower()
            if (
                not existing_text
                or len(existing_text) < _MIN_SNIPPET_LENGTH_FOR_CONTRADICTION
            ):
                continue
            ...
            if _MIN_JACCARD_OVERLAP < jaccard < _MAX_JACCARD_OVERLAP:
                negation_new = bool(re.search(r"\b(não|not|never|nunca)\b", new_text))
                negation_existing = bool(
                    re.search(r"\b(não|not|never|nunca)\b", existing_text)
                )
    ...
    def _is_clickbait(text: str) -> bool:
        ...
        return any(p.search(text_lower) for p in _CLICKBAIT_PATTERNS)
...
class KnowledgeGraph:
    ...
    def get_related(self, concept: str, max_results: int = 10) -> List[Dict]:
        ...
        try:
            rows = conn.execute(
                ...
                LIMIT ?
            """,
                (concept, concept, concept, max_results),
            ).fetchall()
            return [
    ...
    def get_top_concepts(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            return [
                {"name": row[0], "frequency": row[1], "source": row[2]} for row in rows
            ]
        finally:
            conn.close()
...
class AtualizacaoIncremental:
    ...
    def incorporate(
        ...
        new_content: str,
        source: str = "web",
        existing_knowledge: Optional[List[Dict]] = None,
    ) -> Dict:
        ...
        contradiction = noise.detect_contradiction(
            {"content": new_content, "source": source}, existing_knowledge or []
        )
        if contradiction:
    ...
    def get_knowledge_context(self, query: str) -> str:
        ...
        if kg_results:
            ...
            for r in kg_results:
                ...
                rel_str = (
                    ", ".join(rel["concept"] for rel in related)
                    if related
                    else "(sem relações)"
                )
