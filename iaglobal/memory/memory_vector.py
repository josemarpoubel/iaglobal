...
# 2. Garantia de estrutura de pastas (usando o objeto _paths importado)
os.makedirs(_paths.DATA_DIR, exist_ok=True)

# 3. Inicialização do Modelo (Lazy) — SentenceTransformer carregado sob demanda
...
def store(text: str, mtype: str = "fact") -> None:
    ...
    vec = _get_model().encode(text).astype(np.float32).tobytes()
...
def search(query: str, top_k: int = 5) -> List[Tuple[float, Dict[str, Any]]]:
    ...
    qvec = _get_model().encode(query).astype(np.float32)
