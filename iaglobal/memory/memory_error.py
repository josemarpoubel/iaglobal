...
def _load_db() -> Dict[str, Any]:
    ...
    try:
        with open(_get_error_file(), "r", encoding="utf-8") as f:
            data = json.load(f)
...
def _save_db(data: Dict[str, Any]) -> None:
    ...
    _get_error_file().parent.mkdir(parents=True, exist_ok=True)

    data["updated_at"] = datetime.now(timezone.utc).isoformat()

    with open(_get_error_file(), "w", encoding="utf-8") as f:
...
def store_error(
    ...
    with _lock:
        ...
        db["learning_errors"] = db["learning_errors"][-1000:]
...
def query_relevant_errors(current_prompt: str, limit: int = 3) -> List[Dict[str, Any]]:
    """
    ...
    ignorar = {"como", "fazer", "criar", "usando", "python", "script", "erro", "falha"}
    ...
    for entry in db:
        contexto = (
            f"{entry.get('prompt', '')} {entry.get('critica_sandbox', '')}"
        ).lower()

        score = sum(
            1.5 if word in entry.get("critica_sandbox", "").lower() else 1.0
    ...
    scored_entries.sort(key=lambda x: (x[0], x[1].get("time", "")), reverse=True)

    return [item[1] for item in scored_entries[:limit]]


def format_errors_for_prompt(relevant_errors: List[Dict[str, Any]]) -> str:
    ...
    for idx, err in enumerate(relevant_errors, start=1):
        buffer.append(
...
def record_error(
    ...
    with _lock:
        ...
        db["runtime_errors"] = db["runtime_errors"][-500:]
