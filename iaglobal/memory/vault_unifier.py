...
class VaultUnifier:
    ...
    def __init__(self, json_dir: Optional[Path] = None, vault_path: Optional[Path] = None):
    ...
    def _read_json(self, marker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("[VAULT-UNIFIER] JSON corrompido %s (tratado como vazio): %s", path, e)
            return None
    ...
    async def _write_obsidian(self, marker: str, conteudo_md: str) -> None:
        try:
            ...
            logger.debug("[VAULT-UNIFIER] write-through Obsidian falhou (não-bloqueante): %s", e)
    ...
    def migrate_obsidian_to_json(self, marker: str) -> bool:
        ...
        logger.info("[VAULT-UNIFIER] Migrated %d vaccines for %s", len(patterns), marker)
        return True
