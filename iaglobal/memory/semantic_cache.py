...
class SemanticCache:
    ...
    def get(self, query: str) -> Optional[str]:
        ...
        if best_key and best_score >= self.threshold:
            ...
            if entry:
                if self._is_expired(entry):
                    ...
                    logger.info(
                        f"[SEM-CACHE] RAM HIT: score={best_score:.3f} "
        ...
        for qtext, resp, emb_blob, created, acc, cid in rows:
            ...
            if len(vn) != len(qvec):
                logger.debug(
                    f"[SEM-CACHE] Shape mismatch: expected {len(vn)}, got {len(qvec)}"
                )
                continue
        ...
        if best_l2 and best_l2_score >= self.threshold:
            ...
            if self._is_expired({"created_at": created}):
                logger.debug(
                    f"[SEM-CACHE] L2 entry expired: {qtext[:40]}... (age > TTL)"
                )
            ...
            self._ram_embeddings[key] = self._normalize(
                list(struct.unpack(f"{len(emb_blob) // 4}f", emb_blob))
            )
            self._update_access_sql(cid)
            elapsed = time.time() - start
            logger.info(
                f"[SEM-CACHE] L2 HIT: score={best_l2_score:.3f} "
        ...
        elapsed = time.time() - start
        logger.debug(
            f"[SEM-CACHE] MISS: query={query[:40]}... "
    ...
    def set(self, query: str, response: str, score: float = 0.0):
        ...
        elapsed = time.time() - start
        logger.info(
            f"[SEM-CACHE] STORED: query={query[:40]}... "
    ...
    def clear(self):
        ...
        try:
            ...
            conn.close()
        logger.info(
            f"[SEM-CACHE] CLEAR: {ram_count} RAM + {deleted} SQLite entries removidos"
        )
    ...
    def prune_old_embeddings(self, max_age_days: int = 30) -> Dict[str, Any]:
        ...
        for row_id, qtext, last_access, created_at in rows:
            try:
                ts = (
                    datetime.fromisoformat(last_access).timestamp()
                    if last_access
                    else 0
                )
            except Exception:
        ...
        if result["archived"] > 0:
            archived_ids = [
                e["id"] for e in result.get("archived_entries", []) if e.get("id")
            ]
            if archived_ids:
                conn = sqlite3.connect(self.db_path)
                try:
                    ...
                    conn.close()
                logger.info(
                    "[SEM-CACHE] Pruned %d old embeddings (max_age=%d days)",
                    len(archived_ids),
                    max_age_days,
                )
        return result
