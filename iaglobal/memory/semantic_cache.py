...
class SemanticCache:
    ...
    def __init__(
        ...
        self._ram_embeddings: Dict[str, np.ndarray] = {}
        self._init_table()
    ...
    def get(self, query: str) -> Optional[str]:
        ...
        for qtext, resp, emb_blob, created, acc, cid in rows:
            ...
            if v.shape != qvec.shape:
                logger.debug(f"[SEM-CACHE] Shape mismatch: expected {v.shape}, got {qvec.shape}")
                continue
        ...
        if best_l2 and best_l2_score >= self.threshold:
            ...
            self._ram_embeddings[key] = np.frombuffer(emb_blob, dtype=np.float32)
    ...
    def set(self, query: str, response: str, score: float = 0.0):
        ...
        try:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (qhash, query, response, qvec.tobytes(), score, now, now))
            conn.commit()
        finally:
            conn.close()
        elapsed = time.time() - start
    ...
    def _embed(self, text: str) -> np.ndarray:
        ...
        return _get_model().encode(text).astype(np.float32)
