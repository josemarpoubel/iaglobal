...
class Dashboard:
    ...
    def _show_node_health(lines: list, graph: ExecutionGraph) -> None:
        ...
        sorted_nodes = sorted(
            graph.nodes.values(), key=lambda n: n.executions, reverse=True
        )
        ...
        for node in sorted_nodes:
            ...
            lines.append(
                f"  {name:<35} {ntype:<18} {execs:<6} {succ:<6} {fail:<6} {lat:<9} {fit:<8}"
            )
            if node.last_error:
    ...
    @staticmethod
    def _show_evolution_status(
        lines: list, runtime: EvolutionRuntime, engine, graph: ExecutionGraph
    ) -> None:
        ...
        if graph:
            ...
            evo_nodes = [
                ...
                if n.name.startswith("evo_") and n.executions > 0
            ]
            if evo_nodes:
                ...
                for n in top:
                    lines.append(
                        f"    {n.name:<40} fitness={n.fitness():.4f} sr={n.success_rate:.2f} lat={n.avg_latency:.4f}s"
                    )
        lines.append("")
    ...
    def _show_audit_status(lines: list, orch=None) -> None:
        ...
        try:
            ...
            if credit:
                ...
                if credit.stats:
                    best_node = max(
                        credit.stats.items(),
                        key=lambda kv: (
                            kv[1]["success"] / max(1, kv[1]["success"] + kv[1]["fail"])
                        ),
                    )
                    lines.append(
                        f"  Best combo     : {best_node[0]} ({best_node[1]['success']}/{best_node[1]['success'] + best_node[1]['fail']} ok)"
                    )
            ...
            if evo_graph and hasattr(evo_graph, "nodes"):
                evo_nodes = [
                    n
                    for n in evo_graph.nodes.values()
    ...
    def _show_security_status(lines: list) -> None:
        ...
        try:
            ...
            lines.append(
                f"  Checks      : {stats['modules_checked']} modulos, {stats['paths_checked']} paths"
            )
        except Exception:
            lines.append("  (erro ao consultar)")
        lines.append("")
    ...
    def _show_immune_status(lines: list) -> None:
        ...
        try:
            ...
            lines.append(
                f"  Barreira Events     : cache_poison={events.get('cache_poison', 0)}, stale={events.get('stale_cache', 0)}"
            )

            # Quarentena
            lines.append(f"  Quarantine Skills   : {quarantine.get('skills', 0)}")
            lines.append(
                f"  Active Detectors    : {quarantine.get('active_detectors', 0)}"
            )
        except Exception as e:
