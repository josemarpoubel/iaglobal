...
async def _run_cli_impl():
    ...
    parser.add_argument("command", type=str, nargs="?", help="Comando (run, status, replay, dashboard-sleep, etc)")
    ...
    parser.add_argument("--refresh", type=int, default=10, help="Intervalo de atualizacao em segundos (para dashboard-sleep)")
    parser.add_argument("--continuous", action="store_true", help="Modo continuo para auditoria MCP")
    ...
    if args.command == "dashboard-sleep":
        ...
        print(f"{ANSI.BOLD}{ANSI.CYAN}🌡️ DASHBOARD METABÓLICO (T+{health_data.uptime_min}min){ANSI.RESET}")
        print("=" * 80)
        print(f"{ANSI.BOLD}| INVARIANTE{' ' * 14} | STATUS       | VALOR         | ÚLTIMA CORREÇÃO     |{ANSI.RESET}")
        print(f"|{'-' * 21}|{'-' * 12}|{'-' * 14}|{'-' * 21}|")
        ...
        for invariant, data in health_data.invariants.items():
            ...
            if isinstance(last_correction, str):
                ...
                correction_display = f"{last_correction['acao']} ({last_correction['ago']} atrás)".ljust(20)
            
            print(f"| {invariant.ljust(20)} | {status_display} | {value_display} | {correction_display} |")
            
            if "alert" in data and data["alert"]:
                alert_len = len(data["alert"])
                padding = ' ' * (13 - alert_len) if alert_len <= 13 else ''
                print(f"| {' ' * 20} | {' ' * 12} | {ANSI.RED}🚨 {data['alert']}{ANSI.RESET}{padding} | {' ' * 21} |")
        ...
        for metric, value in health_data.system_metrics.items():
            ...
            print(f"| {metric.ljust(20)} | {status_display} | {value_display} | {'N/A'.ljust(20)} |")
    ...
    if args.command == "replay" or (args.prompt and args.prompt[0] == "replay"):
        ...
        if any([getattr(args, 'explain', False), getattr(args, 'train_bandit', False), 
            ...
            if args.explain:
                ...
                ReplayRenderer.render_summary(summary) if result else print(f"\n  ❌ Execução {exec_id} não encontrada\n")
    ...
    if args.command == "life-signals" or (args.prompt and args.prompt[0] == "life-signals"):
