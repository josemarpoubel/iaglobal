...
def main():
    ...
    logger.info("🚀 [UI] Iniciando IAGLOBAL Agent Workspace em http://%s:%d", args.host, args.port)
    logger.info("   [UI] Startup pode demorar ~40s em primeiro acesso; se falhar, tente novamente.")
    logger.info("   [UI] Dashboard: http://localhost:%d", args.port)
    logger.info("   [UI] API Docs: http://localhost:%d/docs", args.port)
    logger.info("   [UI] WebSocket: ws://localhost:%d/ws/progress/{execution_id}", args.port)
