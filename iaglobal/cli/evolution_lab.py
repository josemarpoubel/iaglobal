...
def cmd_init(args: argparse.Namespace):
    ...
    try:
        ...
        print(
            f"  Graph initialized: {n_core} core + {n_evo} EVO nodes ({'pipeline' if args.pipeline else 'synthetic'} mode)"
        )
    ...
    try:
        for i in range(n):
            ...
            if last:
                ...
                print(
                    f"      Gen {last.generation}: {last.evo_count} EVO nodes, "
...
def cmd_snapshots(args: argparse.Namespace):
    ...
    if not snaps:
        ...
        return
    print(
        f"  {'Gen':<6} {'EVO':<6} {'Core':<6} {'Mean Fitness':<16} {'Strategies':<12} {'Nodes'}"
    )
    print(f"  {'-' * 60}")
    for snap in snaps:
        node_list = ", ".join(
            n for n in sorted(snap.nodes) if n not in CORE_NODE_NAMES
        )[:80]
        print(
            f"  {snap.generation:<6} {snap.evo_count:<6} {snap.core_count:<6} "
...
def cmd_lineage(args: argparse.Namespace):
    ...
    if name not in graph.nodes:
        ...
        print(
            f"  Node '{name}' not found. Use 'lineage __list__' to see available nodes."
        )
        return
    ...
    try:
        ...
        logger.error(
            f"❌ [LAB] Falha ao gerar árvore de linhagem para {name}: {e}",
            exc_info=True,
        )
        raise
...
def cmd_status(args: argparse.Namespace):
    ...
    try:
        ...
        auto_gen = [
            s
            for s in all_skills
            if "auto_generated" in (s.tags if hasattr(s, "tags") else [])
        ]
        guardrails = [
            s
            for s in all_skills
            if "guardrail" in (s.tags if hasattr(s, "tags") else [])
        ]
    ...
    try:
        ...
        print(
            f"  Mutation rate:       {meta_stats['current_params']['mutation_rate']:.3f}"
        )
        print(
            f"  Crossover rate:      {meta_stats['current_params']['crossover_rate']:.3f}"
        )
        print(
            f"  Exploration rate:    {meta_stats['current_params']['exploration_rate']:.3f}"
        )
    except Exception:
...
def _print_patch(patch):
    ...
    if patch.nodes_added:
        ...
        for name in sorted(patch.nodes_added):
            ...
            print(
                f"      + {name}  (f={_fmt(nd.get('fitness', 0))}, "
    ...
    print(
        f"    Fitness: {_fmt(patch.fitness_before)} → {_fmt(patch.fitness_after)} "
        f"(Δ={patch.fitness_delta:+.4f})"
    )
    print(
        f"    Diversity: {patch.diversity_before} → {patch.diversity_after} "
...
def _print_patch_detail(patch):
    ...
    if patch.nodes_modified:
        for name, before, after in patch.nodes_modified:
            print(
                f"      ~ {name}  {_fmt(before.get('fitness', 0))} → "
...
def run_evolution_lab():
    ...
    # Argumentos globais
    parser.add_argument(
        "--mutation-rate", type=float, default=0.3, help="Mutation rate (default: 0.3)"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument(
        "--pipeline", action="store_true", help="Use real 25-node V3 pipeline graph"
    )
    ...
    subparsers.add_parser("init", help="Seed core graph + EVO population").set_defaults(
        func=cmd_init
    )
    ...
    subparsers.add_parser("snapshots", help="List all generations").set_defaults(
        func=cmd_snapshots
    )
    subparsers.add_parser("report", help="Full replay report").set_defaults(
        func=cmd_report
    )

    sp_diff = subparsers.add_parser(
        "diff", help="Git-style diff between two generations"
    )
    ...
    subparsers.add_parser(
        "patch-sequence", help="All consecutive patches"
    ).set_defaults(func=cmd_patch_sequence)
    subparsers.add_parser("detect-collapse", help="Run CollapseDetector").set_defaults(
        func=cmd_detect_collapse
    )

    sp_lineage = subparsers.add_parser("lineage", help="Show ancestry tree for a node")
    sp_lineage.add_argument(
        "name", type=str, nargs="?", default="__list__", help="Node name"
    )
    sp_lineage.set_defaults(func=cmd_lineage)

    subparsers.add_parser(
        "fitness-curve", help="Show per-generation fitness"
    ).set_defaults(func=cmd_fitness_curve)

    sp_export = subparsers.add_parser(
        "export-json", help="Export patches as JSON array"
    )
    sp_export.add_argument("file", type=str, help="Output JSON file path")
    sp_export.set_defaults(func=cmd_export_json)

    subparsers.add_parser("status", help="Unified evolution dashboard").set_defaults(
        func=cmd_status
    )
    subparsers.add_parser("help", help="Show help").set_defaults(
        func=lambda a: parser.print_help()
    )
    ...
    try:
        ...
        logger.error(
            f"❌ [LAB] Falha crítica ao executar '{args.subcommand}': {str(e)}",
            exc_info=True,
        )
