...
class HistoryRenderer:
    ...
    def render_execution(events: list, execution_id: str):
        ...
        for i, ev in enumerate(events):
            try:
                data = (
                    json.loads(ev["event_data"])
                    if isinstance(ev.get("event_data"), str)
                    else ev.get("event_data", {})
                )
            except Exception:
                data = {}
            step = data.get("step", ev.get("step", "?"))
            ts = (
                data.get("timestamp", ev.get("timestamp", ""))[11:19]
    ...
    def render_list(entries: list):
        ...
        for eid, count, last_ts in entries:
            print(
                f"  {last_ts[11:19] if last_ts else '':8s}  {eid:36s}  {count:3d} eventos"
            )
    ...
    def render_stats(stats: dict):
        ...
        for step, count in sorted(
            stats.get("by_step", {}).items(), key=lambda x: -x[1]
        ):
...
class ReplayRenderer:
    ...
    def render_summary(summary: dict):
        ...
        print(
            f"  Evolução:      {'sim' if summary.get('evolution_triggered') else 'não'}"
        )
        print(f"{'=' * 60}\n")
