...
async def run_compliance_audit(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[COMPLIANCE_AUDIT] Iniciando varredura regulamentar e auditoria de governança..."
    )
    ...
    try:
        ...
        logger.info(
            "[COMPLIANCE_AUDIT] Auditoria de conformidade finalizada com sucesso."
        )
        ...
        return {
            "output": result.get(
                "compliance_report", "Auditoria de conformidade executada"
            ),
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[COMPLIANCE_AUDIT] Falha crítica no pipeline do Compliance Audit Agent: %s",
            e,
        )
