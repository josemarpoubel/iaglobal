...
async def run_database_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[DATABASE_DESIGN] Analisando blueprint arquitetural para modelagem de dados..."
    )
    ...
    try:
        ...
        database_design = {
            "engine": db_tech,
            "orm": "SQLAlchemy"
            if "python" in db_tech_lower or "fastapi" in db_tech_lower
            else "Prisma",
            "tables": tables,
        ...
        logger.info(
            "[DATABASE_DESIGN] Modelagem concluída com sucesso: %d tabelas mapeadas para a engine '%s'.",
            len(tables),
            db_tech,
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[DATABASE_DESIGN] Falha crítica no pipeline do Database Design Node: %s", e
        )
