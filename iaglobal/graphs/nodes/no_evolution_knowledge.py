...
async def run_evolution_knowledge(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[EVOLUTION_KNOWLEDGE] Iniciando varredura assíncrona na base de memórias operacionais..."
    )
    ...
    try:
        ...
        logger.info(
            "[EVOLUTION_KNOWLEDGE] Sucesso! %d entradas recuperadas para a tarefa: %s",
            len(entries),
            task[:60],
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[EVOLUTION_KNOWLEDGE] Falha crítica no pipeline do Evolution Knowledge Node: %s",
            e,
        )

        return {
            "output": "",
            "evolution_knowledge": {"entries": [], "count": 0},
            "execution_metrics": {
                "model": resolved_model,
                "success": False,
                "latency": latency_ms,
