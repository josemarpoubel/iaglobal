...
async def _register_pipeline_nodes() -> Dict[str, Any]:
    ...
    for name in node_names:
        node_uid = (
            f"{name}-{name.__hash__()}"
            if hasattr(name, "__hash__")
            else f"{name}-{id(name)}"
        )
...
async def run_lineage_proof(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if batch_manifest is None and single_uid is None:
        try:
            ...
            logger.info(
                "[LINEAGE_PROOF] Integração no pipeline concluída — sovereign=%d/%d",
    ...
    # Nenhum input válido
    latency_ms = (time.time() - start_time) * 1000.0
    logger.warning("[LINEAGE_PROOF] Nenhum uid ou batch_manifest fornecido")
    return {
        "output": {
