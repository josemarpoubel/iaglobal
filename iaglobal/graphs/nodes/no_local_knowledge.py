    ...
    try:
        ...
        for m in ltm_results:
            if isinstance(m, dict):
                ...
                if len(content) > 30:
                    results.append(
                        {"source": "ltm", "content": content, "relevance": 0.9}
                    )
    except Exception as err:
    ...
    try:
        ...
        for m in vec_results:
            ...
            if content and len(str(content)) > 30:
                results.append(
                    {"source": "cbor2", "content": str(content)[:500], "relevance": 0.8}
                )
    except Exception as err:
...
async def run_local_knowledge(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[LOCAL_KNOWLEDGE] Iniciando varredura paralela em camadas de memórias locais..."
    )
    ...
    try:
        ...
        if results:
            ...
            logger.info(
                "[LOCAL_KNOWLEDGE] Sucesso! %d entradas recuperadas (%d caracteres lidos).",
                len(results),
                total_chars,
            )
        ...
        logger.info(
            "[LOCAL_KNOWLEDGE] Varredura finalizada. Nenhum conhecimento local correspondente encontrado."
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[LOCAL_KNOWLEDGE] Falha crítica no processamento de memórias locais: %s", e
        )
