...
async def run_reflexion(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not code:
        logger.warning(
            "[REFLEXION] Nenhum código encontrado nas memórias para ciclo de autorreflexão."
        )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(agent.analisar_resultado):
            analysis = await agent.analisar_resultado(
                codigo=code, resultado_sandbox=resultado_sandbox, task=task_str
            )
        else:
            analysis = await asyncio.to_thread(
                agent.analisar_resultado,
                codigo=code,
                resultado_sandbox=resultado_sandbox,
                task=task_str,
            )

        analysis_str = str(analysis or "")
        logger.info(
            "[REFLEXION] Análise finalizada com sucesso: %d caracteres gerados.",
            len(analysis_str),
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Processamento de autorreflexão consome mais tokens
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[REFLEXION] Falha crítica no pipeline do Reflexion Node: %s", e
        )
