...
async def run_sandbox_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[SANDBOX_VALIDATOR] Iniciando validação semântica e de segurança do código..."
    )
    ...
    try:
        ...
        logger.info(
            "[SANDBOX_VALIDATOR] Validação concluída. Código analisado com sucesso."
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custo estimado de processamento/inferência do validador
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[SANDBOX_VALIDATOR] Falha crítica no pipeline de validação de sandbox: %s",
            e,
        )
