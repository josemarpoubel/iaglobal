...
async def run_performance(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[PERFORMANCE] Iniciando auditoria de eficiência e performance no código...")
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(agent.audit):
            result = await agent.audit(code=code, performance_requirements=performance_reqs)
        else:
            result = await asyncio.to_thread(agent.audit, code=code, performance_requirements=performance_reqs)
        ...
        return {
            "output": result.get("output", "Auditoria de performance concluída") if isinstance(result, dict) else str(result),
            "performance": result,
            "execution_metrics": {
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[PERFORMANCE] Falha crítica no pipeline do Performance Audit Node: %s", e)
