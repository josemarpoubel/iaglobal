...
async def run_result_agent(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if output:
        try:
            ...
            project_dir = await asyncio.to_thread(save_result_artifact, task=task, files=files, code=output)
        ...
        if analysis.total_executions >= 10:  # Mínimo de execuções para análise
            ...
            if proposals:
                logger.info(
                    ...
                    len(proposals), analysis.total_executions,
                )
                
                # Validar e aplicar mutações (se bandit validator configurado)
                for proposal in proposals:
                    approved = await genomic_reflection.validate_with_bandit_async(proposal)
                    if approved:
                        proposal.status = "approved"
                        await genomic_reflection.apply_mutation_async(proposal)
                        logger.info(
                            "[RESULT_AGENT] ✅ Mutação aplicada: %s → %s",
                            proposal.agent_id, proposal.mutation_type.value,
                        )
