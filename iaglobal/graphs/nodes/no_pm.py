...
async def run_pm(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if asyncio.iscoroutinefunction(_pm_agent.extract_requirements):
        ...
        req_inputs = await asyncio.to_thread(
            _pm_agent.extract_requirements, raw, enhancement
        )
    ...
    if bus is not None and inbox is not None:
        ...
        if msgs:
            for msg in msgs:
                logger.info(
                    "[PM] Mensagem recebida de %s: type=%s", msg.sender, msg.type
                )
    ...
    if bus is not None:
        ...
        logger.info(
            "[PM] Evento 'requirements_ready' despachado com sucesso via barramento."
        )
    ...
    return {
        ...
        "execution_metrics": {
            ...
            ),  # Custos estimados de tokens para a análise do PM
        },
    }
