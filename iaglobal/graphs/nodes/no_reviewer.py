...
async def run_reviewer(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[REVIEWER] Iniciando ciclo de revisão técnica e auditoria de código...")
    ...
    try:
        ...
        return {
            "output": result.get("feedback", "Revisão concluída com sucesso") if isinstance(result, dict) else str(result or ""),
            "reviewer": result,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.004)  # Custo de inferência estimado do revisor
            }
        }
