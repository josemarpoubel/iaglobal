...
async def run_dependency(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    template = _load_requirements_template(project_root=ctx.get("working_directory", ""))
    resolved: DependencyResult | None = None
    if template:
        resolved = _DEPENDENCY_AGENT.resolve_dependencies(template, architecture_context)
        logger.info(
    ...
    install_result = None
    if missing and os.getenv("IAGLOBAL_AUTO_INSTALL_DEPENDENCIES", "").lower() in {"1", "true", "yes"}:
        install_result = auto_install(missing)

    logger.info(
        "[DEPENDENCY] %d dependencias, %d ausentes, auto_install=%s",
        len(result.get("dependencies", [])), len(missing), bool(install_result),
    )
