...
async def run_api_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[API_DESIGN] Analisando arquitetura do sistema para mapeamento de endpoints..."
    )
    ...
    try:
        ...
        logger.info(
            "[API_DESIGN] Desenho de API concluído: %d endpoints mapeados via framework '%s'.",
            len(endpoints),
            backend_tech,
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[API_DESIGN] Falha crítica no pipeline do API Design Node: %s", e
        )
