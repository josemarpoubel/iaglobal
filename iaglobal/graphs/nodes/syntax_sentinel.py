...
async def _try_ast_parse(code: str) -> Optional[ast.AST]:
    ...
    try:
        ...
        logger.debug("[SYNTAX_SENTINEL] SyntaxError: linha=%d col=%d msg=%s",
                      e.lineno or 1, e.offset or 0, e.msg)
        return None
...
async def run_syntax_sentinel(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if js_lang in ("js", "jsx", "ts"):
        logger.info("[SYNTAX_SENTINEL] Código %s detectado — pulando validação Python AST", js_lang)
        return {
            "output": code,
            "syntax_valid": True,
            "syntax_error": None,
