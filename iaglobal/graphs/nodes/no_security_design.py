...
async def run_security_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    error_context = str(ctx.get("error_context", "") or ctx.get("memory", {}).get("errors", {}).get("security_design", ""))
    ...
    logger.info("[SECURITY_DESIGN] Iniciando modelagem e injeção de controles de segurança...")
    ...
    try:
        ...
        logger.info("[SECURITY_DESIGN] Modelagem finalizada: %d falhas arquiteturais mitigadas.", total_issues)
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "success": True,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.004)  # Custo de inferência estimado do design
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[SECURITY_DESIGN] Falha crítica no pipeline do Security Design Agent: %s", e)
