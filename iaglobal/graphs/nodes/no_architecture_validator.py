...
async def run_architecture_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[ARCH_VALIDATOR] Iniciando validação semântica e estrutural da arquitetura...")
    ...
    try:
        ...
        logger.info("[ARCH_VALIDATOR] Validação concluída. Estrutura de código auditada com sucesso.")
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[ARCH_VALIDATOR] Falha crítica no pipeline do Architecture Validator: %s", e)
