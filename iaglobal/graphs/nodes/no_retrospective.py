...
async def run_retrospective(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[RETROSPECTIVE] Iniciando ciclo de análise retrospectiva e consolidação de aprendizado..."
    )
    ...
    try:
        ...
        logger.info(
            "[RETROSPECTIVE] Ciclo finalizado com sucesso: %d caracteres de aprendizado gerados.",
            len(analysis_str),
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[RETROSPECTIVE] Falha crítica no pipeline do Retrospective Node: %s", e
        )
