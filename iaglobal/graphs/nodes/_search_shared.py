...
async def retry_call(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_error = str(e)
            logger.debug(
                "[SEARCH] Attempt %d/%d failed: %s", attempt + 1, max_retries + 1, e
            )
        if attempt < max_retries:
            delay = base_delay * (2**attempt) + random.uniform(0, 0.5)
            logger.info(
                "[SEARCH] Retry %d/%d in %.1fs...", attempt + 1, max_retries, delay
            )
            await asyncio.sleep(delay)
    if last_error:
        logger.warning(
            "[SEARCH] All %d attempts failed: %s", max_retries + 1, last_error
        )
    return ""
