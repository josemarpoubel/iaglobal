                ...
                with urllib.request.urlopen(req, timeout=8) as r:
                    ...
                    if data:
                        ...
                        for c in data[:3]:
                            ...
                            lines.append(
                                f"• {name} | Capital: {capital} | Moeda: {currency} | Região: {region} | Pop: {population:,}"
                            )
                        return "\n".join(lines)
            except Exception:
                continue
...
def _exchange_rate(query: str) -> str:
    ...
    try:
        ...
        lines = [
            f"Taxas de câmbio base {base} ({data.get('time_last_update_utc', '')})"
        ]
        for t in targets:
            ...
            items.append(
                {
                    ...
                    "summary": summary.text.strip()[:200]
