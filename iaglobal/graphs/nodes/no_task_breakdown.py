...
async def run_task_breakdown(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[TASK_BREAKDOWN] Iniciando decomposição atômica e estruturação de subtarefas..."
    )
    ...
    try:
        ...
        return {
            "output": result.get("output", "Quebra de tarefas concluída")
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para decomposição
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[TASK_BREAKDOWN] Falha crítica no pipeline do Task Breakdown Node: %s", e
        )
