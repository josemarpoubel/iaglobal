    ...
    try:
        ...
        class LSPReporter:
            ...
            def flake(self, msg):
                ...
                severity = 2 if any(k in msg.__class__.__name__ for k in (
                    "UndefinedName", "UndefinedExport", "UndefinedLocal",
                    "DuplicateArgument", "LateFutureImport",
                )) else 3
                self.diagnostics.append({
                    "line": line,
                    "column": col,
...
async def run_lsp_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sources = ("multi_coder", "coder", "debug_coder", "backend_builder", "frontend_builder", "api_builder", "database_builder")
    ...
    if not code:
        code = ctx.get("generated_code", "") or str(ctx.get("input", {}).get("task", ""))
    ...
    has_syntax_error = any("Erro de sintaxe" in d.get("message", "") for d in all_diagnostics)
    ...
    logger.info(
        ...
        len(errors), len(warnings), success, latency,
    )
    if errors:
        for e in errors[:5]:
