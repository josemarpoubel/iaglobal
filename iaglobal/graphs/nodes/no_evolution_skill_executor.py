...
def _execute_auto_skills(skills: list, ctx: dict) -> Tuple[int, List[dict]]:
    ...
    for skill in skills:
        if skill is not None and "auto_execute" in getattr(skill, "tags", []):
            try:
                logger.info(
                    "[EVOLUTION_SKILL_EXECUTOR] Invocando runtime isolado para skill: %s",
                    skill.name,
                )
                ...
                execution_results.append(
                    {"skill": skill.name, "result": str(result)[:200]}
                )

            except SkillExecutionError as e:
                logger.warning(
                    "[EVOLUTION_SKILL_EXECUTOR] Falha controlada ao executar %s: %s",
                    skill.name,
                    e,
                )
                ...
                logger.error(
                    "[EVOLUTION_SKILL_EXECUTOR] Crash inesperado na skill %s: %s",
                    skill.name,
                    e,
                )
                execution_results.append(
                    {"skill": skill.name, "error": f"Unexpected crash: {str(e)}"}
                )
...
async def run_evolution_skill_executor(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[EVOLUTION_SKILL_EXECUTOR] Mapeando a árvore de habilidades registradas no ecossistema..."
    )

    try:
        # Coleta de forma segura as instâncias de skills registradas
        skills = (
            list(skill_registry.skills.values())
            if skill_registry and hasattr(skill_registry, "skills")
            else []
        )
        logger.info(
            "[EVOLUTION_SKILL_EXECUTOR] %d skills detectadas no registry.", len(skills)
        )
        ...
        logger.info(
            "[EVOLUTION_SKILL_EXECUTOR] %d skills executadas automaticamente com sucesso.",
            executed,
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[EVOLUTION_SKILL_EXECUTOR] Falha crítica no pipeline do Skill Executor: %s",
            e,
        )
