...
async def _gerar_testes_saude(
    dados_chappie: Dict[str, Any], ctx: Dict[str, Any]
) -> str:
...
def _fmt_expression(expr: Dict[str, Any]) -> str:
    ...
    mem = expr.get("memory", {})
    cycles = (
        ", ".join(k for k, v in expr.get("cycles_activated", {}).items() if v)
        or "nenhum"
    )
    lines = [
    ...
    lines.append(
        f"- **Geração**: {expr.get('generation', '?')} | marker `{dna.get('lineage_marker', '?')}`"
    )
    lines.append(f"- **Urgência**: {expr.get('urgency', '?')}")
    lines.append(f"- **Ciclos ativados**: {cycles}")
    lines.append(
        f"- **SAMe**: {res.get('same_balance', '?')} | NADPH: {res.get('nadph_reserve', '?')}"
    )
...
def _format_markdown_report(
    ...
    lines = [
        ...
        (
            ...
            + ", ".join(f"{k}={v}" for k, v in immune_events.items() if v)
            if any(immune_events.values())
            else "- _sem eventos de degradação nesta sessão_"
        ),
