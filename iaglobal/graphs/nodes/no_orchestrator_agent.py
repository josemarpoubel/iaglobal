...
async def run_orchestrator_agent(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if asyncio.iscoroutinefunction(_orchestrator_agent.route):
        ...
        orch = await asyncio.to_thread(
            _orchestrator_agent.route, enhancement, requirements
        )
    ...
    if bus is not None and inbox is not None:
        ...
        if msgs:
            for msg in msgs:
                logger.info(
                    "[ORCHESTRATOR] Mensagem recebida de %s: type=%s",
                    msg.sender,
                    msg.type,
                )
    ...
    if bus is not None:
        ...
        logger.info(
            "[ORCHESTRATOR] Evento 'orchestration_ready' despachado com sucesso para o PM."
        )
    ...
    return {
        ...
        "execution_metrics": {
            ...
            ),  # Custo estimado de tokens do roteamento
        },
    }
