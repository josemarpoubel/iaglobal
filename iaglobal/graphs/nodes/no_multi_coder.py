...
async def run_multi_coder(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not built_prompt or len(built_prompt) < 10:
        logger.warning("[MULTI_CODER] Prompt muito curto ou vazio. Repassando coder output legado.")
    ...
    logger.info("[MULTI_CODER] Ativando geração paralela para backend, frontend e database...")
    ...
    try:
        ...
        logger.info("[MULTI_CODER] Ciclo finalizado. Status=%s | Falhas parciais=%d/3 | %d caracteres totais",
                    status, failures, total_chars)

        # Formata o agregado final de códigos estruturados
        multi_output = "\n\n".join(
            f"# === {k.upper()} ===\n{v}" for k, v in parts.items() if v
        ) if any(parts.values()) else ""
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.030) * (3 - failures)  # Ajusta o custo dinamicamente pelo número de camadas geradas
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[MULTI_CODER] Falha crítica no pipeline do Multi-Coder Agent: %s", e)
        
        # REPORTA A FALHA EXPLICITAMENTE PARA O BANDIT POLICY APRENDER COM O COMPORTAMENTO DO MODELO
        return {
            "output": "",
            "multi_coder": {"status": "failed", "parts": {}, "total_chars": 0, "failures": 3, "error": str(e)},
            "execution_metrics": {
                "model": resolved_model,
                "success": False,
                "latency": latency_ms,
