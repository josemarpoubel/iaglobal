        ...
        try:
            ...
            if result and len(str(result).strip()) > 30:
                ...
                try:
                    ...
                    logger.debug("[SEARCH_AGENT] Ignorada falha na rotina de aprendizado: %s", learner_err)
                    
                logger.info("[SEARCH_AGENT] Sucesso técnico: %d chars extraídos (q: %.50s)", len(result_str), q)
...
async def run_search_agent(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not task or len(task) < 5:
        await asyncio.to_thread(record_error, SOURCE, "Task string empty or too short", {"task": task})
    ...
    logger.info("[SEARCH_AGENT] Iniciando decomposição analítica e geração de queries técnicas...")
    ...
    try:
        ...
        logger.info("[SEARCH_AGENT] Despachando laço sequencial de requisições de rede para a Thread Pool...")
        
        # DESVIA TODO O TRÁFEGO DE REDE, RETRIES SÍNCRONOS E GRAVAÇÃO DE CONHECIMENTO PARA A THREAD POOL ISOLADA
        result_content, is_success = await asyncio.to_thread(_sync_execute_and_learn, candidates, task)
        ...
        if is_success and result_content:
            ...
            return {
                ...
                "execution_metrics": {
                    ...
                    "success": True,
                    "latency": latency_ms,
                    "cost": ctx.get("estimated_cost", 0.0)  # Consumo puramente local/motores abertos
                }
            }

        # Caso todas as queries segmentadas falhem ou retornem strings curtas
        await asyncio.to_thread(record_error, SOURCE, "All technical queries returned empty answers", {"task": task[:100]})
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[SEARCH_AGENT] Falha crítica no processamento concorrente do Search Agent: %s", e)
