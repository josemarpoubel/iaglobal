...
async def run_evolution_dynamic_registry(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[EVOLUTION_DYNAMIC_REGISTRY] Iniciando carregamento assíncrono de skills mutadas no SQLite..."
    )
    ...
    try:
        ...
        stats = {
            ...
            "active_skills": sum(
                1 for s in dynamic_skills if isinstance(s, dict) and s.get("active", 0)
            ),
        }

        logger.info(
            "[EVOLUTION_DYNAMIC_REGISTRY] Sucesso! %d skills dinâmicas carregadas para o ecossistema.",
        ...
        return {
            ...
            "evolution_dynamic_registry": {
                "stats": stats,
                "skills": [
                    {"name": s["name"], "description": s.get("description", "")[:100]}
                    for s in dynamic_skills[:10]
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[EVOLUTION_DYNAMIC_REGISTRY] Falha crítica ao ler repositório de skills dinâmicas: %s",
            e,
        )
