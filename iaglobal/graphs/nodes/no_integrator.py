...
import iaglobal.graphs.nodes.no_applied_ai_engineer 
...
async def run_integrator(ctx: dict) -> dict:
    ...
    source_count = 0
    last_artifact = None
    sources = [("frontend_builder", "frontend"), ("backend_builder", "backend"), ("database_builder", "database"), ("api_builder", "api")]
    ...
    return {"output": {"code": integrated_code, "files": files, "source_count": source_count}, "integrated_code": integrated_code, "files": files, "source_count": source_count, "execution_metrics": {"model": "deterministic_integrator", "success": True, "latency": latency_ms, "cost": 0.0}}
...
def register_all(orchestrator):
    ...
    for _, name, _ in pkgutil.iter_modules(nodes_pkg.__path__):
        if name.startswith("_") or name == "no_integrator": continue
        if name in registered_nodes: continue
        try:
            ...
            if hasattr(orchestrator, "registered_nodes"): orchestrator.registered_nodes.add(name)
    ...
    for skill_name, opts in PIPELINE_SKILLS:
        try:
            ...
            if not skill or not getattr(skill, 'run_fn', None):
                ...
                from iaglobal.graphs.nodes import create_skill_node as _create_skill_node
                node = _create_skill_node(skill_name, depends_on=list(opts.get("depends_on", [])))
                if node:
                    graph.add_node(node)
                continue
                
            node_name = opts.get("name", skill_name)
            node = skill.to_node(name=node_name, depends_on=list(opts.get("depends_on", [])), strategy=opts.get("strategy", "general"), critical=bool(opts.get("critical", False)))
            graph.add_node(node)
        except Exception as e:
