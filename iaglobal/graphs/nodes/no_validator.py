...
async def run_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not code:
        logger.warning(
            "[VALIDATOR] Nenhum código encontrado nas memórias para validação."
        )
    ...
    logger.info(
        "[VALIDATOR] Iniciando auditoria semântica e pontuação do código gerado..."
    )
    ...
    try:
        ...
        logger.info(
            "[VALIDATOR] Auditoria concluída. Score=%.1f | Erros detectados=%d",
            score,
            len(errors),
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para a validação
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[VALIDATOR] Falha crítica no pipeline do Validator Node: %s", e
        )
