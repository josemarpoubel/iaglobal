...
async def run_observability_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[OBSERVABILITY_DESIGN] Iniciando desenho cognitivo dos componentes de telemetria..."
    )
    ...
    try:
        ...
        logger.info(
            "[OBSERVABILITY_DESIGN] Desenho de observabilidade finalizado com sucesso."
        )
        ...
        return {
            "output": result.get("observability_report", result)
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para modelagem
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[OBSERVABILITY_DESIGN] Falha crítica no pipeline do Observability Design Node: %s",
            e,
        )
