...
async def run_knowledge(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if local_result and not search_result:
        ...
        logger.info("[KNOWLEDGE] Merge de conhecimento: local (%d) + web (%d)", len(local_result), len(search_result))
    ...
    try:
        if search_result and len(search_result) > 50:
            ...
            try:
                ...
                logger.info("[KNOWLEDGE] KnowledgeAgent armazenado com sucesso: %d chars.", len(search_result))
            except Exception as e:
                logger.warning("[KNOWLEDGE] Falha controlada no armazenamento do KnowledgeAgent: %s", e)
            ...
            try:
                ...
                logger.info("[KNOWLEDGE] KnowledgeGraph indexou %d novos conceitos.", len(concepts) if concepts else 0)
            except Exception as e:
                logger.warning("[KNOWLEDGE] Falha controlada na indexação do KnowledgeGraph: %s", e)
        ...
        knowledge_summary = ""
        try:
            knowledge_summary = await asyncio.to_thread(knowledge.summarize, max_entries=5)
        except Exception as e:
            logger.warning("[KNOWLEDGE] Falha ao computar o resumo de conhecimento: %s", e)
        ...
        try:
            ...
            logger.warning("[KNOWLEDGE] Falha ao coletar conceitos no topo do grafo: %s", e)
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[KNOWLEDGE] Falha crítica no pipeline do Knowledge Node: %s", e)
        
        return {
            "output": "",
            "knowledge": {"relevant": [], "summary": "", "concepts": [], "error": str(e)},
            "execution_metrics": {
                "model": resolved_model,
                "success": False,
                "latency": latency_ms,
