...
async def run_system_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[SYSTEM_DESIGN] Analisando componentes de infraestrutura para modelagem não-funcional...")
    ...
    try:
            ...
            "component_details": [
                {
                    ...
                    "responsibility": c.get("responsibility") if isinstance(c, dict) else "unknown",
        ...
        logger.info("[SYSTEM_DESIGN] Desenho de sistema concluído: %d componentes instrumentados.", len(components))
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[SYSTEM_DESIGN] Falha crítica no pipeline do System Design Node: %s", e)
