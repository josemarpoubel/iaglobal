...
from iaglobal.graphs.nodes._search_sources import (
    google_playwright_search,
    bing_playwright_search,
    github_search,
...
async def _try_source(name: str, fn: Callable, task: str, timeout: int = 12) -> str:
    ...
    try:
        ...
        if len(result_str) > 30:
            logger.info(
                "[SEARCH] Fonte '%s' OK: %d caracteres extraídos.",
                name,
                len(result_str),
            )
            return result_str
        ...
        logger.debug(
            "[SEARCH] Timeout atingido para a fonte: %s (limite de %ds)", name, timeout
        )
...
async def _async_router_wrapper(task: str):
    return await run_search_router(task)


async def _async_search_agent_wrapper(task: str):
    return await _search_agent.process_task(task)


async def _async_web_brain_wrapper(task: str):
    return await _web_brain.search_text(task, max_results=5)
...
async def run_search(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not task or len(task) < 5:
        await asyncio.to_thread(
            record_error, SOURCE, "Empty task string", {"task": task}
        )
    ...
    if skip:
        logger.info(
            "[SEARCH] skip-list ativo: %s — %d/%d fontes ativas (economia de ATP)",
            sorted(skip),
            len(active_sources),
            len(SOURCES),
        )
    ...
    if cached:
        logger.info(
            "[SEARCH] Cache em disco atingido (Hit): %d caracteres restaurados.",
            len(cached),
        )
        latency_ms = (time.time() - start_time) * 1000.0
        return {
    ...
    logger.info(
        "[SEARCH] Cache Miss. Iniciando orquestração em lote de %d fontes de busca...",
        len(active_sources),
    )
    all_results = []
    ...
    try:
        for batch_start in range(0, len(active_sources), _BATCH_SIZE):
            ...
            tasks = [
                _try_source(name, fn, task, timeout) for name, fn, timeout in batch
            ]
            ...
            if total_chars >= _MIN_RESULTS:
                logger.info(
                    "[SEARCH] Carga ideal atingida (%d caracteres em %d fontes) — Parada antecipada.",
                    total_chars,
                    len(all_results),
                )
                break
        ...
        if combined:
            ...
            logger.info(
                "[SEARCH] Varredura concluída. Total: %d caracteres de %d/%d fontes ativas.",
                len(combined),
                len(all_results),
                len(active_sources),
            )
            ...
            return {
                ...
                "execution_metrics": {
                    ...
                    "cost": ctx.get(
                        "estimated_cost", 0.0
                    ),  # Motores abertos locais/web sem custo direto
                },
            }

        # Fallback: todas as fontes de busca falharam (sandbox bloqueia rede)
        logger.warning(
            "[SEARCH] Todas as %d fontes falharam. Gerando contexto via LLM local...",
            len(active_sources),
        )
        await asyncio.to_thread(
            record_error,
            SOURCE,
            "All sources empty or timed out, using LLM fallback",
            {"task": task[:100]},
        )
        try:
            ...
            if len(fallback_text) > 50:
                logger.info(
                    "[SEARCH] Fallback LLM gerou %d caracteres", len(fallback_text)
                )
                return {
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[SEARCH] Falha crítica durante o processamento concorrente do nó de buscas: %s",
            e,
        )
