...
async def run_coder(ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if msgs:
            for msg in msgs:
                ...
                if msg.payload.get("plan"):
                    logger.info("[CODER] Plano estratégico recebido do planner via bus.")
    ...
    if not task and not built_prompt:
        await asyncio.to_thread(record_error, "coder", "Empty task context", {"task": task})
    ...
    security_feedback = (
        memory.get("code_executor", {}).get("security_feedback", "")
    ...
    try:
        ...
        if not code or len(code.strip()) <= 5:
            await asyncio.to_thread(record_error, "coder", "Empty/short content generated", {"task": task[:100]})
            latency_ms = (time.time() - start_time) * 1000.0
            return {
                "output": "", "code": "",
        ...
        if bus is not None:
            ...
            logger.info("[CODER] Evento 'code_ready' injetado no barramento para o Critic.")
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "success": True,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.015)  # Custo de inferência estimado para geração de código
            }
        }
