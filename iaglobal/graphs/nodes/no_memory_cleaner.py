...
async def run_memory_cleaner(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[CLEANER] Iniciando ciclo de limpeza e compactação de memória (critic score=%.1f, approved=%s)...",
        score,
        approved,
    )

    used_sources = {
        k for k in memory.keys() if k in ("coder", "multi_coder", "prompt_builder")
    }
    discarded = []
    ...
    for src in ("search", "local_knowledge"):
        ...
        if src_output and src not in used_sources:
            ...
            logger.debug(
                "[CLEANER] Descartando cache não utilizado de %s (%d caracteres).",
                src,
                len(src_output),
            )
    ...
    if discarded:
        logger.info(
            "[CLEANER] %d fontes descartadas (%d caracteres liberados na memória).",
            len(discarded),
