...
async def run_execution_plan(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "[EXECUTION_PLAN] Plano de execução técnica finalizado com sucesso."
        )
        ...
        return {
            "output": result.get("output", "Plano de execução concluído")
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para o plano
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[EXECUTION_PLAN] Falha crítica no pipeline do Execution Plan Node: %s", e
        )
