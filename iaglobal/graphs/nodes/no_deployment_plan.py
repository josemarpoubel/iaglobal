...
async def run_deployment_plan(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[DEPLOYMENT_PLAN] Iniciando desenho e modelagem do plano de implantação..."
    )
    ...
    try:
        ...
        return {
            "output": result.get("output", "Plano de deploy finalizado")
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado do planejamento
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[DEPLOYMENT_PLAN] Falha crítica no pipeline do Deployment Plan Node: %s", e
        )
