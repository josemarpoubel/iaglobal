...
"""
Deployment Plan Node — Gera a estratégia e o plano de implantação (deploy) da aplicação.
...
async def run_deployment_plan(ctx: Dict[str, Any]) -> Dict[str, Any]:
    """
    Executa a geração do plano de deploy de forma assíncrona e não-bloqueante.
    Mapeia latência, custos e sucesso do planejamento para o JointOptimizationLoop.
    """
    start_time = time.time()
    resolved_model = "deployment_planner_agent_llm"
    
    # Lazy import mantido para preservar a inicialização limpa original
    from iaglobal.agents.planner_agent import PlannerAgent

    logger.info("[DEPLOYMENT_PLAN] Iniciando desenho e modelagem do plano de implantação...")
    ...
    try:
        ...
        # Como planos de deploy envolvem inteligência densa orientada a IA,
        ...
        logger.info("[DEPLOYMENT_PLAN] Plano de deploy gerado com sucesso.")
        ...
        return {
            "output": result.get("output", "Plano de deploy finalizado") if isinstance(result, dict) else str(result),
            "deployment_plan": result,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.005)  # Custo de inferência estimado do planejamento
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[DEPLOYMENT_PLAN] Falha crítica no pipeline do Deployment Plan Node: %s", e)
        
        # REPORTA A FALHA EXPLICITAMENTE PARA O BANDIT POLICY APRENDER COM O COMPORTAMENTO DO MODELO
        return {
            "output": "Falha no processo de planejamento de deploy",
