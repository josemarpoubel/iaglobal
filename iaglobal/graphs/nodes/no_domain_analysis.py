...
async def run_domain_analysis(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        entities_count = len(result.get("entities", [])) if isinstance(result, dict) else 0
        logger.info("[DOMAIN_ANALYSIS] Análise concluída. Mapeadas %d entidades do domínio.", entities_count)
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.004)  # Custo estimado de tokens da inferência
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[DOMAIN_ANALYSIS] Falha crítica no pipeline do Domain Analysis Agent: %s", e)
