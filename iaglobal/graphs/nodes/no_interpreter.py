...
async def run_interpreter(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[INTERPRETER] Iniciando interpretação cognitiva e análise do prompt...")
    ...
    try:
        ...
        logger.info("[INTERPRETER] Interpretação e refinamento de intenções concluídos com sucesso.")
        ...
        return {
            "output": result.get("enhanced_task", "") if isinstance(result, dict) else str(result),
            "interpreter": result,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.003)  # Custo de inferência estimado para a interpretação
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[INTERPRETER] Falha crítica no pipeline do Interpreter Node: %s", e)
