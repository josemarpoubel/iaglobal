...
async def run_documentation(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bus is not None and inbox is not None:
        ...
        if msgs:
            for msg in msgs:
                logger.info(
                    "[DOCUMENTATION] Mensagem recebida de %s: type=%s",
                    msg.sender,
                    msg.type,
                )

    # Coleta de forma resiliente as saídas dos nós anteriores para contextualização
    coder_output = memory.get("coder", {}).get("output", "") or memory.get(
        "multi_coder", {}
    ).get("output", "")
    ...
    if not task and not coder_output and not built_prompt:
        await asyncio.to_thread(
            record_error, "documentation", "Empty task context", {"task": task}
        )
    ...
    try:
        logger.info(
            "[DOCUMENTATION] Disparando inferência assíncrona para redação técnica do documento..."
        )

        # Usa AgentBase para chamar BanditPolicy
        agent = _get_documentation_agent()
        doc = await agent._call_llm(
            prompt=prompt, task_type="documentation", timeout=60.0
        )
        ...
        if is_success:
            logger.info(
                "[DOCUMENTATION] Documento técnico gerado com sucesso: %d caracteres.",
                len(doc),
            )
            ...
            if bus is not None:
                ...
                logger.info(
                    "[DOCUMENTATION] Evento 'doc_ready' injetado no barramento com sucesso."
                )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[DOCUMENTATION] Falha crítica no pipeline do Documentation Agent: %s", e
        )
        await asyncio.to_thread(
