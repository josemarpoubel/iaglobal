...
async def run_prompt_intake(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[PROMPT_INTAKE] Iniciando classificação cognitiva de intenções e escopo..."
    )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(_intent_classifier.classify):
            ...
            classification = await asyncio.to_thread(
                _intent_classifier.classify, normalized
            )
        ...
        if bus is not None and inbox is not None:
            ...
            if msgs:
                for msg in msgs:
                    logger.info(
                        "[PROMPT_INTAKE] Mensagem recebida de %s: type=%s",
                        msg.sender,
                        msg.type,
                    )
        ...
        if bus is not None:
            ...
            logger.info(
                "[PROMPT_INTAKE] Evento 'prompt_ready' despachado com sucesso via barramento."
            )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para classificação
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[PROMPT_INTAKE] Falha crítica na largada do pipeline (Intake Node): %s", e
        )
