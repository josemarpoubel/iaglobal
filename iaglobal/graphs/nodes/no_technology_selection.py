...
async def run_technology_selection(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[TECH_SELECTION] Iniciando análise e seleção cognitiva da pilha de tecnologias...")
    ...
    try:
        ...
        logger.info("[TECH_SELECTION] Definição de stack tecnológica finalizada com sucesso.")
        ...
        return {
            "output": result.get("output", "Seleção de tecnologia concluída") if isinstance(result, dict) else str(result),
            "technology_selection": result,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.005)  # Custo de inferência estimado para seleção de stack
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[TECH_SELECTION] Falha crítica no pipeline do Technology Selection Node: %s", e)
