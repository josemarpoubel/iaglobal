...
from iaglobal.memory.memory_vector import (
    store as mem_vector_store,
    init_db as vector_init_db,
)
...
def _calc_relevance(text: str) -> float:
    ...
    score = sum(
        weight for kw, weight in _RELEVANCE_KEYWORDS.items() if kw in text_lower
    )
    ...
    for source_name, raw_content in raw_caches.items():
        ...
        if useful_text:
            ...
            extracted_parts.append(
                f"=== KNOWLEDGE SOURCE: {source_name.upper()} (relevance: {relevance:.2f}) ===\n{useful_text}"
            )
            curated_logs.append(
                {
                    "source": source_name,
                    "relevance": relevance,
                    "chars": len(useful_text),
                }
            )
            ...
            try:
                ...
                logger.debug(
                    "[KNOWLEDGE_ANALYZER] Erro controlado ao persistir fragmento: %s",
                    db_err,
                )
...
async def run_knowledge_analyzer(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[KNOWLEDGE_ANALYZER] Iniciando ciclo assíncrono de higienização e filtragem de cache..."
    )
    ...
    try:
        ...
        logger.info(
            "[KNOWLEDGE_ANALYZER] Curadoria finalizada. Extraídas %d fontes úteis de conhecimento.",
            source_count,
        )
        latency_ms = (time.time() - start_time) * 1000.0
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[KNOWLEDGE_ANALYZER] Falha crítica no pipeline do Analyzer: %s", e
        )
