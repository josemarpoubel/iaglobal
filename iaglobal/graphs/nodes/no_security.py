...
async def run_security(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[SECURITY] Iniciando auditoria de vulnerabilidades e brechas no código..."
    )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(agent.audit):
            ...
            result = await asyncio.to_thread(
                agent.audit, code=code, security_requirements=security_reqs
            )

        logger.info(
            "[SECURITY] Auditoria estática de segurança finalizada com sucesso."
        )
        ...
        return {
            "output": result.get("output", "Auditoria de segurança concluída")
