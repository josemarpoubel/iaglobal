...
async def run_performance_audit(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not code:
        logger.warning(
            "[PERF_AUDIT] Nenhum código encontrado nas memórias para auditoria de performance."
        )
    ...
    try:
        logger.info(
            "[PERF_AUDIT] Iniciando varredura analítica profunda contra gargalos e vazamentos..."
        )
        ...
        report = (
            result.get("performance_audit_report", {})
        ...
        logger.info(
            "[PERF_AUDIT] Auditoria finalizada. Score de risco de performance obtido: %s",
            risk_score,
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[PERF_AUDIT] Falha crítica no pipeline do Performance Audit Agent: %s", e
        )
