...
def _get_ltm():
    ...
    if _ltm is None:
        try:
            ...
            logger.warning("[MEMORY_WRITER] Falha ao inicializar LTM: %s, usando fallback em memória.", e)
...
def _execute_sync_persistence(task: str, built_prompt: str, coder_output: str, critic_data: dict, search_insights: list) -> int:
    ...
    if approved and coder_output:
        try:
            ...
            logger.warning("[MEMORY_WRITER] Falha ao persistir dados aprovados na LTM/Vetor: %s", e)
    else:
...
async def run_memory_writer(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bus is not None and inbox is not None:
        ...
        if msgs:
            for msg in msgs:
                logger.info("[MEMORY_WRITER] Mensagem processada na inbox: de %s, tipo %s.", msg.sender, msg.type)
                if msg.type == "review_done":
                    logger.info("[MEMORY_WRITER] Contrato 'review_done' interceptado via barramento.")
    ...
    if critic_data.get("_skip"):
        logger.info("[MEMORY_WRITER] Critic foi pulado (sem output para avaliar) — nada a persistir")
        latency_ms = (time.time() - start_time) * 1000.0
        return {
            "output": "0 itens (skip)",
            "stored": False, "stored_count": 0, "success": True,
            "execution_metrics": {
                "model": resolved_model, "success": True,
    ...
    logger.info("[MEMORY_WRITER] Analisando veredito do Critic: score=%.1f | approved=%s", score, approved)
    ...
    if src_data and len(src_data) > 100:
        search_insights.append({"content": src_data[:500], "source": "search", "type": "web_search"})
    ...
    try:
        ...
        stored_count = await asyncio.to_thread(
            ...
            task, built_prompt, coder_output, critic_data, search_insights
        )
        
        logger.info("[MEMORY_WRITER] Ciclo finalizado. Persistidos %d itens com sucesso nos bancos locais.", stored_count)
        ...
        if bus is not None:
            ...
            logger.info("[MEMORY_WRITER] Notificação 'memory_persisted' despachada com sucesso via barramento.")
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[MEMORY_WRITER] Falha crítica no pipeline de persistência de memórias: %s", e)
        await asyncio.to_thread(record_error, "memory_writer", str(e), {"task": task[:100]})
