    ...
    for q in candidates:
        ...
        try:
            ...
            if result and len(str(result).strip()) > 30:
                ...
                logger.info(
                    "[WEB_BRAIN] Sucesso prático: %d chars extraídos (q: %.50s)",
                    len(result_str),
                    q,
                )
...
async def run_search_web_brain(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not task or len(task) < 5:
        await asyncio.to_thread(
            record_error, SOURCE, "Task description empty", {"task": task}
        )
    ...
    logger.info(
        "[WEB_BRAIN] Gerando sub-queries fragmentadas focadas em exemplos práticos..."
    )
    ...
    try:
        ...
        logger.info(
            "[WEB_BRAIN] Despachando threads de scraping de rede em background..."
        )
        ...
        result_content, is_success = await asyncio.to_thread(
            _execute_sync_brain_search, candidates
        )
        ...
        await asyncio.to_thread(
            record_error,
            SOURCE,
            "WebBrain returned empty response for all variations",
            {"task": task[:100]},
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[WEB_BRAIN] Falha crítica no pipeline concorrente do WebBrain Node: %s", e
        )
