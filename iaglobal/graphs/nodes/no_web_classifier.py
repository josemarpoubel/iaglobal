...
async def run_web_classifier(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[WEB_CLASSIFIER] Iniciando classificação cognitiva de conteúdos coletados na internet..."
    )
    ...
    try:
        ...
        logger.info(
            "[WEB_CLASSIFIER] Classificação e categorização concluídas com sucesso."
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para o classificador
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[WEB_CLASSIFIER] Falha crítica no pipeline do Web Classifier Node: %s", e
        )
