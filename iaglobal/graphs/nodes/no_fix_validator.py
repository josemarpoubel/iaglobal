...
async def run_fix_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[FIX_VALIDATOR] Iniciando validação semântica da correção aplicada no código..."
    )
    ...
    try:
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[FIX_VALIDATOR] Falha crítica no pipeline do Fix Validator: %s", e
        )
