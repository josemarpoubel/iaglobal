...
# iaglobal/graphs/nodes/no_fix_validator.py

"""
Fix Validator Node — Valida se as correções de bugs aplicadas estão corretas e seguras.
...
async def run_fix_validator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    """
    Executa a validação da correção aplicada de forma assíncrona e não-bloqueante.
    Mapeia latência, custo e sucesso analítico para o JointOptimizationLoop.
    """
    start_time = time.time()
    resolved_model = "fix_validator_agent_llm"
    
    logger.info("[FIX_VALIDATOR] Iniciando validação semântica da correção aplicada no código...")
    
    # Coleta os dados de entrada de forma resiliente de fontes de depuração ou código direto
    memory = ctx.get("memory", {})
    coder_data = ctx.get("debug_coder", {}) or ctx.get("coder", {}) or memory.get("debug_coder", {}) or memory.get("coder", {})
    ...
    try:
        ...
        logger.info("[FIX_VALIDATOR] Validação de correção concluída com sucesso.")
        
        latency_ms = (time.time() - start_time) * 1000.0
        is_success = isinstance(result_dict, dict) and result_dict.get("valid", True)

        # Retorno higienizado cumprindo as Regras 1, 3 e 5 do AGENTS.md (Sem dar dict unpack do ctx na RAM)
        return {
            "output": result_dict.get("summary", "Validação de correção finalizada"),
            "fix_validator": result_dict,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.003)  # Custo de inferência estimado do validador
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[FIX_VALIDATOR] Falha crítica no pipeline do Fix Validator: %s", e)
        
        # REPORTA A FALHA EXPLICITAMENTE PARA O BANDIT POLICY APRENDER COM O COMPORTAMENTO DO MODELO
        return {
            "output": "Falha no processo de validação de correção",
            "fix_validator": {"valid": False, "error": str(e)},
            "execution_metrics": {
                "model": resolved_model,
                "success": False,
                "latency": latency_ms,
