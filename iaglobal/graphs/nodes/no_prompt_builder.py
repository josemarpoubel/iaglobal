...
async def run_prompt_builder(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[PROMPT_BUILDER] Iniciando consolidação e engenharia de contexto do prompt...")
    ...
    try:
        ...
        if knowledge_concepts:
            concept_lines = [f"  - {c['name']} (freq: {c['frequency']})" for c in knowledge_concepts[:10] if isinstance(c, dict)]
            if concept_lines:
        ...
        logger.info("[PROMPT_BUILDER] Engenharia concluída: %d caracteres estruturados em %d seções.", 
                    len(built_prompt), len(sections))
        ...
        return {
            ...
            "prompt_sections": {src: bool(memory.get(src, {}).get("output") if isinstance(memory.get(src), dict) else memory.get(src)) for src in search_sources},
            "success": is_success,
            "execution_metrics": {
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[PROMPT_BUILDER] Falha crítica no pipeline do Prompt Builder Node: %s", e)
        await asyncio.to_thread(record_error, "prompt_builder", str(e), {"task": task[:100]})
