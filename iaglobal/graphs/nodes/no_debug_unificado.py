...
async def _usar_debugger_agent(
    ...
    has_syntax_error = any(
        "syntax" in err.lower() or "'('" in err for err in (lsp_errors or [])
    )
...
async def _try_syntax_sentinel(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return await run_syntax_sentinel(
            {"memory": {"coder": code}, "input": {"task": ""}}
        )
    except Exception as e:
...
async def _try_js_validator(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return await run_js_syntax_sentinel(
            {"memory": {"coder": code}, "input": {"task": ""}}
        )
    except Exception as e:
