...
async def run_genesis_builder(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[GENESIS_BUILDER] Iniciando bootstrap e orquestração de gênese do ecossistema..."
    )
    ...
    try:
        ...
        logger.info(
            "[GENESIS_BUILDER] Bootstrap de pipeline multiagente finalizado com sucesso."
        )
        ...
        return {
            "output": result.get("summary", "Gênese do pipeline concluída")
            if isinstance(result, dict)
            else str(result or ""),
            "genesis_builder": {"output": result},
            "execution_metrics": {
                ...
                "cost": ctx.get(
                    "estimated_cost", 0.01
                ),  # Orquestrações de gênese costumam gastar mais tokens
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[GENESIS_BUILDER] Falha crítica durante a orquestração de gênese: %s", exc
        )
