...
async def run_enhancement(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[ENHANCEMENT] Iniciando refinação cognitiva e enriquecimento da tarefa...")
    ...
    knowledge_context = str(ctx.get("knowledge_context", "") or ctx.get("memory", {}).get("knowledge", {}).get("summary", ""))
    error_context = str(ctx.get("error_context", "") or ctx.get("memory", {}).get("errors", {}).get("enhancement", ""))
    ...
    try:
        ...
        enhancement_def = {
            ...
            "scope": {
                "phases": ["definition"],
                "complexity": result.get("complexity", "medium") if isinstance(result, dict) else "medium",
            },
            "specialization_instructions": result.get("specialization_instructions") if isinstance(result, dict) else None,
            "enhanced_task": result.get("enhanced_task", "") if isinstance(result, dict) else raw,
            "approach": result.get("approach", []) if isinstance(result, dict) else [],
            "prerequisites": result.get("prerequisites", []) if isinstance(result, dict) else [],
            "suggested_libs": result.get("suggested_libs", []) if isinstance(result, dict) else [],
        }

        logger.info("[ENHANCEMENT] Refinação finalizada. Escopo e abordagens injetados com sucesso.")
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[ENHANCEMENT] Falha crítica no pipeline do Enhancement Node: %s", e)
