...
async def run_multi_agent(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        context = memory.get("prompt_builder", {}).get("output", "") or memory.get(
            "prompt_builder", {}
        ).get("built_prompt", "")
        plan = memory.get("planner", {}).get("output", {})

        logger.info(
            "[MULTI_AGENT] Estruturando plano e prompts de delegação para o grafo..."
        )
        ...
        logger.info(
            "[MULTI_AGENT] Delegação preparada com sucesso! Prompt estruturado: %d caracteres.",
            len(delegation_prompt),
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custo de inferência estimado para estruturação do prompt
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[MULTI_AGENT] Falha crítica no pipeline do Multi-Agent Node: %s", e
        )
