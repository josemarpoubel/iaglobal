...
async def run_critic(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bus is not None and inbox is not None:
        ...
        if msgs:
            for msg in msgs:
                logger.info(
                    "[CRITIC] Mensagem recebida de %s: type=%s", msg.sender, msg.type
                )
    ...
    if not coder_output:
        logger.warning(
            "[CRITIC] Nada para avaliar — output dos programadores está vazio"
        )
        ...
        return {
            ...
            "critic": {
                "approved": True,
                "score": 0.0,
                "issues": ["Sem output para avaliar"],
                "fix_suggestions": [],
                "_skip": True,
            },
            "execution_metrics": {
    ...
    try:
        ...
        logger.info(
            "[CRITIC] score=%.1f approved=%s issues=%d", score, approved, len(issues)
        )
        if issues:
            for iss in issues[:3]:
        ...
        if score < 30:
            await asyncio.to_thread(
                record_error,
                "critic",
                f"Baixa qualidade detectada: score={score}",
                {"task": task[:100]},
            )
        ...
        if bus is not None:
            ...
            logger.info(
                "[CRITIC] Notificação 'review_done' injetada no barramento com sucesso."
            )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Custos de inferência do Critic Agent
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[CRITIC] Falha severa durante ciclo analítico do Critic: %s", e
        )
