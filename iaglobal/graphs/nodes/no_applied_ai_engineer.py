...
from iaglobal.evolution.skills.skill_executor import SkillExecutor
...
import iaglobal.evolution.skills.skill_model_router  # noqa: F401
import iaglobal.evolution.skills.skill_rag_optimizer  # noqa: F401
import iaglobal.evolution.skills.skill_prompt_structurer  # noqa: F401
...
async def run_applied_ai_engineer(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ivm < IVM_THRESHOLD:
        logger.warning("[APPLIED_AI] IVM=%.2f abaixo do threshold %.2f — rejeitando", ivm, IVM_THRESHOLD)
        return {
            "output": {"status": "REJECTED", "reason": f"IVM {ivm:.2f} abaixo do threshold {IVM_THRESHOLD}"},
            "execution_metrics": {
    ...
    try:
        for skill_name in SKILL_SEQUENCE:
            if not executor.can_execute(skill_name):
                logger.warning("[APPLIED_AI] Skill '%s' não disponível — marcando como vazia", skill_name)
                continue
        ...
        if isinstance(structured_prompt, dict):
            prompt_text = structured_prompt.get("user_prompt", "") or structured_prompt.get("system_prompt", "")

        if prompt_text:
            scan = pathogen_analyzer.analyze_code(prompt_text, context="applied_ai_engineer")
            if scan.get("is_pathogen"):
                logger.warning("[APPLIED_AI] Patógeno detectado no prompt gerado: %s", scan.get("threats"))
        ...
        logger.info("[APPLIED_AI] Ciclo concluído: modelo=%s ivm=%.2f latência=%.0fms", selected_model, ivm, latency_ms)
