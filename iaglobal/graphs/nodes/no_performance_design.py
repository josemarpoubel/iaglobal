...
async def run_performance_design(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    design_context = {
        ...
        or memory.get("architecture", {}).get("output", {}),
    ...
    logger.info(
        "[PERF_DESIGN] Iniciando modelagem e validação cruzada de performance de sistemas..."
    )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(agent.analyze):
            result = await agent.analyze(
                design_context=design_context, knowledge_context="", error_context=""
            )
        else:
        ...
        logger.info(
            "[PERF_DESIGN] Modelagem concluída com sucesso. Score de eficiência: %s",
            score,
        )
        ...
        return {
            ...
            "execution_metrics": {
                ...
                ),  # Desenhos complexos de arquitetura gastam mais tokens
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[PERF_DESIGN] Falha crítica no pipeline do Performance Design Node: %s", e
        )
