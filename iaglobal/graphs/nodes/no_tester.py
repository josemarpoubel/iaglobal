...
async def run_tester(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not code:
        logger.warning(
            "[TESTER] Nenhum código encontrado nas memórias para geração de testes."
        )
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(agent.gerar_testes):
            test_output = await agent.gerar_testes(
                codigo=code, task=task_str, contexto=contexto_busca
            )
        else:
            test_output = await asyncio.to_thread(
                agent.gerar_testes, codigo=code, task=task_str
            )

        test_code = (
            test_output.test_code
            if hasattr(test_output, "test_code")
            else (test_output or "")
        )
        test_len = len(test_code)
        logger.info(
            "[TESTER] Ciclo finalizado com sucesso! Gerados %d caracteres de testes.",
            test_len,
        )

        is_success = (
            test_output.success if hasattr(test_output, "success") else (test_len > 10)
        )
        latency_ms = (time.time() - start_time) * 1000.0
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "cost": ctx.get(
                    "estimated_cost", 0.01
                ),  # Custo de inferência estimado para geração de testes
            },
        }
