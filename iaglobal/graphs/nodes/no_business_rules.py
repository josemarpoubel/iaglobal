...
async def run_business_rules(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not rules:
            rules = ["RN-0: O sistema deve cumprir integralmente o escopo e restrições do requisito principal."]

        logger.info("[BUSINESS] Sucesso! Mapeadas %d regras de negócio essenciais.", len(rules))
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[BUSINESS] Falha crítica no pipeline do Business Rules Node: %s", e)
