...
async def run_prompt_improver(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not domain:
        ...
        if isinstance(prompt_data, dict) and isinstance(
            prompt_data.get("prompt"), dict
        ):
    ...
    if not raw_prompt:
        ...
        if isinstance(prompt_data, dict):
            raw_prompt = prompt_data.get("prompt", {}).get(
                "normalized", ""
            ) or prompt_data.get("output", "")
    ...
    knowledge_context = (
        knowledge_data.get("summary", "") if isinstance(knowledge_data, dict) else ""
    )

    logger.info(
        "[PROMPT_IMPROVER] Ativando motor de otimização de 5 estágios de forma não-bloqueante..."
    )
    ...
    try:
        ...
        detected_domains = (
            ...
            if hasattr(report, "detected_domains")
            else []
        )
        constraints = (
            report.constraints_applied if hasattr(report, "constraints_applied") else 0
        )

        logger.info(
            "[PROMPT_IMPROVER] Sucesso! %d→%d chars | mod=%s | domínios=%s | restrições=%d",
            report.original_length
            if hasattr(report, "original_length")
            else len(raw_prompt),
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.warning(
            "[PROMPT_IMPROVER] Falha no motor de otimização: %s. Utilizando prompt bruto.",
            e,
        )
