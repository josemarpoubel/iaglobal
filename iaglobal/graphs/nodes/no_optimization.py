...
async def run_optimization(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[OPTIMIZATION] Iniciando ciclo analítico de otimização de performance e recursos...")
    
    # Coleta o contexto de forma resiliente das memórias anteriores
    design_context = ctx.get("design_context") or ctx.get("memory", {}).get("architecture", ctx)
    ...
    try:
        ...
        logger.info("[OPTIMIZATION] Análise e modelagem de otimizações concluídas com sucesso.")
        ...
        return {
            "output": result.get("optimization_report", result) if isinstance(result, dict) else str(result),
            "optimization": result,
            "execution_metrics": {
                "model": resolved_model,
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.004)  # Custo de inferência estimado para o relatório de otimização
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[OPTIMIZATION] Falha crítica no pipeline do Optimization Node: %s", e)
