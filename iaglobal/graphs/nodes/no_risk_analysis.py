...
async def run_risk_analysis(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[RISK_ANALYSIS] Iniciando ciclo de análise e mitigação de riscos técnicos..."
    )
    ...
    try:
        ...
        return {
            "output": result.get("output", "Análise de riscos concluída")
            ...
            "execution_metrics": {
                ...
                ),  # Processamento analítico consome tokens
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[RISK_ANALYSIS] Falha crítica no pipeline do Risk Analysis Node: %s", e
        )
