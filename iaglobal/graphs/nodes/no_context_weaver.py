...
async def run_context_weaver(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if prompt_intake_domain == "web" or any(kw in task_lower for kw in ["pagina", "site", "landing", "email", "newsletter", "dark", "escuro", "html", "css", "frontend"]):
        domain_markers.append("web:responsive")
    if "mobile" in prompt_intake_domain or any(kw in task_lower for kw in ["mobile", "android", "ios", "app"]):
        domain_markers.append("mobile:first")
    
    # Domínios financeiros - prioriza domain detectado
    if prompt_intake_domain == "financeiro" or any(kw in task_lower for kw in ["mercado", "financeiro", "acao", "bolsa", "investimento", "dolar"]):
    ...
    epigenetic_context = f"[EPIGENETIC_MARKERS: {', '.join(domain_markers)}]" if domain_markers else ""
    
    logger.info("[CONTEXT_WEAVER] Domain=%s | Contexto epigenético injetado: %s", prompt_intake_domain, epigenetic_context)
