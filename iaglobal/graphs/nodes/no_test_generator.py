...
async def run_test_generator(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[TEST_GENERATOR] Iniciando geração automatizada de suíte de testes unitários...")
    ...
    try:
        ...
        if is_success:
            ...
            logger.warning("[TEST_GENERATOR] Geração retornou uma suíte de testes vazia ou curta demais.")
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "success": is_success,
                "latency": latency_ms,
                "cost": ctx.get("estimated_cost", 0.010)  # Geração de código de teste consome tokens de inferência
            }
        }

    except Exception as e:
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[TEST_GENERATOR] Falha crítica no pipeline do Test Generator Node: %s", e)
