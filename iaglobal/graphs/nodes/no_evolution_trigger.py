...
def _extract_metrics(memory: Dict[str, Any]) -> Dict[str, Any]:
    raw_metrics = memory.get("metrics", {}) if isinstance(memory.get("metrics", {}), dict) else {}
    raw_output = raw_metrics.get("output", {}) if isinstance(raw_metrics.get("output", {}), dict) else {}

    evaluator_out = memory.get("evaluator", {}).get("output", {})
    if not isinstance(evaluator_out, dict):
        evaluator_out = {}
    evaluator_score = evaluator_out.get("score", 0) if isinstance(evaluator_out, dict) else 0
...
async def run_evolution_trigger(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info("[EVOLUTION_TRIGGER] Analisando telemetria acumulada para tomada de decisão evolutiva...")
    ...
    try:
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception("[EVOLUTION_TRIGGER] Falha crítica no pipeline do Evolution Trigger Node: %s", e)
        
        return {
            "output": {"should_evolve": False, "reason": f"Trigger agent failure: {str(e)}"},
