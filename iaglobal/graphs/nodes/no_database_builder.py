...
async def run_database_builder(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[DATABASE_BUILDER] Iniciando geração da camada de persistência e schemas..."
    )
    ...
    if architecture_data:
        ...
        for comp in components:
            if comp.get("name") == "database-layer":
                ...
                break
        contexto_refinado = (
            f"database schema and script design context strictly using: {db_tech}"
        )
    ...
    try:
        ...
        if is_success:
            logger.info(
                "[DATABASE_BUILDER] Scripts e schemas de banco de dados gerados: %d caracteres.",
                len(code_output),
            )
        else:
            logger.warning(
                "[DATABASE_BUILDER] Geração retornou código de banco de dados vazio."
            )
        ...
        return {
            "output": code_output,
            "database_builder": {"output": code_output, "files": files_output},
            "execution_metrics": {
                ...
                ),  # Inferência focada em schemas estruturados
            },
        }
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[DATABASE_BUILDER] Falha crítica no pipeline do Database Builder Agent: %s",
            e,
        )
