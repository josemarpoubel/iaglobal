...
async def run_ingestion(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        "[INGESTION] Iniciando varredura de caminhos de arquivos no escopo da task..."
    )
    ...
    try:
        ...
        detected_paths = await asyncio.to_thread(
            FileIngestionAgent.detect_file_paths, task
        )

        if not detected_paths:
            logger.info(
                "[INGESTION] Nenhum caminho de arquivo detectado na task. Ciclo pulado."
            )
        ...
        logger.info(
            "[INGESTION] Detectados %d caminhos em potencial. Iniciando leitura de disco...",
            len(detected_paths),
        )
        ...
        logger.info(
            "[INGESTION] Sucesso! %d arquivos ingeridos com segurança (%d erros de leitura).",
            file_count,
            error_count,
        )
        ...
        latency_ms = (time.time() - start_time) * 1000.0
        logger.exception(
            "[INGESTION] Falha crítica no pipeline do Ingestion Node: %s", e
        )
