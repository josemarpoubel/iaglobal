...
async def _register_lineage_obsidian(
    ...
    try:
        ...
        note_id = await subconscious.escrever_longo_prazo(
            ...
            note_content,
            tags=["#fusion", "#lineage", "#hybrid", f"#gen-{fusion_engine._agent_dnas[hybrid_id].generation if hybrid_id in fusion_engine._agent_dnas else '0'}"]
        )
...
async def run_fusion(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("[FUSION] Orientação OmniMind: %s", orientacao.guidance[:100] if orientacao else "N/A")
    except Exception as e:
    ...
    if len(candidates) < 2:
        logger.warning("[FUSION] Poucos candidatos (%d). Fusão não executada.", len(candidates))
        return {
            "fusion_result": None,
    ...
    logger.info("[FUSION] %d pares compatíveis encontrados. Melhor ressonância: %.2f", len(pairs), pairs[0][2])
    ...
    logger.info("[FUSION] Executando fusão: %s + %s → %s", agent_a, agent_b, hybrid_name)
    ...
    if fusion_result.success:
        ...
        logger.info(
            ...
            hybrid_name, resonance, fusion_result.viability_score,
        )
    else:
        logger.warning(
