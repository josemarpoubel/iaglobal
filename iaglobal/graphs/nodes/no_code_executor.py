...
async def run_code_executor(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuição_exec = await subconscious.sussurrar_intuicao(
        ["#execution", "#sandbox", "#static"]
    )
    logger.info(
        "[EXECUTOR] Intuição do subconsciente: %s",
        intuição_exec[:100] if intuição_exec else "vazio",
    )
    ...
    raw_code = (
        memory.get("frontend_builder", {}).get("output", "")
        or memory.get("frontend_builder", {}).get("output", "")
    ...
    logger.info(
        "[EXECUTOR] %d chars, ext=%s, fence=%s", len(cleaned), ext, fence_lang or "-"
    )
    ...
    try:
        ...
        if ext in _STATIC_EXTS:
            ...
            project_dir = await asyncio.to_thread(
                save_result_artifact, task=task, files={}, code=output_text
            )
            if project_dir:
                ...
                saved = str(out_path)
            logger.info(
                "[EXECUTOR] Arquivo estático salvo de forma assíncrona: %s", saved
            )
        ...
        # --- 2. PYTHON / SANDBOX ---
            ...
            if output_text or not error:
                ...
                if project_dir:
                    out_path = Path(project_dir) / (
                        "output.pdf" if pdf_found and not error else f"output{ext}"
                    )
