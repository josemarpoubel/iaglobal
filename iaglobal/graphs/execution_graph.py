    ...
    async def async_run(
        ...
        while len(executed) < len(self.nodes):
            ...
            for name, node in self.nodes.items():
                if name in executed or name in self.results: continue
                if not all(dep in executed or dep in self.results for dep in node.depends_on): continue
            ...
            if not ready_nodes:
                ...
                if len(processed) >= len(self.nodes):
                    logger.info("[ASYNC] Grafo finalizado: %d nós processados (%d executados, %d em results)", len(processed), len(executed), len(self.results))
                    break
                logger.warning("[ASYNC] DEADLOCK SUSPEITO — pendentes: %s | executados: %d/%d", [n for n in self.nodes if n not in processed], len(executed), len(self.nodes))
            ...
            for (name, node, node_id), result in zip(ready_nodes, results):
                ...
                is_failed = is_exception or (isinstance(result, dict) and not result.get("success"))
