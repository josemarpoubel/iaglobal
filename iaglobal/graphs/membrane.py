...
class Membrane:
    ...
    def send(self, message: MembraneMessage) -> Optional[Any]:
        ...
        if (
            message.source.value not in CORE_ORGANELLES
            and message.target.value in CORE_ORGANELLES
        ):
            if message.event_type not in ("query", "read"):
                logger.warning(
                    "[MEMBRANE] BLOQUEADO: '%s' tentou modificar core '%s'",
        ...
        if not handler:
            logger.warning(
                "[MEMBRANE] Organela '%s' sem handler registrado", message.target.value
            )
            return None
        try:
            ...
            logger.error(
                "[MEMBRANE] Falha em '%s' — isolada (erro: %s)", message.target.value, e
            )
            return None
