...
class Nodes:
    ...
    def __new__(cls, logger_instance=None):
        if cls._instance is None:
            ...
            cls._instance.logger.info("🧠 Nodes Pipeline Director Singleton initialized via dynamic loading")
    ...
    def _load_dynamic_nodes(self):
        ...
        for filename in sorted(os.listdir(nodes_folder)):
            ...
            if filename.endswith(".py") and filename != "__init__.py" and not filename.startswith("_disk"):
                        ...
                        for attr_name in dir(module):
                            ...
                            if attr_name.startswith("run_"):
                                 ...
                                 if callable(func):
                                     ...
                                     from iaglobal.core.law_enforcement import enforce_universal_laws
                                ...
                                if isinstance(cls_target, type):
                                    for sub_attr in dir(cls_target):
                                        if sub_attr.startswith("run_"):
                                            ...
                                            setattr(self, sub_attr, functools.partial(func))
                                            loaded_count += 1
...
def create_skill_node(name: str, depends_on: Optional[List[str]] = None) -> Any:
    ...
    try:
        ...
        if run_fn is None:
            logger.warning("create_skill_node: run_%s não encontrada em no_%s.py", name, name)
