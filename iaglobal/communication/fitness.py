...
class ColonyFitness:
    ...
    def register_organism(self, organism_id: str):
        if organism_id not in self._organisms:
            self._organisms[organism_id] = OrganismMetrics(organism_id=organism_id, last_seen=time.time())
    ...
    async def check_apoptosis(self, organism_id: str) -> Optional[dict]:
        ...
        if metrics.ivm < IVM_CRITICO:
            logger.warning("[FITNESS] Apoptose: %s IVM=%.4f < %.2f",
                           organism_id, metrics.ivm, IVM_CRITICO)
    ...
    async def check_mitosis(self, organism_id: str, specialization: str = "") -> Optional[str]:
        """Gera novo organismo a partir de um saudável (IVM ≥ 0.7)."""
        metrics = self._organisms.get(organism_id)
        if not metrics or metrics.ivm < IVM_EXCELENTE:
            return None

        child_id = f"{organism_id}-{specialization or 'child'}-{int(time.time()) % 1000}"
        ...
        logger.info("[FITNESS] Mitose: %s → %s (IVM pai=%.4f)",
                    organism_id, child_id, metrics.ivm)
        return child_id
    ...
    async def _extract_lessons(self, organism_id: str) -> dict:
        ...
        return {
            "organism_id": organism_id,
            "total_tasks": (metrics.tasks_completed + metrics.tasks_failed) if metrics else 0,
