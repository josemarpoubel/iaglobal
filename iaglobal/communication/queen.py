...
class ColonyQueen:
    ...
    def __init__(self, organism_id: str = "queen", message_bus: Optional[AcetylcholineBus] = None):
    ...
    async def decompose(self, super_task: dict) -> list[dict]:
        ...
        if task_type == "analysis":
            subtasks = [
                {"id": _uid(), "type": "search", "description": f"Coletar dados: {description}", "parent": super_task.get("id")},
                {"id": _uid(), "type": "code_review", "description": f"Revisar código: {description}", "parent": super_task.get("id")},
                {"id": _uid(), "type": "report", "description": f"Gerar relatório: {description}", "parent": super_task.get("id")},
            ]
        elif task_type == "development":
            subtasks = [
                {"id": _uid(), "type": "planner", "description": f"Planejar: {description}", "parent": super_task.get("id")},
                {"id": _uid(), "type": "coder", "description": f"Codificar: {description}", "parent": super_task.get("id")},
                {"id": _uid(), "type": "tester", "description": f"Testar: {description}", "parent": super_task.get("id")},
                {"id": _uid(), "type": "documenter", "description": f"Documentar: {description}", "parent": super_task.get("id")},
            ]
        else:
            subtasks.append({
                "id": _uid(),
    ...
    async def on_result_share(self, message: AgentMessage):
        ...
        if parent in self._completed:
            ...
            logger.info("[QUEEN] result_share recebido: %s → %s (%d/%d)",
                        task_id, parent,
