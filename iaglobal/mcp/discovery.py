...
CACHE_PATH = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "mcp_tools.json"
...
class MCPDiscovery:
        ...
        for server in servers:
            ...
            try:
                if server.get("type") == "stdio":
                    await client.connect_stdio(server["command"], server.get("args", []))
                ...
                logger.warning("Falha ao descobrir tools em %s: %s", server.get("name"), e)

        return all_tools

    def _load_server_registry(self) -> list[dict[str, Any]]:
        registry_path = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "mcp_servers.json"
    ...
    def _persist_cache(self, data: dict[str, Any]):
        try:
            ...
            logger.info("Cache MCP salvo: %s (%d tools)", self._cache_path, len(data["tools"]))
        except Exception as e:
