...
class MetaLearner:
    ...
    async def analyze_execution(self, 
        ...
        logger.debug(f"🧠 [META] Execução analisada: {task_type} {'✅' if success else '❌'}")
    
    def _find_or_create_pattern(self, pattern_key: str, task_type: str, success: bool,
                                metrics: Dict, config: Dict, provider: str, ivm: float) -> ExecutionPattern:
        """Encontra ou cria padrão de execução."""
        existing = self.success_patterns.get(pattern_key) or self.failure_patterns.get(pattern_key)
        ...
        if existing:
            ...
            return ExecutionPattern(
                pattern_id=pattern_key, task_type=task_type, success=success,
                avg_latency_ms=metrics.get('latency_ms', 0), avg_ivm=ivm,
                provider_used=provider, config_used=config, occurrence_count=1,
            )
    
    async def _analyze_for_insights(self, task_type: str, success: bool,
                                   metrics: Dict, config: Dict, ivm: float):
        """Analisa execução em busca de insights."""
        if not success:
            failure_count = sum(p.occurrence_count for k, p in self.failure_patterns.items() if task_type in k)
            if failure_count >= 5:
                await self._generate_suggestion(
                    ...
                    current_state={"task_type": task_type, "failure_count": failure_count},
    ...
    async def _generate_suggestion(self, category: str, priority: str, description: str,
                                  current_state: Dict, suggested_state: Dict,
                                  expected_improvement: str, confidence: float):
        ...
        suggestion = ArchitecturalSuggestion(
            suggestion_id=suggestion_id, category=category, priority=priority,
            description=description, current_state=current_state,
            suggested_state=suggested_state, expected_improvement=expected_improvement,
            confidence=confidence,
        )
        self.suggestions[suggestion_id] = suggestion
        logger.info(f"🧠 [META] Nova sugestão: {suggestion_id} ({category}, {priority})")
    ...
    async def _add_to_backlog(self, item_type: str, description: str, rationale: str,
                             priority_score: float, estimated_impact: str):
        """Adiciona item ao backlog evolutivo."""
        item_id = f"{item_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        item = EvolutionBacklogItem(
            item_id=item_id, item_type=item_type, description=description,
            rationale=rationale, priority_score=priority_score,
    ...
    def _auto_tune_configs(self):
        ...
        if success_rate < 0.7:
            self.auto_tuned_configs["bandit_epsilon"] = min(0.3, 
                self.auto_tuned_configs["bandit_epsilon"] + 0.02)
            logger.info(f"🎛️ [META] Aumentando epsilon para {self.auto_tuned_configs['bandit_epsilon']:.3f}")
        elif success_rate > 0.9 and avg_ivm > 0.8:
            self.auto_tuned_configs["bandit_epsilon"] = max(0.05,
                self.auto_tuned_configs["bandit_epsilon"] - 0.01)
            logger.info(f"🎛️ [META] Reduzindo epsilon para {self.auto_tuned_configs['bandit_epsilon']:.3f}")
        
        if avg_ivm > 0.85:
            self.auto_tuned_configs["mitosis_threshold"] = max(0.6,
                self.auto_tuned_configs["mitosis_threshold"] - 0.05)
    
    def get_suggestions(self, category: Optional[str] = None, min_confidence: float = 0.5) -> List[ArchitecturalSuggestion]:
    ...
    def get_backlog(self, priority_min: float = 0.5, status: str = "backlog") -> List[EvolutionBacklogItem]:
        """Retorna backlog evolutivo priorizado."""
        items = [i for i in self.evolution_backlog.values() 
    ...
    async def _save_state(self):
        ...
        state = {
            ...
            "success_patterns": {k: asdict(v) for k, v in self.success_patterns.items()},
            ...
            "evolution_backlog": {k: asdict(v) for k, v in self.evolution_backlog.items()},
