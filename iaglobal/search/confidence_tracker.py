...
class ConfidenceTracker:
    ...
    def should_search(
        self, agent_id: str, task_hash: str, threshold: Optional[float] = None
    ) -> bool:
        """
    ...
    def record_confidence(
        ...
        task_hash: str,
        confidence: float,
        search_helped: Optional[bool] = None,
    ):
        """
    ...
    def record_search_outcome(self, agent_id: str, task_hash: str, helped: bool):
        ...
        if key in self._confidence_cache:
            ...
            if helped:
                ...
                logger.info(
                    "[CONF] %s: busca ajudou → confidence=%.2f",
                    agent_id,
                    conf.confidence,
                )
            else:
                ...
                logger.info(
                    "[CONF] %s: busca atrapalhou → confidence=%.2f",
                    agent_id,
                    conf.confidence,
                )
    ...
    def get_stats(self, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if agent_id:
            entries = [
                c for c in self._confidence_cache.values() if c.agent_id == agent_id
            ]
        else:
        ...
        return {
            ...
            "search_success_rate": round(
                search_helped_count / max(1, search_helped_count + search_hurt_count), 3
            ),
        }
    ...
    def _load(self):
        ...
        if self._persistence_file.exists():
            try:
                ...
                logger.info(
                    "[CONF] %d confianças carregadas", len(self._confidence_cache)
                )
    ...
    def clear(self, agent_id: Optional[str] = None):
        ...
        if agent_id:
            self._confidence_cache = {
                k: v
                for k, v in self._confidence_cache.items()
...
# Funções utilitárias
def should_search(
    agent_id: str, task_hash: str, threshold: Optional[float] = None
) -> bool:
...
def record_confidence(
    ...
    get_confidence_tracker().record_confidence(
        agent_id, task_hash, confidence, search_helped
    )
