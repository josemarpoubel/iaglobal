...
class SourceValidator:
    ...
    def validate(self, snippet: dict, all_snippets: Optional[List[dict]] = None) -> SourceScore:
        """
        ...
        # 2. Score de recência
        recency = self._score_recency(date_str) if date_str else 0.5  # Neutro se sem data
        ...
        logger.debug(
            ...
            domain, credibility, recency, consistency, overall
        )
    ...
    def _score_consistency(self, snippet: dict, all_snippets: List[dict]) -> float:
        ...
        snippet_text = (snippet.get("title", "") + " " + snippet.get("snippet", "")).lower()
        ...
        for other in all_snippets:
            ...
            other_text = (other.get("title", "") + " " + other.get("snippet", "")).lower()
    ...
    def _extract_keywords(self, text: str) -> set:
        ...
        # Remover stopwords
        stopwords = {
            "a", "o", "as", "os", "um", "uma", "de", "do", "da", "em", "para",
            "com", "por", "the", "a", "an", "and", "or", "in", "on", "for",
    ...
    def _has_contradiction(self, text1: str, text2: str) -> bool:
        ...
        return has_contradiction_1 != has_contradiction_2  # XOR: apenas um tem contradição

    def filter_by_score(self, snippets: List[dict], min_score: Optional[float] = None) -> List[dict]:
        """
        ...
        logger.info(
            ...
            len(validated), len(snippets), threshold
        )
...
# Funções utilitárias
def validate_source(snippet: dict, all_snippets: Optional[List[dict]] = None) -> SourceScore:
