...
class SearchMemory:
    """Singleton de persistência de buscas no Obsidian."""

    # Caminho absoluto para a pasta search_memory
    OBSIDIAN_PATH = (
        Path(__file__).parent.parent / "obsidian" / "04_Synapses" / "search_memory"
    )
    ...
    def _load_index(self):
        ...
        try:
            ...
            logger.info(
                "[SEARCH_MEMORY] Carregadas %d buscas do Obsidian", len(self._cache)
            )
    ...
    def _save_index(self):
        ...
        try:
            ...
            logger.debug(
                "[SEARCH_MEMORY] Índice salvo (%d registros)", len(self._cache)
            )
