...
class LocalSummarizer:
    ...
    _NOISE_PREFIXES = [
        "você é", "você deve", "você precisa", "seu papel", "sua tarefa",
        "retorne apenas", "não inclua", "não explique", "responda apenas",
        "atuando como", "como um especialista", "considerando que",
        "formato de saída", "saída esperada", "exemplo de saída",
        "use o contexto", "com base nas informações", "analise o",
        "you are", "you must", "you need to", "your role", "your task",
        "return only", "do not include", "do not explain",
        "acting as", "as an expert", "considering that",
        "output format", "expected output", "example output",
        "use the context", "based on the information",
    ]

    # Assinaturas multi-linguagem — cada padrão captura {lang, tipo, nome}
    _SIG_PATTERNS = [
        # Python
        (r"(?:async\s+)?def\s+(\w+)\s*\(.*?\)\s*(?:->\s*\w+(?:\[.*?\])?)?\s*:", "python", "def"),
        (r"class\s+(\w+)\s*\(?.*?\)?\s*:", "python", "class"),
        (r"@\w+(?:\.\w+)?\(.*?\)", "python", "decorator"),

        # JavaScript / TypeScript
        (r"(?:export\s+)?(?:async\s+)?function\s+(\w+)\s*\([^)]*\)", "js", "function"),
        (r"(?:export\s+)?(?:async\s+)?\(?\s*(?:\w+\s*:\s*\w+\s*,?\s*)*\s*\)?\s*=>", "js", "arrow"),
        (r"(?:export\s+)?class\s+(\w+)(?:\s+extends\s+\w+)?(?:\s+implements\s+\w+(?:,\s*\w+)*)?", "js", "class"),
        (r"(?:export\s+)?interface\s+(\w+)(?:\s+extends\s+\w+(?:,\s*\w+)*)?", "ts", "interface"),
        ...
        # Java
        (r"(?:public|private|protected)\s+(?:static\s+)?(?:final\s+)?(?:\w+(?:<.*?>)?\s+)?(\w+)\s*\([^)]*\)\s*(?:\{|throws)", "java", "method"),
        (r"(?:public|private|protected)\s+(?:static\s+)?(?:abstract\s+)?class\s+(\w+)", "java", "class"),
        (r"(?:public|private|protected)\s+interface\s+(\w+)", "java", "interface"),

        # SQL
        (r"(?:CREATE|ALTER|DROP)\s+(?:TABLE|VIEW|INDEX|PROCEDURE|FUNCTION|TRIGGER)\s+(\w+)", "sql", "ddl"),
        (r"SELECT\s+.*?\s+FROM\s+(\w+)", "sql", "query"),

        # C / C++
        (r"(?:int|void|char|float|double|long|short|unsigned|size_t|bool|FILE|struct\s+\w+|\w+_t)\s+(\w+)\s*\([^)]*\)\s*(?:\{|;)", "c", "function"),
