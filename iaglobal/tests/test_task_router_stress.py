...
CONGESTION_FILE = Path("/home/kitohamachi/iaglobal-main/iaglobal/memory/data/jol/jol_congestion.jsonl")
...
class TestTaskRouterMapping:
    ...
    def test_critic_nodes_map_to_glm4(self, task_router_instance):
        ...
        critical_nodes = [
            "no_critic", "no_critic_batch", "critic_agent",
            "no_requirements", "no_technology_selection",
            "no_architecture_design", "no_system_design"
        ]
    ...
    def test_coder_nodes_map_to_qwen(self, task_router_instance):
        ...
        router = task_router_instance
        coder_nodes = [
            "no_coder", "coder_agent", "no_debugger", "debugger_agent",
            "no_multi_coder", "no_code_generator", "no_reflexion"
        ]
    ...
    def test_sentinel_nodes_map_to_lfm(self, task_router_instance):
        ...
        router = task_router_instance
        sentinel_nodes = [
            "no_sentinel_audit", "no_auditor_sentinel", "no_entropy_sentinel",
            "no_syntax_sentinel", "no_lsp_validator", "no_dependency",
            "no_test_generator", "tester_agent"
        ]
    ...
    def test_timeout_per_tier(self, task_router_instance):
        ...
        router = task_router_instance
        assert router.get_timeout_for_tier("glm4") == 600.0  # Juiz: 10 min para raciocínio profundo
...
class TestCongestionAlerts:
    ...
    @pytest.mark.asyncio
    async def test_congestion_alert_fired_on_bucket_full(self, token_gate_instance, jol_collector_instance):
        ...
        assert gate._alerts_fired.get("glm4", 0) >= 1, "Congestion alert should be fired for glm4"

    @pytest.mark.asyncio
    async def test_congestion_logged_to_jsonl(self, token_gate_instance, jol_collector_instance):
...
class TestConcurrentLoad:
    ...
    @pytest.mark.asyncio
    async def test_81_concurrent_tasks_distribution(self, task_router_instance, token_gate_instance):
        ...
        logger.info(f"Concurrent load: glm4={glm4_successes}/27, qwen={qwen_successes}/27, lfm={lfm_successes}/27")
        ...
        assert glm4_successes <= 2, f"glm4 should have max 2 successes, got {glm4_successes}"
        assert qwen_successes <= 6, f"qwen should have max 6 successes, got {qwen_successes}"
        assert lfm_successes <= 8, f"lfm should have max 8 successes, got {lfm_successes}"
    ...
    @pytest.mark.asyncio
    async def test_concurrent_load_triggers_alerts(self, token_gate_instance, jol_collector_instance):
        ...
        assert gate._alerts_fired.get("qwen", 0) >= 1, "qwen alert should be fired under concurrent load"
...
class TestHomeostaticDegradation:
    ...
    async def test_gate_tracks_rejections(self, token_gate_instance):
        ...
        assert final_glm4 >= initial_glm4, f"Should track rejections for glm4: {initial_glm4} → {final_glm4}"
...
if __name__ == "__main__":
    ...
    async def run_manual():
        ...
        for node, expected in [("no_critic", 2), ("no_coder", 6), ("no_sentinel_audit", 8)]:
            successes = 0
