...
async def test_set_priority_boost_respeita_teto(cpu_manager):
    ...
    assert budget == BUDGET_ADRENALINA, f"Boost deveria ser limitado a {BUDGET_ADRENALINA}, got {budget}"
...
async def test_reset_budgets_restaura_homeostase(cpu_manager):
    ...
    for aid in agents:
        budget = await cpu_manager.get_cpu_budget(aid)
        assert budget == BUDGET_PADRAO, f"Agente {aid} deveria ter 25% após reset, got {budget}"
...
async def test_boost_preserva_agents_nao_envolvidos(cpu_manager):
    ...
    assert budget_fora < 0.50, f"Agente fora do batch não deveria receber boost, got {budget_fora}"