...
class TestAuditTrail:
    ...
    def test_audit_records_events(self):
        ...
        reg.register("api-a", "http://a:8080")
        data = json.loads(AUDIT_PATH.read_text(encoding="utf-8"))
        events = [e for e in data["events"] if e["service"] == "api-a" and e["event"] == "service_registered"]
        assert len(events) >= 1
    ...
    def test_audit_records_failure(self):
        ...
        data = json.loads(AUDIT_PATH.read_text(encoding="utf-8"))
        failures = [e for e in data["events"] if e["event"] == "failure_detected" and e["service"] == "api-a"]
    ...
    def test_audit_records_recovery(self):
        ...
        data = json.loads(AUDIT_PATH.read_text(encoding="utf-8"))
        recovery = [e for e in data["events"] if e["event"] == "recovery" and e["service"] == "api-a"]
...
class TestPhospholipidE2E:
    ...
    def test_two_services_fallback(self):
        reg = PhospholipidRegistry()
        svc1 = reg.register("provider-alpha", "http://alpha:8080", weight=1.0, capacity=5)
