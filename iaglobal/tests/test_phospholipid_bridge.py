...
from iaglobal.observability.search_bridge import PhospholipidSearchBridge, search_bridge
...
class TestBanditBridge:
    ...
    def test_select_instance_returns_healthy(self):
        ...
        result = _run(b.select_instance("groq"))
        assert result is not None, f"Esperava instância groq, mas select_instance retornou None. healthy={[s.name for s in reg.healthy_services()]}"
        assert "groq" in result.name

    def test_register_provider_instance(self):
        reg = PhospholipidRegistry()
        b = PhospholipidBanditBridge(registry=reg, load_balancer=PhospholipidLoadBalancer(registry=reg))
        svc = b.register_provider_instance("groq-api", "http://groq:8080", weight=0.8, capacity=5)
...
class TestSearchBridge:
    ...
    def test_register_search_service(self):
        reg = PhospholipidRegistry()
        sb = PhospholipidSearchBridge(registry=reg, load_balancer=PhospholipidLoadBalancer(registry=reg))
