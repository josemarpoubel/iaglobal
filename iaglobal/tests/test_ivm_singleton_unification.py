...
def test_agente_via_get_ivm_axiom_popula_pool_observavel(tests_temp_dir: Path):
    ...
    async def _run():
        await writer.atualizar_metricas(
            "coder", tasks_completed=1, total_latency_ms=500.0,
            skills_exchanged=0, mhc_validation_score=0.9,
        )
...
def test_agente_via_get_ivm_axiom_popula_pool_observavel(tests_temp_dir: Path):
    ...
    async def _run():
        ...
        await ivm_writer.atualizar_metricas(
            "system-analysis-evo", tasks_completed=1, total_latency_ms=1200.0,
            skills_exchanged=1, mhc_validation_score=0.9,
        )
