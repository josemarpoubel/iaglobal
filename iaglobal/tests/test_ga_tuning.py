...
class TestBiologicalMemory:
    ...
    def test_persist_biological_memory_creates_ltm_note_if_fitness_high(self):
        ...
        genome_id = f"ga_gen_{gen}"
        sub = SubconsciousAPI()
        genome_path = runner.population.genome_path if hasattr(runner.population, 'genome_path') else None

        ltm_path = sub.long_term_dir / f"{genome_id}.md"
        if runner.step()["best_fitness"] >= 0.7:
            assert ltm_path.exists(), f"LTM note should exist for high-fitness genome at {ltm_path}"
        else:
    ...
    def test_persist_biological_memory_stores_insight(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT agent, task_id, score FROM insights WHERE task_id = ?",
            (genome_id,)
        )
    ...
    def test_persist_biological_memory_registers_epigenetic_success(self):
        ...
        for cbor_file in epi.base_path.glob("*.cbor"):
            try:
                ...
                if data.get("task_hash") == task_hash and data.get("error_type") == "success":
                    found = True
                    break
            except Exception:
                continue
