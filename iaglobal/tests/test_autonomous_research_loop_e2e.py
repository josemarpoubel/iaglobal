...
class TestAutonomousResearchLoopE2E:
    ...
    async def test_full_autonomous_research_loop(self, tmp_path):
        ...
        with patch("iaglobal.agents.ingestion.paper_ingestor.RESEARCH_QUEUE_FILE", queue_file):
            with patch.object(PaperIngestor, "_download_paper", new_callable=AsyncMock) as mock_download:
        ...
        for hyp in hypotheses:
            with patch.object(runner, "_generate_code", new_callable=AsyncMock) as mock_gen:
                ...
                with patch.object(runner, "_execute_in_sandbox", new_callable=AsyncMock) as mock_exec:
        ...
        with patch.object(consolidator, "_write_to_obsidian", new_callable=AsyncMock) as mock_obsidian:
            ...
            with patch.object(consolidator, "_save_consolidated_json") as mock_save:
                consolidated = await consolidator.consolidate(metadata, results, obsidian_enabled=False)
...
class TestAutonomousResearchLoopWithFailures:
    ...
    async def test_pipeline_with_mixed_results(self, tmp_path):
        ...
        hypotheses = [
            Hypothesis(id="H1", description="Claim 1", method="experiment", expected_outcome="Result", success_criteria="x > 0.5"),
            Hypothesis(id="H2", description="Claim 2", method="experiment", expected_outcome="Result", success_criteria="y > 0.5"),
            Hypothesis(id="H3", description="Claim 3", method="experiment", expected_outcome="Result", success_criteria="z > 0.5"),
        ]
        
        results = [
            ExperimentResult(hypothesis_id="H1", paper_id="2401.99999", success=True, confidence=0.85, execution_time_ms=100, stdout="Success", stderr="", metrics={}, code="", validation_details="OK"),
            ExperimentResult(hypothesis_id="H2", paper_id="2401.99999", success=False, confidence=0.3, execution_time_ms=50, stdout="Failed", stderr="", metrics={}, code="", validation_details="Failed"),
            ExperimentResult(hypothesis_id="H3", paper_id="2401.99999", success=False, confidence=0.2, execution_time_ms=60, stdout="Failed", stderr="", metrics={}, code="", validation_details="Failed"),
        ]
...
class TestPipelineIntegration:
    ...
    def test_lineage_marker_present(self):
        ...
        for module_name in modules:
            module_path = Path(__file__).parent.parent / "iaglobal" / "agents" / "ingestion" / f"{module_name}.py"
            content = module_path.read_text(encoding="utf-8")
            
            assert "# 🧬 LINEAGE_MARKER:" in content, f"{module_name}.py não tem LINEAGE_MARKER"
