...
def test_structured_e2e_summary_emitted():
    ...
    summary["membrane_confined_count"] = sum(
        1 for d in decisions if d["action"] == "confined_local"
    )
    summary["membrane_authorized_count"] = sum(
        1 for d in decisions if d["action"] == "authorized_cloud"
    )
    ...
    assert loaded["membrane_decisions_structured"] == len(_NON_CRITIC_NODES) + len(
        _CRITIC_NODES
    )
...
def test_live_iaglobal_run_membrane_logs():
    ...
    try:
        out_coder = asyncio.run(
            pr.async_route_generate_parallel(
                "responda 'ok'", task_type="general", node_id="coder"
            )
        )
        out_critic = asyncio.run(
            pr.async_route_generate_parallel(
                "responda 'ok'", task_type="general", node_id="critic"
            )
        )
    finally:
        logger.removeHandler(handler)
    ...
    assert out_coder and out_coder.strip(), (
        "Live: coder não retornou texto (Ollama indisponível?)"
    )
...
def test_cloud_auth_failure_degrades_gracefully(monkeypatch):
    ...
    out = asyncio.run(
        pr.async_route_generate_parallel(
            "auth-fail-probe-xyz", task_type="general", node_id="critic"
        )
    )
    assert out and out.strip(), (
        "Crítico deveria degradar para Ollama mesmo com nuvem em auth-failure"
    )
    # Sinal estruturado registra a decisão (nível-independente)
    assert any(
        d["node_id"] == "critic" and d["action"] == "authorized_cloud"
...
def test_non_critic_ollama_down_no_cloud_escalation(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        asyncio.run(
            pr.async_route_generate_parallel(
                "ollama-down-probe-xyz", task_type="general", node_id="coder"
            )
        )
