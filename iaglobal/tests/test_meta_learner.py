...
class TestMetaLearner:
    ...
    async def test_load_consolidated_papers(self, tmp_path):
        ...
        for i in range(3):
            paper_data = {
                "paper": {"paper_id": f"2401.{i:03d}", "title": f"Paper {i}", "topics": ["ml"]},
                "summary": {"total_hypotheses": 3, "validated": 2, "fitness_score": 0.67},
            }
    ...
    async def test_generate_meta_analysis_sufficient_papers(self, tmp_path):
        ...
        for i in range(10):
            ...
            paper_data = {
                "paper": {"paper_id": f"2401.{i:03d}", "title": f"Paper {i}", "topics": ["ml", "dl"]},
                "summary": {"total_hypotheses": 3, "validated": validated, "fitness_score": fitness},
            }
            paper_file = learner.consolidated_dir / f"2401_{i:03d}_consolidated.json"
            paper_file.write_text(json.dumps(paper_data, indent=2))
    ...
    async def test_curate_low_fitness_papers(self, tmp_path):
        ...
        for i in range(5):
            ...
            paper_data = {
                "paper": {"paper_id": f"2401.{i:03d}", "title": f"Paper {i}", "topics": ["ml"]},
                "summary": {"total_hypotheses": 3, "validated": int(3*fitness), "fitness_score": fitness},
            }
            paper_file = learner.consolidated_dir / f"2401_{i:03d}_consolidated.json"
            paper_file.write_text(json.dumps(paper_data, indent=2))
    ...
    async def test_generate_recommendations(self, tmp_path):
        ...
        for i in range(5):
            paper_data = {
                "paper": {"paper_id": f"2401.{i:03d}", "title": f"Paper {i}", "topics": ["ml", "dl"]},
                "summary": {"total_hypotheses": 3, "validated": 3, "fitness_score": 0.9},
            }
            paper_file = learner.consolidated_dir / f"2401_{i:03d}_consolidated.json"
            paper_file.write_text(json.dumps(paper_data, indent=2))
    ...
    async def test_trigger_replication(self, tmp_path):
        ...
        paper_data = {
            "paper": {"paper_id": "2401.500", "title": "Mixed Results", "topics": ["ml"]},
    ...
    async def test_generate_cross_citations(self, tmp_path):
        ...
        for pdata in papers_data:
            paper_data = {
                "paper": {"paper_id": pdata["paper_id"], "title": pdata["paper_id"], "topics": pdata["topics"]},
                "summary": {"total_hypotheses": 3, "validated": 2, "fitness_score": 0.67},
            }
            paper_file = learner.consolidated_dir / f"{pdata['paper_id'].replace(':', '_')}_consolidated.json"
            paper_file.write_text(json.dumps(paper_data, indent=2))
...
class TestRunMetaLearning:
    ...
    async def test_run_meta_learning_empty(self, tmp_path):
        ...
        with patch("iaglobal.agents.ingestion.meta_learner.MetaLearner") as mock_learner:
    ...
    async def test_run_meta_learning_full(self, tmp_path):
        ...
        from iaglobal.agents.ingestion.meta_learner import MetaAnalysis, PaperRecommendation
        
        with patch("iaglobal.agents.ingestion.meta_learner.MetaLearner") as mock_learner:
            mock_instance = mock_learner.return_value
            mock_instance.load_consolidated_papers = AsyncMock(return_value=[
                {"paper": {"paper_id": "2401.001", "topics": ["ml"]}, "summary": {"fitness_score": 0.9}},
            ])
            mock_instance.generate_meta_analysis = AsyncMock(return_value=MetaAnalysis(
                total_papers=10, total_hypotheses=30, validated_hypotheses=25,
                overall_fitness=0.83, top_topics=[("ml", 5)],
                low_fitness_papers=[], high_fitness_papers=["2401.001"], replication_candidates=[],
            ))
            mock_instance.curate_low_fitness_papers = AsyncMock(return_value=[])
            mock_instance.generate_recommendations = AsyncMock(return_value=[
                PaperRecommendation("2401.001", "Test", ["ml"], 0.9, "High fitness", []),
            ])
            mock_instance.generate_cross_citations = AsyncMock(return_value={"2401.001": []})
            mock_instance.get_summary = AsyncMock(return_value={"status": "active", "total_papers": 10})
