...
ANCESTRY_TREE = Path("/home/kitohamachi/projeto-iaglobal/iaglobal/memory/data/ancestry_tree.jsonl")
...
async def test_stress_fase31_complete_flow(registered_manager):
    ...
    logger.info("[STRESS] JobTicket criado: %s | latency=%.2fms", ticket.ticket_id, (t1 - t0) * 1000)
    ...
    logger.info(
        ...
        len(planner_mb.inbox), (t3 - t2) * 1000,
    )

    if planner_mb.inbox:
        msg = planner_mb.inbox[0]
        assert "metadata" in msg, "JobTicket sem metadata!"
        assert "lineage_signature" in msg["metadata"], "JobTicket sem lineage_signature!"
        genesis_marker = msg["metadata"].get("genesis_marker", "")
        assert genesis_marker == GENESIS_HASH_OFFICIAL[:16], f"Genesis marker inválido: {genesis_marker}"
    ...
    report["metrics"] = {
        ...
        "avg_latency_ms": round(sum(agent_latencies.values()) / len(agent_latencies), 2) if agent_latencies else 0.0,
    ...
    logger.info(
        ...
        report["status"], len(bottlenecks), report["metrics"]["total_latency_ms"],
    )
    return report
...
if __name__ == "__main__":
    ...
    async def _stub(ctx: dict, name: str) -> dict:
        ...
        return {
            "output": f"[{name}] Processado",
            "execution_metrics": {"model": "stub_async", "success": True, "latency": 0.5, "cost": 0.0},
        }
