...
from iaglobal.graphs.communication.agent_mailbox import (
    AgentMailbox, MailboxManager, JobTicket, SynapseMonitor,
...
async def test_scenario_a_pathogen(registered_manager, caplog):
    ...
    ancestry_path = Path("/home/kitohamachi/projeto-iaglobal/iaglobal/memory/data/ancestry_tree.jsonl")
    ...
    return {"status": "PASS", "scenario_a": "PASS", "monitor_violations": registered_manager.monitor._violations_detected}
...
async def test_scenario_b_legitimate_job_ticket(registered_manager):
    ...
    assert planner_mailbox._messages_processed >= 0  # Pode falhar se executor não for compatível
...
async def test_scenario_c_observability(registered_manager):
    ...
    required_fields = [
        "agents_count", "total_inbox", "agents", "violations_detected"
    ]
...
@pytest.mark.asyncio
async def test_synapse_full_flow_integration(registered_manager, agents, caplog, tmp_path):
    """
    ...
    report["summary"] = {
        ...
        "passed_count": sum(1 for s in report["scenarios"].values() if "PASS" in str(s)),
        "failed_count": sum(1 for s in report["scenarios"].values() if "FAIL" in str(s)),
    }
...
if __name__ == "__main__":
    ...
    async def run_integration():
        ...
        # Executar teste
        report = await test_synapse_full_flow_integration(manager, agents, None, Path("."))
        return report
    
    report = asyncio.run(run_integration())
    print(f"\n{'='*60}")
    print(f"RELATÓRIO FINAL: {report.get('report_path', 'N/A')}")
    print(f"Resultado: {report['summary']['passed_count']}/{report['summary']['total_scenarios']} cenários passaram")
    print(f"{'='*60}")
