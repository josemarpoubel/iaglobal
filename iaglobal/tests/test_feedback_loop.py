...
class TestFeedbackLoop:
    ...
    async def test_compute_search_effectiveness_mixed(self, feedback):
        ...
        # 3 helped, 1 harmed
        for i in range(3):
            await feedback.record_outcome(f"q{i}", "coder", f"t{i}", FeedbackOutcome.HELPED)
    ...
    async def test_compute_search_effectiveness_by_agent(self, feedback):
        ...
        debugger_effectiveness = feedback.compute_search_effectiveness(agent_id="debugger")
    ...
    def test_clear_cache(self, feedback):
        ...
        import asyncio
        asyncio.run(feedback.record_outcome("q1", "coder", "t1", FeedbackOutcome.HELPED))
...
class TestFeedbackLoopE2E:
    ...
    async def test_multiple_agents_feedback(self, temp_obsidian):
        ...
        # Tester: misto
        await feedback.record_outcome("tester q1", "tester", "task_t1", FeedbackOutcome.HELPED)
        await feedback.record_outcome("tester q2", "tester", "task_t2", FeedbackOutcome.NEUTRAL)
