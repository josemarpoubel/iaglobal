...
def test_runtime_error_is_persisted():
    ...
    logging.getLogger("iaglobal.test_error_persist").error("erro de teste persistido %s", "xyz")

    after = _count_runtime_errors()
    after_files = len(list(ERROR_DIR.glob("*.md")))
    assert after == before + 1, f"errors.json deveria ganhar 1 entrada (antes={before}, depois={after})"
...
async def test_ivm_does_not_credit_cache_hit(tests_temp_dir: Path):
    from iaglobal.chappie.ivm_axiom import get_ivm_axiom, init_ivm_axiom_com_persistencia
    ...
    assert after_cache == 0, f"cache hit NÃO deve incrementar produtividade (got {after_cache})"

    await pr._report_ivm_telemetry(agent, True, 0.5, "ollama", cache_hit=False)
    after_real = axiom._metricas[agent].tasks_completed
    assert after_real == 1, f"geração real DEVE incrementar produtividade (got {after_real})"
...
def _candidate(desc: str, gap: str, score: float = 0.9) -> CandidateSkill:
    return CandidateSkill(
        skill=Skill(name="cand", description=desc, inputs=[], outputs=[],
                    constraints=[], run_fn=None, version="1.0.0", tags=["candidate"]),
        score=score, source_gap=gap,
    )
