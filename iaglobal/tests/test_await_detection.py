...
_SAFE_BUILTINS = {
    "super", "range", "len", "type", "isinstance", "getattr", "hasattr",
    "setattr", "print", "open", "min", "max", "sum", "list", "dict", "set",
    "str", "int", "float", "bool", "repr", "zip", "map", "filter",
    "enumerate", "sorted", "reversed", "any", "all", "issubclass",
    "property", "classmethod", "staticmethod", "object",
    "callable", "iter", "next", "input", "format", "bytes",
    "bytearray", "tuple", "frozenset", "memoryview",
    "abs", "round", "pow", "divmod", "hex", "oct", "bin",
    "ord", "chr", "ascii", "vars", "dir", "locals", "globals",
    "compile", "eval", "exec", "__import__",
}

ASYNC_GATHER_FUNCS = {"create_task", "gather", "wait", "as_completed",
...
def _is_self_call(node: ast.Call) -> bool:
    return isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id == "self"


def _is_async_def_local(call_name: str, file_key: str, file_async_defs: set[str]) -> bool:
...
def test_missing_await_across_codebase():
    ...
    for pyfile in sorted(PACKAGE_DIR.rglob("*.py")):
        ...
        try:
            ...
            violations.append({
                "file": file_key, "line": 0, "func": "<syntax-error>",
                "call": "<syntax-error>", "reason": "syntax_error"
            })
            continue
            ...
            for node in ast.walk(func):
                ...
                if _is_self_call(node):
                    ...
                    if class_name and class_name in class_methods:
                        is_async_def = call_name in class_methods.get(class_name, set()) and \
                                       _is_async_method(call_name, pyfile, class_name)

                if is_async_def and _is_value_discarded(node) and not _is_in_comprehension(node):
                    violations.append({
                        "file": file_key,
                        "line": node.lineno,
                        "func": func.name,
...
def _print_report(violations, total_files, total_async_funcs, total_async_all):
    ...
    print(f"  Funcoes async: {total_async_all} definidas | {total_async_funcs} analisadas")
    print(f"{'='*60}")
