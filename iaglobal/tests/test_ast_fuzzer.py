...
rec(10000)
"""),
    ("list comprehension massiva", "[x for x in range(10000) for y in range(100) for z in range(10)]"),
    ("aninhamento dict/set", "{i: {j: {k: 1 for k in range(10)} for j in range(10)} for i in range(10)}"),
...
def test_fuzzer_report(temp_fuzzer_dir: Path):
    ...
    report = []
    stats = {
        "bloqueados": 0, "sanitizados_ok": 0, "falhas_sanitizacao": 0,
        "permitidos": 0, "nao_detectados": 0, "total_threat": len(THREAT_PAYLOADS),
    }

    for name, payload in THREAT_PAYLOADS + SAFE_PAYLOADS:
        result = _ast_gateway.parse(payload)
        entry = {
            "name": name, "payload_len": len(payload),
            "gateway_valid": result.valid, "gateway_errors": result.errors,
...
@pytest.mark.parametrize("name,payload", [
    (f"[ESTRUTURA] {n}", c) for n, c in ESTRUTURA
])
def test_stress_parse_performance(name: str, payload: str):
    ...
    assert elapsed_ms < 5000, (
        f"STRESS TIMEOUT: {name} levou {elapsed_ms:.0f}ms (>5s)"
    )
