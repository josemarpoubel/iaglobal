...
def test_apoptosis_after_min_executions():
    ...
    chaotic = "sistema sistema sistema sistema falhou repetiu repetiu repetiu repetiu " * 10
    for i in range(35):
    ...
    assert report["last_entropy_score"] > 0, "Entropia deveria ser > 0 para payload caótico"
...
async def test_interceptor_event_published():
    ...
    with patch("iaglobal.graphs.communication.acetylcholine_bus.bus.emit") as mock_emit:
        result = await intercept_execution("test_chaotic", chaotic, {})
