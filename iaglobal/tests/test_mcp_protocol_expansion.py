...
from iaglobal.immunity.glutathione_guardrails import GlutathioneGuardrails, MCP_RATE_LIMITS
...
class TestMCPServer:
    ...
    def test_imports_and_tools_exist(self):
        ...
        for fn in [metabolic_audit, get_ivm, read_file, write_file,
                    list_dir, web_search, web_fetch, execute_code]:
...
class TestFileSystemTool:
    ...
    def test_write_file_allowed(self):
        ...
        ok = _run(FileSystemTool().write_file(
...
class TestToolCallerAgent:
    ...
    def test_tool_caller_read_file_success(self):
        ...
        result = _run(ToolCallerAgent().run({
            "tool_name": "read_file",
...
class TestGlutathioneRateLimit:
    ...
    def test_rate_limit_for_unconfigured_tool(self):
        result = GlutathioneGuardrails.check_mcp_rate_limit("unknown_tool", "test_agent")
        assert result["allowed"] is True
...
class TestAuditTrail:
    ...
    def test_audit_json_exists_and_valid(self):
        path = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "audit.json"
        assert path.exists()
...
class TestMCPToolsJSON:
    ...
    def test_json_schema(self):
        path = Path(PROJECT_ROOT) / "iaglobal" / "memory" / "data" / "json" / "mcp_tools.json"
        assert path.exists()
