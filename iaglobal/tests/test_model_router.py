...
from iaglobal.evolution.skills.skill_model_router import (
    run_model_router,
    CRITICAL_KEYWORDS,
    REASONING_KEYWORDS,
...
async def test_critical_task_elevates_to_cloud():
    ...
    result = await run_model_router({"task": "detect mhc vulnerability in code", "ivm": 0.9})
...
async def test_analysis_low_ivm_elevates_to_cloud():
    ...
    result = await run_model_router({"task": "faça uma analise tecnica do sistema", "ivm": 0.3})
...
async def test_analysis_high_ivm_enters_evaluation():
    ...
    result = await run_model_router({"task": "faça uma analise tecnica do sistema", "ivm": 0.9})
    # Entra na avaliação de score, não no atalho ATP
    assert result["model_decision"]["reason"] in ("reasoning_high_ivm", "local_sufficient")
...
async def test_architecture_low_ivm_elevates():
    ...
    result = await run_model_router({"task": "revise a arquitetura do sistema", "ivm": 0.4})
    assert result["model_decision"]["reason"] == "reasoning_low_ivm_elevation"
...
async def test_refactoring_low_ivm_elevates():
    ...
    result = await run_model_router({"task": "refatore o codigo do modulo X", "ivm": 0.2})
    assert result["model_decision"]["reason"] == "reasoning_low_ivm_elevation"
...
async def test_threats_detected_flag_elevates():
    ...
    result = await run_model_router({"task": "codigo normal", "ivm": 0.9, "threats_detected": True})
