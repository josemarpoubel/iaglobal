...
async def test_sem_homocisteina_durante_falhas_forcadas(monkeypatch):
    ...
    logger.info(
        "[STRESS-TEST] %d falhas forçadas | resultados=%s", FORCED_FAILURES, resultados
    )

    fantasmas = [
        r for r in resultados if r["ok"] and r["latency_ms"] < REAL_INFERENCE_FLOOR_MS
    ]
...
async def test_regeneracao_apos_cooldown(monkeypatch):
    ...
    assert ranked[0][0] == float("-inf"), (
        f"Modelo em cooldown deveria ter score -inf, mas tem {ranked[0][0]}"
    )
    ...
    ranked = bandit.rank_models("test", "general", [MODEL])
    assert ranked[0][0] == 0.8, (
        f"Após regeneração o score deveria voltar a 0.8, mas permanece {ranked[0][0]}"
    )
