...
class TestHypothesisGenerator:
    ...
    async def test_generate_with_mock_llm(self):
        ...
        mock_response = {
            "hypotheses": [
                {"id": "H1", "description": "H1 test", "method": "experiment", "expected_outcome": "Result1", "success_criteria": "metric > 0.9"},
                {"id": "H2", "description": "H2 test", "method": "data_analysis", "expected_outcome": "Result2", "success_criteria": "p < 0.05"},
                {"id": "H3", "description": "H3 test", "method": "simulation", "expected_outcome": "Result3", "success_criteria": "time < 1.0"},
            ]
        }
    ...
    async def test_generate_with_llm_failure_uses_fallback(self):
        ...
        with patch.object(generator, "_call_llm", new_callable=AsyncMock) as mock_call:
            ...
            assert "topic1" in hypotheses[0].description.lower() or "method" in hypotheses[0].description.lower()

    def test_save_hypotheses_creates_json(self, tmp_path):
        """save_hypotheses deve criar arquivo JSON."""
        hypotheses = [
            Hypothesis(id="H1", description="Test", method="experiment", expected_outcome="Result", success_criteria="metric > 0.9"),
            Hypothesis(id="H2", description="Test2", method="data_analysis", expected_outcome="Result2", success_criteria="p < 0.05"),
        ]

        with patch("iaglobal.agents.ingestion.hypothesis_generator.JSON_DIR", tmp_path / "papers"):
            generator = HypothesisGenerator()
    ...
    def test_validate_hypotheses_schema(self):
        ...
        hypotheses = [
            Hypothesis(id="H1", description="Valid hypothesis with enough text", method="experiment", expected_outcome="Result", success_criteria="metric > 0.9"),
            Hypothesis(id="H2", description="Short", method="invalid_method", expected_outcome="Result", success_criteria="x"),  # Inválida
            Hypothesis(id="H3", description="Another valid hypothesis here", method="simulation", expected_outcome="Result", success_criteria="time < 1.0"),
        ]
...
class TestIntegration:
    ...
    async def test_paper_to_hypotheses_pipeline(self, tmp_path):
        ...
        mock_response = {
            "hypotheses": [
                {"id": "H1", "description": "DL improves NLP accuracy by >10%", "method": "experiment", "expected_outcome": "Accuracy improvement", "success_criteria": "acc_dl > acc_baseline * 1.10"},
                {"id": "H2", "description": "Method generalizes across languages", "method": "data_analysis", "expected_outcome": "Similar performance in 5+ languages", "success_criteria": "std(performance) < 0.05"},
                {"id": "H3", "description": "Transformer attention is interpretable", "method": "simulation", "expected_outcome": "Attention maps align with syntax", "success_criteria": "correlation > 0.7"},
            ]
        }
