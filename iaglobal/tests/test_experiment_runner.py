...
"""
    ...
    def test_save_result_creates_json(self, tmp_path):
        ...
        with patch("iaglobal.agents.ingestion.experiment_runner.JSON_DIR", tmp_path / "papers"):
            runner = ExperimentRunner()
...
class TestValidateHypotheses:
    ...
    async def test_validate_multiple_hypotheses(self):
        ...
        hypotheses = [
            Hypothesis(id="H1", description="Test 1", method="experiment", expected_outcome="R1", success_criteria="x > 0.5", paper_id="2401.001"),
            Hypothesis(id="H2", description="Test 2", method="data_analysis", expected_outcome="R2", success_criteria="y < 0.3", paper_id="2401.001"),
        ]
        
        with patch("iaglobal.agents.ingestion.experiment_runner.ExperimentRunner.run_experiment", new_callable=AsyncMock) as mock_run:
            ...
            with patch("iaglobal.agents.ingestion.experiment_runner.ExperimentRunner.save_result"):
                with patch("iaglobal.agents.ingestion.experiment_runner.ExperimentRunner.register_ivm_reward"):
