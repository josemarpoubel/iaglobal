...
async def test_get_cpu_state(aggregator):
    ...
    mock_cpu = MagicMock()
    mock_cpu.get_all_budgets = AsyncMock(return_value={
        "agent1": 0.25,
        "agent2": 0.50,
        "agent3": 0.25,
    })
    mock_cpu.get_all_metrics = AsyncMock(return_value={
...
async def test_health_endpoint_consolidated():
    ...
    with patch.object(HealthAggregator, 'check_all', return_value=mock_health_map):
        with patch.object(HealthAggregator, 'get_metabolic_state', return_value={
            "ivm_medio": 0.75,
            "agents_ativos": 5,
        }):
            with patch.object(HealthAggregator, 'get_cpu_state', return_value={
