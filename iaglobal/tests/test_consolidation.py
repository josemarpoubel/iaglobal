...
class TestResearchConsolidator:
    ...
    def test_generate_markdown_structure(self):
        ...
        results = [
            ExperimentResult(
                hypothesis_id="H1", paper_id="2401.12345", success=True,
                confidence=0.85, execution_time_ms=100, stdout="Success: True",
                stderr="", metrics={"acc": 0.92}, code="", validation_details="OK",
            ),
            ExperimentResult(
                hypothesis_id="H2", paper_id="2401.12345", success=False,
                confidence=0.3, execution_time_ms=50, stdout="Success: False",
                stderr="", metrics={}, code="", validation_details="Failed",
            ),
        ]
        ...
        markdown = consolidator._generate_markdown(paper, results, validated, failed, fitness)
    ...
    def test_format_aggregate_metrics(self):
        ...
        results = [
            ExperimentResult(
                hypothesis_id="H1", paper_id="2401.001", success=True,
                confidence=0.8, execution_time_ms=100, stdout="", stderr="",
                metrics={}, code="", validation_details="",
            ),
            ExperimentResult(
                hypothesis_id="H2", paper_id="2401.001", success=True,
                confidence=0.9, execution_time_ms=150, stdout="", stderr="",
                metrics={}, code="", validation_details="",
            ),
        ]
    ...
    def test_generate_conclusion_high_fitness(self):
        ...
        paper = PaperMetadata(
            paper_id="2401.001", title="Test", abstract="Abstract",
            authors=["A1"], published_date="2024", repository="arxiv", topics=[],
        )
        ...
        conclusion = consolidator._generate_conclusion(paper, fitness, validated, failed)
    ...
    async def test_consolidate_creates_object(self):
        ...
        results = [
            ExperimentResult(
                hypothesis_id="H1", paper_id="2401.12345", success=True,
                confidence=0.85, execution_time_ms=100, stdout="Success",
                stderr="", metrics={}, code="", validation_details="OK",
            ),
        ]
        
        consolidator = ResearchConsolidator()
        
        with patch.object(consolidator, "_write_to_obsidian", new_callable=AsyncMock) as mock_obsidian:
            mock_obsidian.return_value = None  # Sem Obsidian
            
            with patch.object(consolidator, "_save_consolidated_json"):
                consolidated = await consolidator.consolidate(paper, results, obsidian_enabled=False)
    ...
    async def test_write_to_obsidian_mock(self):
        ...
        with patch("iaglobal.obsidian.subconsciousapi.SubconsciousAPI") as mock_sub:
            ...
            mock_instance.escrever_longo_prazo = AsyncMock(return_value=Path("/obsidian/03_Long_Term/paper_2401_12345.md"))
...
class TestConsolidateFunctions:
    ...
    async def test_consolidate_paper_function(self):
        ...
        with patch("iaglobal.agents.ingestion.consolidation.ResearchConsolidator.consolidate", new_callable=AsyncMock) as mock_consolidate:
            mock_consolidate.return_value = ConsolidatedPaper(
                paper_id="2401.001", title="Test", abstract="Abstract",
                authors=["A1"], published_date="2024", repository="arxiv",
                topics=[], hypotheses_count=1, validated_count=1, failed_count=0,
                fitness_score=1.0, obsidian_path=None,
            )
            
            consolidated = await consolidate_paper(paper, results, obsidian_enabled=False)
    ...
    async def test_consolidate_full_pipeline(self):
        ...
        hypotheses = [Hypothesis(id="H1", description="Test", method="experiment", expected_outcome="Result", success_criteria="x > 0.5")]
        results = [
            ExperimentResult(
                hypothesis_id="H1", paper_id="2401.001", success=True,
                confidence=0.8, execution_time_ms=100, stdout="", stderr="",
        ...
        with patch("iaglobal.agents.ingestion.consolidation.consolidate_paper", new_callable=AsyncMock) as mock_consolidate:
            mock_consolidate.return_value = ConsolidatedPaper(
                paper_id="2401.001", title="Test", abstract="Abstract",
                authors=["A1"], published_date="2024", repository="arxiv",
                topics=["ml"], hypotheses_count=1, validated_count=1, failed_count=0,
                fitness_score=1.0, obsidian_path=None,
            )
            
            consolidated = await consolidate_full_pipeline(paper, hypotheses, results, obsidian_enabled=False)
...
class TestIntegration:
    ...
    async def test_full_validation_to_consolidation(self):
        ...
        results = [
            ExperimentResult(
                hypothesis_id="H1", paper_id="2401.12345", success=True,
                confidence=0.92, execution_time_ms=120, stdout="Success: True",
                stderr="", metrics={"acc": 0.92}, code="", validation_details="OK",
            ),
            ExperimentResult(
                hypothesis_id="H2", paper_id="2401.12345", success=True,
                confidence=0.88, execution_time_ms=95, stdout="Success: True",
                stderr="", metrics={"f1": 0.88}, code="", validation_details="OK",
            ),
            ExperimentResult(
                hypothesis_id="H3", paper_id="2401.12345", success=False,
                confidence=0.35, execution_time_ms=80, stdout="Success: False",
                stderr="", metrics={}, code="", validation_details="Failed",
            ),
        ]
        ...
        with patch.object(consolidator, "_write_to_obsidian", new_callable=AsyncMock) as mock_obsidian:
            ...
            with patch.object(consolidator, "_save_consolidated_json") as mock_save:
                ...
                assert consolidated.fitness_score == pytest.approx(0.67, rel=0.01)  # 2/3
