...
async def test_circuit_rejects_when_open():
    ...
    data = response.json()
    assert "circuit_open" in data.get("reason", "") or "unavailable" in data.get(
        "error", ""
    )
...
async def test_circuit_allows_when_closed():
    ...
    with patch(
        "iaglobal.server.asgi.mcp_breaker.call", side_effect=lambda f, *a: f(*a)
    ):
        response = client.get("/mcp/health")
