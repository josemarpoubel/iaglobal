...
class DecisionEngine:
    ...
    def score_option(
        self, agent: str, model: str, memory_score: float, task_complexity: float
    ) -> float:
        ...
        if self._bandit:
            try:
                ...
                metric = (
                    self._bandit._metrics_score(model)
                    if hasattr(self._bandit, "_metrics_score")
                    else 0.5
                )
                ...
                bandit_score = (
                    (credit_score * 0.5) + (metric * 0.25) + (reputation * 0.25)
                )
            except Exception:
                bandit_score = 0.5
