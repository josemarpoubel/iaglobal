...
EMBEDDING_CACHE_PATH = (
    PACKAGE_DIR / "memory" / "data" / "json" / "fewshot_embeddings.cbor"
)
...
class FewShotProvider:
    ...
    def __init__(self, preload: bool = False):
        ...
        self._example_cache: "OrderedDict[str, Tuple[List[FewShotExample], float]]" = (
            OrderedDict()
        )
        ...
        def _scan_and_load() -> int:
            ...
            for fpath in qdir.glob("cache_poison_*.json"):
                ...
                ex = FewShotExample(
                    ...
                    query="[NÃO REPETIR — razão: {}] {}".format(
                        reason, prompt_snippet[:150]
                    ),
    ...
    async def get_few_shot_async(
        self,
        task: str,
        max_examples: int = MAX_EXAMPLES,
        domain: str = "",
    ) -> FewShotResult:
        """Versão async do get_few_shot — executa em thread para não bloquear o event loop.

        Única chamadora no pipeline que escala para cloud (critic_agent) precisa
        desta versão para não travar o event loop durante embedding/ranking.
        """
        return await asyncio.to_thread(self.get_few_shot, task, max_examples, domain)
    ...
    def _from_mta_pool(self, query: str) -> List[FewShotExample]:
        ...
        try:
            ...
            for item in items[-20:]:
                ...
                task_hint = (
                    metadata.get("task", "") if isinstance(metadata, dict) else ""
                )
                examples.append(
                    FewShotExample(
                        source="mta_pool",
                        ...
                        answer="[{}] {}".format(
                            item_type.upper(), content[: self.MAX_ANSWER_CHARS]
                        ),
                        score=0.3,
                        language="text",
                        tags=["negative"],
    ...
    def _select_diverse(
        ...
        for c in candidates:
            if c.score < FewShotProvider.MIN_SCORE:
                continue
    ...
    def _format_section(examples: List[FewShotExample]) -> str:
        ...
        lines = [
            "",
            "[EXEMPLOS DE REFERÊNCIA — aprendidos de execuções anteriores]",
            "",
        ]
        for i, ex in enumerate(examples, 1):
            if ex.source == "mta_pool" or ex.source.startswith("dlq:"):
                ...
                lines.append(
                    "Exemplo {} (✅ padrão de sucesso — {}):".format(i, ex.source)
                )
    ...
    def _extract_callable_source(fn: Any) -> str:
        ...
        try:
            ...
            # inspect.BlockFound (Python <3.11) ou qualquer outro erro interno
            pass
        try:
