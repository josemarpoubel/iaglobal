...
class ErrorDetector:
    ...
    def detect(output: str, error: Optional[str] = None) -> str:
        ...
        if re.search(
            r"(não tenho certeza|posso estar enganado|talvez|provavelmente)",
            output.lower(),
        ):
...
class ModelEscalator:
    ...
    def get_stats(self) -> Dict:
        return {
            f"level_{k}_rate": f"{self.get_success_rate(k):.2f}" for k in self.TIERS
        }
...
class RetryHandler:
    ...
    def __init__(
        self, llm_router: Callable, max_attempts: int = 5, local_first: bool = True
    ):
    ...
    def execute(self, prompt: str, model: str = "auto") -> RetryResult:
        ...
        logger.info(
            f"[RETRY] Iniciando: prompt_len={len(prompt)} model={model} "
        ...
        while attempt < self.max_attempts:
            ...
            for m in models:
                try:
                    ...
                    last_error = str(e)
                    logger.debug(
                        f"[RETRY] Model {m.split('/')[-1]} fail: {str(e)[:60]}"
                    )
                    continue
            ...
            if not output:
                ...
                error_type = "error"
                logger.warning(
                    f"[RETRY] Attempt {attempt}: todos os modelos falharam (vazio/erro)"
                )
            else:
                error_type = self.detector.detect(output, last_error)
                if error_type != "ok":
                    logger.warning(
                        f"[RETRY] Attempt {attempt}: erro detectado={error_type} "
                        f"output_len={len(output)} model={model_used.split('/')[-1]}"
                    )
                else:
                    logger.info(
                        f"[RETRY] Attempt {attempt}: sucesso model={model_used.split('/')[-1]} "
            ...
            if error_type == "ok":
                ...
                logger.info(
                    f"[RETRY] Concluído com sucesso: attempts={attempt} "
            ...
            if error_type == "unknown":
                ...
                elapsed = time.time() - start_time
                logger.info(
                    f"[RETRY] UNKNOWN (intencional): attempts={attempt} elapsed={elapsed:.2f}s"
                )
                return RetryResult(
                    success=True,
                    output=output,
            ...
            current_prompt = self.fix_prompt(
                current_prompt, last_error or output[:200], error_type
            )
            last_error = f"attempt {attempt}: {error_type}"
            logger.debug(
                f"[RETRY] Prompt corrigido para attempt {attempt + 1}: "
            ...
            if attempt >= 2 and current_level == 0:
                ...
                if next_level is not None:
                    logger.info(
                        f"[RETRY] Escalando de {self.escalator.get_label(current_level)} "
                ...
                if next_level is not None:
                    logger.info(
                        f"[RETRY] Escalando para external (após {attempt} tentativas)"
                    )
                    current_level = next_level

        elapsed = time.time() - start_time
        logger.error(
            f"[RETRY] Esgotado após {self.max_attempts} tentativas: "
    ...
    def get_stats(self) -> Dict:
        ...
        logger.debug(
            f"[RETRY] Stats: total={total} success_rate={success / total:.2f} "
