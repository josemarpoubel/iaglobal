...
class Assistant:
    ...
    def _processar_assistente(self, prompt: Union[str, Task]) -> str:
        ...
        bus.publish(
            EventType.TASK_CREATED, {"task": prompt_str[:200]}, source="assistant"
        )
        ...
        if self.skill_gen and self.kb_writer:
            ...
            if kb_stats.get("total_entries", 0) % 5 == 1:
                ...
                if new_skills:
                    logger.info(
                        f"[ASSISTANT] {len(new_skills)} novas skills geradas da KB"
                    )
    ...
    def _dispatch_modelo(self, modelo: str, prompt: str) -> str:
        ...
        return blackjack_executar_local(model_name.replace("ollama/", ""), prompt)
    ...
    def _montar_prompt_cognitivo(self, ctx: dict) -> str:
        ...
        sections.append(
            "Você é um assistente com capacidade de consultar memória local e conhecimento da internet. Responda com base no contexto unificado abaixo."
        )
        ...
        sections.append("\n[CONSOLIDAÇÃO]")
        sections.append(
            "Use o contexto acima para responder com base em conhecimento atualizado. Se houver conflito entre web e memória, prefira a web por ser mais recente."
        )
    ...
    def _eh_teorico(self, prompt: str) -> bool:
        keywords = ["explique", "o que é", "conceito", "teoria", "descreva"]
        return any(re.search(rf"\b{kw}\b", prompt.lower()) for kw in keywords)
    ...
    def _consolidate_knowledge(
        self, prompt: str, resposta: str, web_results: List[Dict]
    ):
