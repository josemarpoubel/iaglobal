...
class CognitiveProxy:
    ...
    def run(self, user_input: str) -> ProxyResult:
        ...
        try:
            ...
            self.state_buffer.set(fingerprint.key(), status, output=validated, attempt=attempts)
            ...
            self._batch.emit(BatchEvent(
                event_type="cognitive_proxy_run",
                payload=json.dumps({"query": query[:200], "validated_len": len(validated) if validated else 0}),
            ...
            self._store_result(query, validated, fingerprint, success=True)
            self.stm.add(f"Q: {query}", {"type": "query", "fingerprint": fingerprint.key()})
            self.stm.add(f"A: {validated[:100]}", {"type": "response", "fingerprint": fingerprint.key()})
            ...
            if self.state_buffer.should_snapshot():
                snap_id = snapshotter.create_snapshot(self.state_buffer.get_snapshot_data())
                if snap_id:
...
"""
    ...
    def _route(self, prompt: str, fingerprint: Optional[TaskFingerprint] = None) -> tuple:
    ...
    def _record_bandit_result(self, model: str, success: bool, latency: float, 
        ...
        try:
            ...
            logger.error(f"[COGNITIVE-PROXY] Falha crítica ao registrar aprendizado: {e}")

    # =========================================================================
    # 5. FEEDBACK ENGINE (VALIDAÇÃO FORMAL + PATCH SAFE APPLY)
    # =========================================================================
    def _validate(self, input_prompt: str, output: str, context: Optional[Dict] = None) -> tuple:
        ...
        logger.info(f"[PROXY] FeedbackEngine rejeitou: {fb_result.errors} "
    ...
    # =========================================================================
    def _store_result(self, query: str, output: str, fingerprint: Optional[TaskFingerprint] = None, success: bool = True):
        ...
        try:
            ...
            self.ltm.store(output[:500], {"query": query, "fingerprint": fp_key, "success": success}, source="cognitive_proxy")
        except Exception as e:
