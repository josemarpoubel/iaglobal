...
class MitochondrialProbe:
    ...
    def register_alosteric_inhibitor(self, callback: Callable[[float], Awaitable[None]]):
    ...
    async def start_monitoring(self):
        ...
        while True:
            try:
                ...
                logger.exception(
                    "[MITO-PROBE] Erro no ciclo de monitoramento: %s", e
                )
    ...
    async def _trigger_alosteric_inhibitors(self, lag: float):
        ...
        for i, callback in enumerate(self._alert_callbacks):
            try:
                await callback(lag)
                logger.debug(
                    "[MITO-PROBE] Inibidor %d executado com sucesso", i + 1
                )
            except Exception as e:
                logger.exception(
                    "[MITO-PROBE] Falha em inibidor alostérico %d: %s",
                    i + 1, e
                )
