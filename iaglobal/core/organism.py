...
class Organism:
    ...
    async def start(self):
        ...
        logger.info("[Organism:%s] Spawned PID=%d data_root=%s", self.id, self.process.pid, self.data_root)
    ...
    async def _request(self, method: str, params: dict) -> Dict[str, Any]:
        ...
        if not line:
            ...
            raise OrganismError(f"Organism {self.id} morreu: {stderr.decode(errors='replace')}")
    ...
    async def stop(self):
        ...
        if self.process and self.process.returncode is None:
            ...
            try:
                ...
                logger.warning("[Organism:%s] Force kill PID=%d", self.id, self.process.pid)
                self.process.kill()
