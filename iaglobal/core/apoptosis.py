...
class ApoptosisEngine:
    ...
    async def trigger_apoptosis(self, agent_id: str, failure_count: int, max_retries: int = 3) -> Optional[str]:
        """Dispara a apoptose se o agente falhar além do limite. Retorna o ID do clone evoluído."""
        if failure_count < max_retries:
            return None

        logger.critical(f"💀 [APOPTOSIS] Agente {agent_id[:8]}... excedeu {max_retries} falhas. Iniciando morte programada...")

        # 1. Coleta todas as marcas epigenéticas do agente (async)
        agent_patterns = await asyncio.to_thread(lambda: list(self.epigenetic_path.glob("*.cbor")))
        agent_memories = []
        ...
        if not agent_memories:
            logger.warning(f"⚠️ [APOPTOSIS] Nenhuma memória encontrada para {agent_id}. Apoptose sem herança.")
            return None

        # 2. Cria um novo ID (clone) com DNA idêntico, mas geração incrementada
        new_agent_id = hashlib.sha3_512((agent_id + "_evolved").encode()).hexdigest()[:32]
        ...
        await asyncio.to_thread(self._write_cbor, archive_file, {
        ...
        for mem in agent_memories:
            ...
            new_file = self.epigenetic_path / f"{hashlib.sha3_512(f'{new_agent_id}:{mem['task_hash']}:{mem['error_type']}'.encode()).hexdigest()[:16]}.cbor"
            await asyncio.to_thread(self._write_cbor, new_file, mem)

        logger.info(f"🧬 [APOPTOSIS] {len(agent_memories)} memórias herdadas pelo clone {new_agent_id[:8]}...")
