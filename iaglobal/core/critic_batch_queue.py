...
class CriticBatchQueue:
    ...
    def _montar_prompt_cruzado(
        ...
        context = "\n\n".join(context_parts) if context_parts else "Nenhuma avaliacao previa disponivel."
    ...
    async def _call_llm(self, prompt: str, bandit: Any) -> Optional[str]:
        ...
        if bandit:
            try:
                ...
                if resultado:
                    logger.info("[CRITIC_BATCH] Geração concluída (%d chars)", len(resultado))
                    return resultado
            except Exception as e:
        ...
        try:
            ...
            return await async_route_generate(
                "ollama/qwen2.5:0.5b", prompt,
                task_type="critic", node_id="critic_batch",
            )
        except Exception as e:
