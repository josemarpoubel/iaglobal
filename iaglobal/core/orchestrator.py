...
class Orchestrator:
    ...
    def __init__(
        self, organism_id: str = "default", data_root: Path | None = None
    ) -> None:
        ...
        logger.info(
            "[Orchestrator:%s] Initialized (data_root=%s)", organism_id, self.data_root
        )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                "[Orchestrator:%s] Pipeline init failed: %s", self.organism_id, e
            )
            raise
        ...
        try:
            if os.environ.get("EVOLUTION_AUTO") == "1":
                ...
                logger.info(
                    "[Orchestrator:%s] Evolution runtime started", self.organism_id
                )
        except Exception as e:
            logger.debug(
                "[Orchestrator:%s] Evolution runtime skip: %s", self.organism_id, e
            )
            self.evolution_runtime = None
        ...
        from iaglobal.graphs.comms.agent_mailbox import mailbox_manager
    ...
    def _bind_agents_to_mailboxes(self):
        ...
        try:
            ...
            self.mailbox_manager.bind_executor(
                "PlannerAgent", planner.criar_plano_execucao
            )
    ...
    async def start_heartbeat_loop(self, sleep_interval: float = 0.5):
        ...
        """
        logger.info(
            "[SYNAPSE] Iniciando heartbeat loop (interval=%.2fs)", sleep_interval
        )
    ...
    def run(self, task: str, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            fut = asyncio.run_coroutine_threadsafe(
                self.async_run_graph_task(task, force=force), loop
            )
    ...
    async def shutdown(self) -> None:
        ...
        if self.evolution_runtime:
            try:
                ...
                logger.debug(
                    "[Orchestrator:%s] Evolution stop error: %s", self.organism_id, e
                )
        self.initialized = False
