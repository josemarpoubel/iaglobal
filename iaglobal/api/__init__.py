...
class IAGlobalAPI:
    ...
    def __init__(self, lazy_init: bool = False):
        ...
        if not lazy_init:
            try:
                ...
                logger.debug(
                    "[API] Event loop ativo detectado — inicialização adiada para initialize_async()"
                )
            except RuntimeError:
                self.initialize()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {
            "version": {
                ...
                "graph_gen": graph.generation if graph else 0,
            },
            "dag": {
        ...
        if evolver:
            ...
            status["evolution"]["mutation_rates"] = getattr(
                evolver, "strategy_mutation_rates", {}
            )
    ...
    def get_insights(
        ...
        return db.get_insights(
            agent=agent, limit=limit, offset=offset, min_score=min_score
        )
    ...
    def list_scripts(self) -> List[Dict[str, Any]]:
        ...
        scripts = []
        for f in sorted(
            SCRIPTS_DIR.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True
        ):
            if f.suffix == ".py":
                scripts.append(
                    {
