...
_SECURITY_BLOCKLIST = {
    ...
    "fcntl", "signal", "multiprocessing", "threading",
    "inspect", "sys", "code", "codeop", "bdb", "pdb",
    "traceback", "atexit", "gc", "pickle", "shelve",
    "marshal", "imp", "importlib.metadata", "pkgutil",
    "plistlib", "sched", "spwd", "grp", "termios",
    "tty", "pty", "webbrowser", "winreg",
}
...
class SandboxExpansion:
    ...
    def is_safe(self, module_name: str) -> bool:
        ...
        if base in _SECURITY_BLOCKLIST:
            logger.warning("[SANDBOX_EXPANSION] Lib bloqueada por seguranca: %s", module_name)
            return False
        return True
    ...
    def request_install(self, module_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error("[SANDBOX_EXPANSION] Falha ao instalar %s: %s", pypi_name, result.stderr)
                return False
    ...
    def _add_to_allowed_modules(self, module_name: str) -> None:
        try:
            ...
            logger.info("[SANDBOX_EXPANSION] %s adicionado ao allowed_modules", module_name)
        except Exception as e:
            logger.warning("[SANDBOX_EXPANSION] Nao foi possivel atualizar allowed_modules: %s", e)
