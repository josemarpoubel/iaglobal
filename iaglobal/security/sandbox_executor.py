...
class SandboxExecutor:
    ...
    def execute(self, code: str, workdir: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not guardrail["safe"]:
            logger.warning(
                "[SANDBOX] Glutathione bloqueou execucao: %s", guardrail["issues"]
            )
            immune_response = GlutathionePool().respond(
                "hallucination", {"issues": guardrail["issues"]}
            )
            return {
                "sucesso": False,
        ...
        if not self.rules.is_path_allowed_for_write(sandbox_dir):
            logger.warning(
                "[SANDBOX] Path '%s' nao permitido — criando em /tmp", sandbox_dir
            )
            sandbox_dir = "/tmp"
        ...
        try:
            ...
            logger.info(
                "[SANDBOX] Executando codigo em subprocesso isolado (exec #%d)...",
                self._execution_count,
            )
            logger.debug(
                "[SANDBOX] Regras ativas: %d modulos, %d paths leitura, %d paths escrita",
                len(self.rules.allowed_modules),
                len(self.rules.allowed_read_paths),
            ...
            if not sucesso and "ModuleNotFoundError" in stderr:
                logger.warning(
                    "[SANDBOX] Falha de dependencia detectada: %s",
                    stderr.split("\n")[-1],
                )

            logger.info(
                "[SANDBOX] Execucao #%d: returncode=%d | stdout=%d chars | stderr=%d chars",
                self._execution_count,
                result.returncode,
                len(stdout),
                len(stderr),
            ...
            logger.warning(
                "[SANDBOX] Timeout (%ds) na execucao #%d",
                self.timeout,
                self._execution_count,
            )
            return {
