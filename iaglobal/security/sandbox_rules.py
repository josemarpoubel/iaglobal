...
class SandboxRules:
    ...
    def __init__(self):
        ...
        self._auto_discover_installed_packages()
    ...
    def _auto_discover_installed_packages(self):
        ...
        try:
            ...
            pkgs = [
                dist.metadata.get("Name", "").lower() for dist in md.distributions()
            ]
            pkgs = [p for p in pkgs if p]
            # Converte nome do pacote para nome do modulo (ex: beautifulsoup4 -> bs4)
            for pkg in pkgs:
                self.allowed_modules.add(pkg.replace("-", "_"))
            # Tenta importar rapidamente para descobrir alias comuns
            for alias in ["bs4", "yfinance", "sklearn", "cv2", "PIL"]:
                try:
                    __import__(alias)
                    self.allowed_modules.add(alias)
                except ImportError:
                    pass
            logger.info(
                "[SANDBOX-RULES] Auto-descobertos %d pacotes instalados", len(pkgs)
            )
        except Exception as e:
            logger.debug("[SANDBOX-RULES] Auto-descoberta falhou: %s", e)
    ...
    def is_module_allowed(self, module_name: str) -> bool:
        ...
        if base not in self.allowed_modules:
            logger.warning(
                "[SANDBOX-RULES] Modulo bloqueado: '%s' (base='%s')", module_name, base
            )
            return False
        return True
    ...
    def is_path_allowed_for_read(self, path: str) -> bool:
        ...
        for blocked in self.blocked_paths:
            if abs_path.startswith(blocked):
                logger.warning(
                    "[SANDBOX-RULES] Path bloqueado para leitura: %s", abs_path
                )
                return False
    ...
    def is_path_allowed_for_write(self, path: str) -> bool:
        ...
        for blocked in self.blocked_paths:
            if abs_path.startswith(blocked):
                logger.warning(
                    "[SANDBOX-RULES] Path bloqueado para escrita: %s", abs_path
                )
                return False
    ...
    def sanitize_environment(
        ...
        if removed:
            logger.info(
                "[SANDBOX-RULES] Variaveis de ambiente sanitizadas: %d removidas",
                len(removed),
            )
            for v in removed:
