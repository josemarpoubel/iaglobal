...
class MCPSandbox:
    ...
    async def check_rate_limit(self, tool_name: str, agent_id: str = "unknown") -> bool:
        ...
        if len(calls) >= limits["calls_per_minute"]:
            logger.warning("Rate limit excedido para %s/%s: %d/min", agent_id, tool_name, len(calls))
            return False

        self._call_log[key].append(now)
        return True

    async def audit_call(self, tool_name: str, arguments: dict, result: Any, agent_id: str = "unknown", allowed: bool = True):
        audit_entry = {
        ...
        logger.debug("Audit MCP: %s/%s → %s", agent_id, tool_name, audit_entry["sandbox_decision"])
        ...
        async with self.lock: # Bloqueia a escrita para evitar colisões
            try:
                ...
                if AUDIT_PATH.exists():
                    async with aiofiles.open(AUDIT_PATH, mode='r', encoding='utf-8') as f:
