...
class ResultAgent(AgentBase):
    ...
    async def _call_summary_extractor(
        self, agent_name: str, data: Any
    ...
    def _extract_generic_summary(self, data: Any):
        ...
        try:
            if isinstance(data, dict):
                ...
                if output:
                    return (f"dados produzidos por {type(output).__name__}", [])
    ...
    async def build_result(self, ctx: Dict[str, Any]) -> Dict[str, Any]:

        logger.info("[RESULT_AGENT] Building Final Execution Contract...")
        ...
        for agent_name in self._summary_extractors:
            ...
            try:
                summary, next_steps = await self._call_summary_extractor(
                ...
                logger.exception("Extractor failure [%s]: %s", agent_name, exc)

                summary_parts.append(f"{agent_name}: extractor failure")
        ...
        health = PipelineHealth(
            ...
            success_rate=round(successful_agents / total_agents * 100, 2)
            if total_agents
            else 0.0,
        )
        ...
        logger.info("[RESULT_AGENT] Contract generated successfully")
                ...
                if isinstance(data, dict) and "model" in data:
                    ...
                    if success and latency < 2.0:
                        ...
                        bandit.update_policy(
                            node, model, "default", False, latency, 0.0
                        )

            logger.info(
                "[RESULT_AGENT] 🧬 Reflexão Genômica concluída para exec=%s",
                execution_id,
            )
        except Exception as e:
    ...
    def _resolve_main_payload(self, ctx: Dict[str, Any]) -> Any:
        ...
        return ctx.get("final_output") or ctx.get("generated_file") or {}

    # ========================================================
    # CHECKSUM
    # ========================================================

    def _generate_checksum(self, payload: Any) -> str:
    ...
    async def _extract_docs_summary(self, data: Any) -> Tuple[Optional[str], List[str]]:

        if isinstance(data, dict):
            if data.get("readme"):
                return ("Technical documentation generated", [])

        return ("Documentation available", ["Review generated documentation"])
    ...
    async def _extract_release_summary(
        ...
        return ("Release metadata processed", [])
    ...
    async def _extract_metrics_summary(
        ...
        if durations:
            total_time = sum(float(v) for v in durations.values())

            metrics.append(f"Execution Time: {total_time:.2f}s")

        if quality:
            avg_quality = sum(quality.values()) / len(quality)

            metrics.append(f"Quality: {avg_quality:.2f}/10")
    ...
    async def _extract_code_summary(self, data: Any) -> Tuple[Optional[str], List[str]]:
        ...
        tests = data.get("tests_generated", False)

        return (
            f"Code artifact generated [{language}]",
            ([] if tests else ["Generate unit tests"]),
        )
    ...
    async def _extract_security_summary(
        ...
        criticals = [v for v in vulnerabilities if v.get("severity") == "critical"]
    ...
    def register_extractor(
        self,
        agent_name: str,
        extractor: Callable[[Any], Tuple[Optional[str], List[str]]],
    ) -> None:

        logger.info("Registering extractor [%s]", agent_name)

        self._summary_extractors[agent_name] = extractor
