...
from iaglobal.agents.semantic_validator import (
    RuleRegistry,
    ScoreAggregator,
    LanguageDetector,
)
...
class SemanticValidatorAgent(AgentBase):
    ...
    async def validate_async(self, code: str, task: str) -> ValidationResult:
        ...
        try:
            ...
            score, by_cat, errors, suggestions = ScoreAggregator.aggregate(
                valid_results
            )
            ...
            logger.error(
                "[VALIDATOR] Timeout de %ss atingido na validação.", self.timeout
            )
            return ValidationResult(
                valid=False,
                score=0.0,
    ...
    async def _safe_detect_language(self, code: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "[VALIDATOR] Falha ao detectar linguagem: %s. Assumindo 'python'.", e
            )
            return "python"

    async def _evaluate_rule_safe(
        self, rule, code: str, task: str, language: str
    ) -> Optional[RuleResult]:
        """
        ...
        try:
            ...
            logger.warning(
                "[VALIDATOR] Regra '%s' falhou com erro inesperado: %s", rule_name, e
            )
            return None
