...
class Severity(str, Enum):
    ...
    @property
    def weight(self) -> int:
        return {"critical": 40, "high": 20, "medium": 8, "low": 2, "info": 0}[
            self.value
        ]
...
class PatternRegistry:
    ...
    def register(self, rule: AuditRule) -> None:
        ...
        if rule.rule_id in self._rules:
            logger.warning(
                "⚠️  [REGISTRY] Sobrescrevendo regra existente: %s", rule.rule_id
            )
...
class IAuditEngine(Protocol):
    def scan(
        self, code: str, rules: list[AuditRule], recidivism_ids: set[str]
    ) -> list[Finding]: ...
...
class StructuralAnalyzer:
    ...
    def analyze(self, code: str) -> list[str]:
        ...
        max_depth = (
            max((len(l) - len(l.lstrip())) // 4 for l in non_empty if l.strip())
            if non_empty
            else 0
        )
        if max_depth > 5:
        ...
        def_positions = [
            i for i, l in enumerate(lines) if l.lstrip().startswith("def ")
        ]
...
class PerformanceAuditAgent(AgentBase):
    ...
    def audit(
        ...
        """
        logger.info(
            "📊 [PERF-AUDIT] Iniciando auditoria — %d bytes de código", len(code)
        )
        ...
        logger.info(
            ...
            risk_score,
            severity_count.get(Severity.HIGH.value, 0)
            + severity_count.get(Severity.CRITICAL.value, 0),
