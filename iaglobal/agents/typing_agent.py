...
class TypingAgent(AgentBase):
    ...
    def simulate_typing(
        ...
        for word_idx, word in enumerate(words):
            for char_idx, char in enumerate(word):
                ...
                if (
                    char_idx == len(word) - 1
                    and random.random() < self.profile.pause_chance
                ):
        ...
        logger.debug(
            f"[TYPING] Concluído: {len(text)} chars em {elapsed:.1f}s "
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "avg_speed": f"{self._chars_typed / self._total_time:.1f} cps"
    ...
    def estimate_time(self, text: str) -> float:
        ...
        pauses = (
            len(words)
            * self.profile.pause_chance
            * (sum(self.profile.pause_duration) / 2)
        )
...
class TypingService:
    ...
    def web_llm_call(self, prompt: str, model_name: str = "chatgpt_web") -> str:
        ...
        logger.info(
            f"[TYPING-SERVICE] Chamada web: model={model_name} prompt_len={len(prompt)}"
        )
        ...
        try:
            ...
            with sync_playwright() as pw:
                ...
                if model_name == "chatgpt_web":
                    ...
                    page.wait_for_selector(
                        "[data-testid^='conversation-turn']", timeout=60000
                    )
                    result = (
                        page.text_content(
                    ...
                    page.fill("[contenteditable='true']", prompt)
                    page.keyboard.press("Enter")
                    page.wait_for_timeout(30000)
                    result = page.text_content("div.mb-xs") or ""
                else:
                    result = prompt
            ...
            logger.warning(f"[TYPING-SERVICE] Playwright falhou: {e}")
            return prompt
