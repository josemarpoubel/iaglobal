...
class FileIngestionAgent:
    ...
    def ingest(cls, paths: List[str], max_size_kb: int = 2000) -> Dict[str, Any]:
        ...
        for path_str in paths:
            ...
            if p.suffix.lower() not in SUPPORTED_EXTENSIONS:
                errors.append(
                    {"path": path_str, "error": f"Extensão não suportada: {p.suffix}"}
                )
                continue
    ...
    def _ingest_file(cls, path: Path, max_size_kb: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            if size_kb > max_size_kb:
                logger.warning(
                    "[INGEST] Arquivo muito grande: %s (%.1f KB)", path.name, size_kb
                )
                return None
    ...
    def _extract_pdf_text(cls, path: Path) -> str:
        ...
        try:
            ...
            pass
        logger.warning(
            "[INGEST] Nenhum extrator de PDF disponível (tente: pip install pdfminer.six)"
        )
