...
"""
    ...
    async def generate(self, paper: PaperMetadata) -> List[Hypothesis]:
        ...
        try:
            ...
            if not hypotheses_data or "hypotheses" not in hypotheses_data:
                logger.warning("[HYP] LLM não retornou hipóteses válidas para %s", paper.paper_id)
            ...
            logger.info("[HYP] %s: %d hipóteses geradas", paper.paper_id, len(hypotheses))
            return hypotheses
    ...
    def _generate_fallback_hypothesis(self, paper: PaperMetadata, index: int) -> Hypothesis:
        """Gera uma hipótese fallback individual."""
        methods = ["experiment", "data_analysis", "simulation"]
        method = methods[index % 3]
        
        topic = paper.topics[index % len(paper.topics)] if paper.topics else f"method_{index+1}"
    ...
    def save_hypotheses(self, hypotheses: List[Hypothesis], paper_id: str) -> Path:
        ...
        output_path = JSON_DIR / "papers" / f"{paper_id.replace(':', '_')}_hypotheses.json"
    ...
    def validate_hypotheses(self, hypotheses: List[Hypothesis]) -> List[bool]:
        ...
        for hyp in hypotheses:
            ...
            method_valid = hyp_dict.get("method") in ["experiment", "data_analysis", "simulation"]
