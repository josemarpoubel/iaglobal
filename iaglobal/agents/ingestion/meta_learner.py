...
class MetaLearner:
    ...
    async def load_consolidated_papers(self) -> List[Dict[str, Any]]:
        ...
        if not self.consolidated_dir.exists():
            logger.warning(
                "[META] Diretório de papers não existe: %s", self.consolidated_dir
            )
            return papers
    ...
    async def generate_meta_analysis(
        ...
        if len(papers) < min_papers:
            logger.info(
                "[META] Apenas %d papers (mínimo: %d) — skip meta-análise",
                len(papers),
                min_papers,
            )
            return None
        ...
        overall_fitness = (
            validated_hypotheses / total_hypotheses if total_hypotheses > 0 else 0.0
        )
    ...
    async def curate_low_fitness_papers(self, threshold: float = 0.5) -> List[str]:
        ...
        for p in papers:
            if p["summary"]["fitness_score"] < threshold:
                ...
                consolidated_file = (
                    self.consolidated_dir / f"{file_safe_id}_consolidated.json"
                )
    ...
    async def generate_recommendations(
        ...
        high_fitness_papers = [
            p for p in papers if p["summary"]["fitness_score"] >= 0.8
        ]
        ...
        sorted_topics = sorted(
            topic_avg_fitness.items(), key=lambda x: x[1], reverse=True
        )
    ...
    async def trigger_replication(self, paper_id: str) -> bool:
        ...
        if not (0.33 <= fitness <= 0.67):
            logger.info(
                "[META] %s não é candidato a replicação (fitness=%.0f%%)",
                paper_id,
                fitness * 100,
            )
            return False
    ...
    async def generate_cross_citations(self) -> Dict[str, List[str]]:
        ...
        await asyncio.gather(
            *[_update_citation(pid, cited) for pid, cited in citations.items()]
        )
