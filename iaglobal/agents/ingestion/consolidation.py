...
"""
    ...
    async def consolidate(
        ...
        markdown_content = self._generate_markdown(paper, results, validated, failed, fitness_score)

        # Escrever no Obsidian
        obsidian_path = None
        if obsidian_enabled:
            obsidian_path = await self._write_to_obsidian(markdown_content, paper, fitness_score)
    ...
    def _generate_markdown(
        ...
        # Formatar tópicos
        topics_md = ", ".join(f'"{t}"' for t in paper.topics) if paper.topics else '"sem-topicos"'
        tags_md = ", ".join(f'"{t}"' for t in paper.topics[:5]) if paper.topics else '"research"'
        ...
        topics_list = "\n".join(f"- `{t}`" for t in paper.topics) if paper.topics else "- *sem tópicos*"
        ...
        markdown = self.MARKDOWN_TEMPLATE.format(
            ...
            consolidated_at=datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"),
        )
    ...
    def _get_hypothesis_description(self, result: ExperimentResult) -> str:
        ...
        return result.validation_details[:100] if result.validation_details else "Hipótese não disponível"
    ...
    async def _write_to_obsidian(self, content: str, paper: PaperMetadata, fitness_score: float = 0.5) -> str:
    ...
    def _save_consolidated_json(self, consolidated: ConsolidatedPaper, results: List[ExperimentResult]):
        """Salva consolidação em JSON."""
        output_path = JSON_DIR / f"{consolidated.paper_id.replace(':', '_')}_consolidated.json"
