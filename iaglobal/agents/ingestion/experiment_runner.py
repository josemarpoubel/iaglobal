...
from iaglobal._paths import JSON_DIR, TEMP_DIR
...
"""
            ...
            if self._validate_syntax(code):
                ...
                logger.warning("[EXP] Código gerado tem erro de sintaxe — usando template")
                return None
    ...
    def _generate_code_from_template(self, hypothesis: Hypothesis) -> str:
        ...
        template = self.CODE_TEMPLATES.get(hypothesis.method, self.CODE_TEMPLATES["experiment"])
    ...
    def _evaluate(self, execution_result: Dict[str, Any], hypothesis: Hypothesis) -> Dict[str, Any]:
        ...
        failure_indicators = ["success: false", "success: false", "❌", "failed", "invalid"]
        ...
        if has_success and not has_failure:
            confidence = 0.85 + 0.15 * (len(metrics) / 5)  # Mais métricas = mais confiança
            return {
                "success": True,
    ...
    def save_result(self, result: ExperimentResult) -> Path:
        ...
        output_path = JSON_DIR / "papers" / f"{result.hypothesis_id}_{result.paper_id.replace(':', '_')}_result.json"
