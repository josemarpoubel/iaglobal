...
class PaperRecord:
    ...
    repository: str
    status: str = (
        "pending"  # pending|ingested|parsed|hypothesized|validated|consolidated
    )
...
class PaperIngestor:
    ...
    async def ingest(self, paper_id: str, repository: str = "arxiv") -> Optional[Path]:
        ...
        existing = next(
            (r for r in queue_data["queue"] if r["paper_id"] == paper_id), None
        )
        if existing:
            logger.info(
                "[INGEST] Paper %s já está na fila (status: %s)",
                paper_id,
                existing["status"],
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(pdf_url, timeout=60) as response:
                    if response.status == 200:
                        ...
                        logger.info(
                            "[INGEST] arXiv %s: %.1f KB", paper_id, len(content) / 1024
                        )
                        return local_path
                    else:
                        logger.warning(
                            "[INGEST] arXiv %s retornou status %d",
                            paper_id,
                            response.status,
                        )
                        return None
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(html_url, timeout=60) as response:
                    if response.status == 200:
                        ...
                        logger.info(
                            "[INGEST] PubMed %s: %.1f KB", paper_id, len(content) / 1024
                        )
                        return local_path
                    else:
                        logger.warning(
                            "[INGEST] PubMed %s retornou status %d",
                            paper_id,
                            response.status,
                        )
                        return None
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(html_url, timeout=60) as response:
                    if response.status == 200:
                        ...
                        logger.warning(
                            "[INGEST] HF Papers %s retornou status %d",
                            paper_id,
                            response.status,
                        )
                        return None
    ...
    def get_paper_record(self, paper_id: str) -> Optional[Dict[str, Any]]:
        ...
        queue_data = self._load_queue()
        return next(
            (r for r in queue_data.get("queue", []) if r["paper_id"] == paper_id), None
        )


# Funções utilitárias para ingestão em lote
async def ingest_batch(
    paper_ids: List[str], repository: str = "arxiv"
) -> List[Optional[Path]]:
    """
