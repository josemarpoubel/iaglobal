...
class PaperParser:
    ...
    async def parse(
        self, text: str, paper_id: str, repository: str = "arxiv"
    ...
    def _extract_metadata(
        self, text: str, paper_id: str, repository: str
    ) -> Dict[str, Any]:
        ...
        for pattern in author_patterns:
            ...
            if match:
                ...
                metadata["authors"] = [
                    a.strip() for a in authors if len(a.strip()) > 2
                ][:10]
                break
        ...
        if not metadata["published_date"] and repository == "arxiv":
            arxiv_year_match = re.match(
                r"(\d{2})(\d{2})", paper_id.replace("arxiv:", "").split(".")[0]
            )
            if arxiv_year_match:
        ...
        for pattern in keyword_patterns:
            ...
            if match:
                ...
                keywords = re.split(r"[,;\n]", keywords_text)
                metadata["topics"] = [
                    k.strip().lower() for k in keywords if len(k.strip()) > 2
                ][:20]
                break
    ...
    def save_metadata(
        self, metadata: PaperMetadata, output_path: Optional[Path] = None
    ) -> Path:
        """Salva metadados em JSON."""
        if output_path is None:
            output_path = (
                JSON_DIR / "papers" / f"{metadata.paper_id.replace(':', '_')}.json"
            )
...
# Funções utilitárias
async def parse_paper_file(
    file_path: Path, paper_id: Optional[str] = None
) -> Optional[PaperMetadata]:
    """
