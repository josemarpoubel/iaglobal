...
logger = logging.getLogger("iaglobal.agents.semantic_validator")

# Gateway singleton para AST parsing
_ast_gateway = ASTGateway()

# Normalizer singleton para normalização sintática
_normalizer = PythonNormalizer()
...
class LanguageDetector:
    ...
    async def detect(code: str) -> Language:
        ...
        if code_stripped.startswith("<!DOCTYPE html") or code_stripped.startswith(
            "<html"
        ):
...
class ValidationRule:
    ...
    async def evaluate(
        self, code: str, task: str, language: Language
    ) -> Optional[RuleResult]:
    ...
    async def evaluate(
        ...
        try:
            ...
            if has_func_or_class:
                return RuleResult(
                    self.name,
                    "Possui estrutura (função/classe)",
                    True,
                    self.weight,
                    self.category,
                )
                ...
                return RuleResult(
                    self.name,
                    "Código Python sem função ou classe",
                    False,
                    self.weight,
                    self.category,
...
class HtmlStructureRule(ValidationRule):
    ...
    async def evaluate(
        ...
        if has_doctype and has_html and has_body:
            return RuleResult(
                self.name,
                "HTML completo com doctype, html e body",
                True,
                self.weight,
                self.category,
            )
            ...
            return RuleResult(
                self.name,
                f"HTML incompleto: faltando {', '.join(missing)}",
                False,
                self.weight,
                self.category,
...
class KeywordPresenceRule(ValidationRule):
    ...
    def __init__(
        self, name: str, weight: float, category: str, pattern: Pattern, label: str
    ):
    ...
    async def evaluate(
        ...
        if found:
            return RuleResult(
                self.name,
                f"{self.label} presente no código",
                True,
                self.weight,
                self.category,
            )
            ...
            return RuleResult(
                self.name,
                f"{self.label} ausente no código",
                False,
                self.weight,
                self.category,
...
class PythonImportHashlibRule(ValidationRule):
    ...
    def __init__(self):
        ...
        self.hashlib_pattern = re.compile(
            r"\bimport\s+hashlib\b|\bfrom\s+hashlib\s+import\b"
        )
    ...
    async def evaluate(
        ...
        if has_import:
            return RuleResult(
                self.name, "Import hashlib presente", True, self.weight, self.category
            )
        else:
            return RuleResult(
                self.name,
                "Import hashlib ausente",
                False,
                self.weight,
                self.category,
    ...
    def default(cls) -> "RuleRegistry":
        ...
        registry.register(
            KeywordPresenceRule(
                ...
                pattern=re.compile(r"sha3.?512", re.IGNORECASE),
                label="SHA3-512",
            )
        )
        return registry
    ...
    def aggregate(results: List[RuleResult]) -> tuple:
        ...
        for cat, rules in by_category.items():
            ...
            score_by_category[cat] = (
                (cat_passed / cat_weight * 100.0) if cat_weight > 0 else 100.0
            )
...
class SemanticValidatorAgent(AgentBase):
    ...
    async def _safe_detect_language(self, code: str) -> Language:
        """Detecta linguagem sem bloquear o event loop."""
        try:
            ...
            logger.warning(
                "[SEMANTIC] Falha ao detectar linguagem: %s. Assumindo 'python'.", e
            )
            return Language.PYTHON

    async def _evaluate_rule_safe(
        self, rule, code: str, task: str, language: Language
    ) -> Optional[RuleResult]:
        """Avalia regra com tratamento resiliente de erros."""
        ...
        try:
            ...
            # Syntax errors are intentionally assigned weight=0.0.
            # They are handled earlier by ASTGateway and ValidationEngine,
            # which already stop invalid code before semantic ranking.
            # Keeping zero weight avoids double-penalizing the same failure.
            return RuleResult(
                name=rule_name,
            ...
            logger.warning(
                "[SEMANTIC] Regra '%s' falhou com erro inesperado: %s", rule_name, e
            )
            return None
    ...
    async def validate_async(self, code: str, task: str = "") -> ValidationResult:
        ...
        try:
            # Pipeline de normalização sintática antes da validação
            # Isso garante que o score não seja penalizado por questões cosméticas
            try:
                norm_result = _normalizer.normalize(code)
                if norm_result.syntax_valid:
                    logger.debug(
                        f"Código normalizado: format={len(norm_result.ruff_format_errors)}, "
                        f"check={len(norm_result.ruff_check_errors)}"
                    )
                    code = norm_result.fixed  # Usa código normalizado
                else:
                    logger.debug(
                ...
                logger.warning(f"Normalização falhou: {e}. Usando código original.")

            language = await self._safe_detect_language(code)

            # Paralelismo com timeout global
            tasks = [
            ...
            score, score_by_category, errors, suggestions = ScoreAggregator.aggregate(
                valid_results
            )
            ...
            logger.info(
                "[SEMANTIC] score=%.2f valid=%s lang=%s time=%.2fms errors=%d",
                score,
            ...
            logger.error(
                "[SEMANTIC] Timeout de %ss atingido na validação.", self.timeout
            )
            return ValidationResult(
                valid=False,
                score=0.0,
            ...
            logger.exception("[SEMANTIC] Erro inesperado na validação.")
            return ValidationResult(
                valid=False,
                score=0.0,
                language=Language.GENERIC,
