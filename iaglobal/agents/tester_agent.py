...
class TesterAgent(AgentBase):
    ...
    async def _validar_testes_com_jedi(
        self, test_code: str, codigo_original: str
        ...
        try:
            ...
            has_errors = (
                analysis.get("has_syntax_error", False)
                or len(analysis.get("issues", [])) > 0
            )
            return test_code, {
    ...
    async def _corrigir_testes_com_jedi(
        self, test_code: str, issues: List[Dict[str, Any]]
    ) -> str:
        try:
            import jedi
                ...
                if line and ("undefined" in msg.lower() or "import" in msg.lower()):
                    ...
                    if 1 <= line <= len(lines):
                        ...
                        if line_content.strip().startswith("import "):
                            ...
                            lines[line - 1] = (
                                f"# {line_content}  # modulo nao encontrado"
                            )
                            test_code = "\n".join(lines)
                            break
    ...
    async def _registrar_template_teste(self, test_code: str, task: str) -> None:
        try:
            ...
            tags = ["test", "pytest"] + [w.lower() for w in task.split() if len(w) > 3][
                :3
            ]
    ...
    async def gerar_testes(
        ...
        if not codigo or not str(codigo).strip():
            return TestGenerationResult(
                success=False, error_message="Codigo vazio ou nulo fornecido."
            )
        ...
        try:
            ...
            correcao = await asyncio.to_thread(
                auto_correction.corrigir, test_code, task_text
            )
            ...
            enforce_result = await asyncio.to_thread(
                dependency_enforcer.enforce, test_code
            )
            ...
            test_code, validacao = await self._validar_testes_com_jedi(
                test_code, codigo
            )
            if not validacao.get("valid", True):
                logger.info("[TESTER] Corrigindo testes com Jedi...")
                test_code = await self._corrigir_testes_com_jedi(
                    test_code, validacao.get("issues", [])
                )
            ...
            try:
                ...
                result_exec = {
                    "sucesso": False,
                    "stdout": "",
                    "stderr": "Timeout na execucao dos testes",
                }
            ...
            if not result_exec.get("sucesso", False):
                ...
                if stderr:
                    ...
                    logger.info(
                        "[TESTER] Execucao falhou: %d erros detectados", len(erros)
                    )
                    test_code, _ = await self._validar_testes_com_jedi(
                        test_code, codigo
                    )
            ...
            success = (
                result_exec.get("sucesso", False) or result_exec.get("returncode") == 0
            )
            ...
            return TestGenerationResult(
                success=False,
                error_message=f"Timeout apos {timeout}s",
                language_detected=lang_hint,
            )
        except Exception as e:
            ...
            return TestGenerationResult(
                success=False, error_message=err_msg, language_detected=lang_hint
            )
