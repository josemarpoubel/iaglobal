...
_DOMAIN_SPECIFIC_NODES = {
    ...
    "teste": ["test_design", "qa_validation"], # Adicionado para cobrir o intent_classifier
}
...
_NODE_PRIORITY = [
    "pm", "requirements", "domain_analysis", "architect", "system_design", 
    "api_design", "database_design", "frontend_builder", "security_design", 
    "threat_modeling", "failure_analysis", "test_design", "qa_validation", "performance_testing"
]
    ...
    def route(self, enhancement: Optional[Dict[str, Any]] = None, requirements: Optional[Dict[str, Any]] = None) -> RoutingResult:
        ...
        for intent in intents:
            if intent in _DOMAIN_SPECIFIC_NODES:
                ...
                logger.debug("Intent '%s' não possui mapeamento específico de nós.", intent)
            ...
            for req in non_functional_reqs:
                ...
                for nfr_key, node_name in _NFR_NODE_MAP.items():
                    if nfr_key in req_lower:
                        ...
                        logger.info("[ORCHESTRATOR] Nó '%s' injetado via requisito não-funcional: %s", node_name, req)
        ...
        logger.info(
            ...
            next_phase, domains_detected or ["unknown"], len(final_nodes), final_nodes
        )
