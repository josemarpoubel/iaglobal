...
class FailureAnalysisAgent(AgentBase):
        ...
        try:
            ...
            for prov, stats in by_provider.items():
                stats["avg_latency_ms"] = (
                    round(stats["total_latency_ms"] / stats["calls"], 1)
                    if stats["calls"]
                    else 0
                )
                stats["success_rate"] = (
                    round(stats["success"] / stats["calls"] * 100, 1)
                    if stats["calls"]
                    else 0
                )
            return {
    ...
    def generate_report(cls, system_data: Dict[str, Any]) -> str:
        ...
        if errors.get("total", 0) > 0:
            ...
            if errors.get("by_component"):
                ...
                for comp, count in sorted(
                    errors["by_component"].items(), key=lambda x: -x[1]
                ):
        ...
        if logs.get("content"):
            warning_lines = [
                l
                for l in logs["content"].splitlines()
    ...
    def persist_report(cls, system_data: Dict[str, Any], report: str) -> Dict[str, str]:
        ...
        try:
            ...
            json_path.write_text(
                json.dumps(system_data, indent=2, ensure_ascii=False, default=str),
                encoding="utf-8",
            )
