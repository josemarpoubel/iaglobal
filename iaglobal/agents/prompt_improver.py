...
DOMAIN_REGISTRY: List[DomainProfile] = [
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Use prepared statements (PDO/MySQLi) — NUNCA interpole variáveis em SQL.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Sanitize entradas com filter_input() e FILTER_SANITIZE_*.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Valide CSRF com tokens de sessão em todos os formulários POST.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Configure header('Content-Type: application/json; charset=utf-8') em respostas JSON.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Nunca exponha paths do servidor, versões ou stack traces ao cliente.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
            ...
            Constraint(
                "Aplique princípio do menor privilégio em conexões de banco.",
                ConstraintSeverity.MEDIUM,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Escape toda saída HTML — use textContent, não innerHTML com dados externos.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Aplique Content-Security-Policy via meta ou header.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Nunca exponha tokens, chaves ou paths em código client-side.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Implemente atributos de acessibilidade (aria-*, role, alt) em elementos interativos.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Use HTTPS para todas as requisições externas.", ConstraintSeverity.HIGH
            ),
            Constraint(
                "Valide formulários tanto no cliente quanto declare validação server-side.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Evite dependências externas desnecessárias — minimize surface de ataque.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Mobile-first: meta viewport + CSS media queries (max-width: 768px) para responsividade.",
                ConstraintSeverity.CRITICAL,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Retorne códigos HTTP semânticos (200, 201, 400, 401, 403, 404, 422, 500).",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Valide toda entrada com schema (Pydantic, Joi ou equivalente).",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Nunca retorne stack traces ou mensagens internas em produção.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Implemente autenticação (JWT/OAuth2) e autorização por recurso.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Inclua headers CORS com origens explícitas — nunca '*' em produção.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Versione a API via URL (/v1/) ou header Accept.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Adicione rate limiting e circuit breaker em chamadas externas.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Documente contratos com OpenAPI/Swagger inline.",
                ConstraintSeverity.LOW,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Trate valores nulos/NaN explicitamente — nunca assuma dados limpos.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Valide tipos e ranges antes de operações numéricas/estatísticas.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Documente schema, colunas e tipos esperados no código.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Use tipos explícitos ao carregar CSVs (dtype=).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Evite loops sobre DataFrames — use operações vetorizadas.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Trate encoding de arquivos explicitamente (utf-8, latin-1).",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Adicione logging de volume (linhas lidas/descartadas) em cada etapa do pipeline.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Use chunking para arquivos grandes (> 100MB).", ConstraintSeverity.LOW
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Use Decimal ou bibliotecas financeiras para valores monetários — NUNCA float puro.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Nunca logue chaves de API, tokens ou credenciais de corretoras.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Trate datas com timezone awareness (pytz ou zoneinfo).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Valide dados históricos: cheque gaps, splits e dividendos.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Inclua tratamento de erros para APIs de dados (rate limit, timeout).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Documente premissas de modelagem e limitações explicitamente.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Backtesting deve incluir custo de transação e slippage.",
                ConstraintSeverity.MEDIUM,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Nunca use dados de teste para decisões de treino (leakage).",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Fixe random seeds para reprodutibilidade.", ConstraintSeverity.CRITICAL
            ),
            Constraint(
                "Avalie com métricas adequadas ao problema (não só accuracy).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Normalize/padronize features antes de modelos sensíveis à escala.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Use cross-validation em vez de split simples para datasets pequenos.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Documente hiperparâmetros e versão de bibliotecas.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
            ...
            Constraint(
                "Monitore distribuição de dados de produção vs treino (data drift).",
                ConstraintSeverity.LOW,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "NUNCA implemente criptografia própria — use bibliotecas auditadas (cryptography, libsodium).",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Use bcrypt ou Argon2 para senhas — SHA-1/MD5 são proibidos.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Valide tamanho, tipo e formato de TODAS as entradas externas.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Implemente defesa em profundidade — múltiplas camadas de validação.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Nunca exponha detalhes de implementação em mensagens de erro.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Use timing-safe comparison para segredos (hmac.compare_digest).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Implemente logging de eventos de segurança (falhas de auth, rate limit).",
                ConstraintSeverity.MEDIUM,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Use bibliotecas auditadas (OpenZeppelin) — nunca implemente primitivos próprios.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Proteja contra reentrancy com checks-effects-interactions pattern.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Valide endereços antes de qualquer operação on-chain.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Mantenha chaves privadas FORA do código-fonte — use variáveis de ambiente.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Trate inteiros com SafeMath ou Solidity >= 0.8 (overflow nativo).",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Implemente circuit breaker (pause) em contratos críticos.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Teste em testnet antes de mainnet — documente gas costs.",
                ConstraintSeverity.MEDIUM,
            ),
        ],
        reflection_checks=[
    ...
    DomainProfile(
        ...
        constraints=[
            Constraint(
                "Nunca hardcode credenciais — use secrets manager ou variáveis de ambiente.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Containers devem rodar como non-root user.",
                ConstraintSeverity.CRITICAL,
            ),
            Constraint(
                "Implemente health checks e readiness probes em todos os serviços.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Use imagens base mínimas (alpine/distroless) para reduzir surface de ataque.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Infraestrutura deve ser idempotente — aplicar múltiplas vezes sem efeito colateral.",
                ConstraintSeverity.HIGH,
            ),
            Constraint(
                "Implemente rollback automático em caso de falha no deploy.",
                ConstraintSeverity.MEDIUM,
            ),
            Constraint(
                "Documente resource limits (CPU/memory) de todos os containers.",
                ConstraintSeverity.MEDIUM,
            ),
        ],
        reflection_checks=[
...
class PersonaComposer:
    ...
    def compose(
        ...
        if len(domains) > 1:
            secondary_skills = [
                f"{d.name} (score={round(s, 1)})" for d, s in domains[1:3]
            ]
            base += f" Com conhecimento adicional em: {', '.join(secondary_skills)}."
        return base
...
class DecompositionEngine:
    ...
    def decompose(
        ...
        if mode == PromptMode.COMPACT:
            return "Plano: " + " → ".join(
                s.split("—")[-1].strip() if "—" in s else s for s in steps[:4]
            )
    ...
    def improve_with_report(
        ...
        if self._skill_registry and _SKILL_REGISTRY_AVAILABLE:
            try:
                ...
                for skill in skills:
                    ...
                    codigo_fonte = few_shot_provider._extract_callable_source(
                        skill.run_fn
                    )
        ...
        if all_constraints:
            ...
            report = ImprovementReport(
                ...
                reflection_checks=len(ReflectionEngine._UNIVERSAL_CHECKS)
                + sum(len(d.reflection_checks) for d, _ in domains[:2]),
