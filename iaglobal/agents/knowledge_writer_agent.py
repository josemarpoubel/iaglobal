...
from iaglobal.memory.memory_vector import store as vector_store
...
class KnowledgeWriterAgent(AgentBase):
    ...
    def learn_from_conversation(
        self, prompt: str, response: str, source: str = "dialog"
    ) -> Dict:
        ...
        logger.debug(
            f"[KB-WRITER] Conceitos extraídos: {len(concepts)} ({', '.join(concepts[:3])})"
        )
        ...
        logger.info(
            f"[KB-WRITER] Conversa processada: {total} extrações "
    ...
    def learn_from_text(
        self, text: str, source: str = "auto", entry_type: str = "concept"
    ) -> Dict:
        ...
        entry = {
            ...
            "confidence": self.noise.score_confidence(
        ...
        elapsed = time.time() - start
        logger.info(
            f"[KB-WRITER] Texto aprendido: type={entry_type} "
    ...
    def consolidate_session(self) -> Dict:
        ...
        elapsed = time.time() - start
        logger.info(
            f"[KB-WRITER] Sessão consolidada: {result['entries']} interações, "
    ...
    def _write_definition(
        self, prompt: str, response: str, source: str
    ) -> Optional[Dict]:
