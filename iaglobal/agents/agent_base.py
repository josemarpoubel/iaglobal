...
from iaglobal.artifacts.artifact_factory import artifact_factory
...
def _upgrade_bandit_global() -> Any:
    ...
    try:
        from iaglobal.chappie.bandit_evolution import BanditPolicyEvolution, get_bandit_evolution
        if not isinstance(bp, BanditPolicyEvolution):
            ...
            logger.info("[AgentBase] BanditPolicy atualizado para BanditPolicyEvolution (Chappie)")
            return be
        return bp
    except Exception:
        pass
    ...
    try:
        ...
        if not isinstance(bp, BanditPolicyEvolutiva):
            ...
            logger.info("[AgentBase] BanditPolicy atualizado para BanditPolicyEvolutiva")
            return evol
        return bp
    except Exception:
        pass
...
class AgentBase(ABC):
    ...
    async def _resolve_with_tools(self, task: str, tags: Optional[list] = None) -> Optional[str]:
        try:
            tool_entry, score = tool_library.match(task, tags or [])
            if tool_entry and score >= 0.7:
                logger.info("[%s] Tool match: %s (score=%.2f)", self.agent_name.upper(), tool_entry.name, score)
    ...
    async def _call_llm(
        ...
        if memory_result.found:
            logger.info(
                "[%s] Memory hit | source=%s confidence=%.2f latency=%.0fms",
        ...
        if response is None:
            try:
                response = await self.bandit.generate(
                ...
                model_used = getattr(self.bandit, '_last_model', None) or candidates[0] if success else candidates[0]

                self._llm_calls += 1
                logger.info("[%s] %s (tools=%d llm=%d)", self.agent_name.upper(), "OK" if response else "FAIL", self._tool_hits, self._llm_calls)

                if response:
                    await router.store_result(prompt, response, "llm")
                    if len(str(response)) > 100 and ("def " in str(response) or "class " in str(response)):
                        try:
                            tool_library.register_from_code(prompt, str(response))
                        except Exception:
                            pass

                await self._registrar_ivm(success=success, latencia_ms=latency * 1000, model=model_used)
            except Exception as e:
                latency = time.time() - start_time
                await self._registrar_ivm(success=False, latencia_ms=latency * 1000, model=model_used)
                # Análise de falha → memória imunológica do EvoAgent (FailureAnalyzer)
                try:
                    await self.evo_analyze_failure(
                        e,
                        {"prompt": (prompt or "")[:200], "task_type": task_type,
    ...
    def _estimar_custo_creditos(model: str) -> float:
        ...
        if any(p in m for p in ("ollama/", "opencode/", "hf_router", "hf_router_glm5f")):
            return 0.1
    ...
    async def _registrar_ivm(self, success: bool, latencia_ms: float, model: str) -> None:
        ...
        if hasattr(self.bandit, 'registrar_execucao'):
            try:
                ivm_val = ivm.get_ivm(self.agent_name) if ivm else (0.5 if success else 0.0)
                ...
                logger.debug("[%s] Falha ao registrar evolutivo: %s", self.agent_name.upper(), e)
    ...
    async def _ciclo_evolutivo(self) -> None:
        if hasattr(self.bandit, 'autonomous_cycle'):
            try:
                ...
                logger.debug("[%s] Falha no ciclo evolutivo: %s", self.agent_name.upper(), e)
    ...
    async def get_evo_agent(self) -> Any:
        ...
        async with _get_evo_lock():
            ...
            logger.info("[%s] EvoAgent de linhagem inicializado (marker=%s)",
                        self.agent_name.upper(), evo.lineage_marker)
        return evo
    ...
    def _register_custom_metric(
        ...
        self.credit_engine.record(ExecutionEvent(
        ...
        if extra_data:
            logger.debug(
                "[%s] Metrica customizada: %s",
                self.agent_name.upper(), extra_data
            )
