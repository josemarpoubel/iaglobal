...
class ReflexionAgent:
    ...
    def analisar_resultado_rich(
        ...
        try:
            ...
            elapsed_ms = (time.perf_counter() - t0) * 1000
            self._log.warning("Falha na análise", error=str(exc), elapsed_ms=round(elapsed_ms, 1))
            return self._fallback_analysis(elapsed_ms)
    ...
    def sugerir_melhoria_rich(
        ...
        """
        task_str = str(task)
        self._log.info("Gerando melhoria", code_chars=len(codigo), analysis_chars=len(analise))
        ...
        try:
            ...
            elapsed_ms = (time.perf_counter() - t0) * 1000
            self._log.warning("Falha ao gerar melhoria", error=str(exc), elapsed_ms=round(elapsed_ms, 1))
            return self._fallback_improvement(codigo, elapsed_ms)
