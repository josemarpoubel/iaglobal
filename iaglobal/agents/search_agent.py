...
class SearchAgent(AgentBase):
    ...
    def pesquisar_e_aprender(self, termo_busca: str) -> bool:
        ...
        logger.info(
            f"🛰️ [SEARCH AGENT]: Iniciando rotina de aprendizado para a query: '{termo_busca}'"
        )
        ...
        logger.info(
            f"🛰️ [SEARCH AGENT]: Termo refinado para o buscador: '{query_limpa}'"
        )
        ...
        try:
            ...
            if not resultados:
                logger.warning(
                    "⚠️ [SEARCH AGENT]: ddgs não retornou resultados para a query."
                )
                return False
            ...
            if resultados_processados == 0:
                logger.warning(
                    "⚠️ [SEARCH AGENT]: Resultados ddgs analisados, mas nenhum continha dados válidos."
                )
                return False

            search_success = True
            logger.info(
                f"✅ [SEARCH AGENT]: Sucesso! {resultados_processados} novos dados sincronizados no cbor2."
            )
            return True
    ...
    def _validar_shape_ddgs(resultados: list) -> None:
        ...
        if not isinstance(resultados, list):
            logger.error(
                "⚠️ [SHAPE] ddgs retornou tipo inesperado: %s", type(resultados).__name__
            )
            return
        amostra = resultados[0] if resultados else {}
        chaves_esperadas = {"title", "href", "body"}
        if not isinstance(amostra, dict) or not chaves_esperadas.issubset(
            amostra.keys()
        ):
            actual = (
                sorted(amostra.keys())
                if isinstance(amostra, dict)
                else type(amostra).__name__
            )
            logger.error(
                "⚠️ [SHAPE] ddgs retornou contrato inesperado: %s (esperado: %s)",
                actual,
                sorted(chaves_esperadas),
            )
        else:
    ...
    async def process_task(self, task: str) -> str:
        ...
        if (
            "código" in task.lower()
            or "html" in task.lower()
            or "github" in task.lower()
        ):
