...
def build_multi_agent_prompt(task: str, context: str = "", plan: Dict = None) -> str:
    ...
    parts.append(
        "3. critic → tester → debugger → validator → fix_validator → debug_coder"
    )
    parts.append(
        "4. reflexion → evaluator → gap_analyzer → skill_generator → sandbox_validator"
    )
...
def run_multi_agent_delegation(
    task: Union[str, Task], context: str = "", plan: Dict = None
) -> MultiAgentConfig:
    """
    ...
    return MultiAgentConfig(
        task=task_str, context=context, plan=plan or {}, subtasks=[]
    )
...
def reflect(code: str, result: dict, task: Union[str, Task]) -> str:
    ...
    logger.warning(
        "[MULTI_AGENT] reflect() chamado - deve usar nó 'reflexion' no grafo"
    )
...
class PipelineOrchestrator:
    ...
    def resolve(self, task: Union[str, Task], max_iters: int = 3) -> str:
        task_str = str(task)
        logger.info(
            "[MULTI_AGENT] PipelineOrchestrator.resolve() delegando para grafo: %s",
            task_str[:80],
        )
