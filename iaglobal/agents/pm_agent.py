...
class PMAgent:
    ...
    def __init__(self):
        ...
        non_func_pattern = r'\b(' + '|'.join(re.escape(p) for p in self._NON_FUNC_REQ_PATTERNS) + r')\b'
    ...
    def extract_requirements(self, prompt: str, enhancement: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        context_search = re.compile(rf'\b({actions_pattern})\s+([a-z0-9_]+)', re.IGNORECASE)
        ...
        if not functional and any(w in clean_prompt for w in ["criar", "fazer", "desenvolver", "construir"]):
        ...
        logger.info(
            ...
            func_count, non_func_count, priority, len(prompt)
        )
