...
class AgentMitosisEngine:
    ...
    def __init__(self, db_path: Path = None):
        ...
        logger.info(
            f"🧬 [MITOSE] Engine initialized | stem_agents={len(self.stem_agents)}"
        )
    ...
    async def analyze_task(
        ...
        if performance_score >= 0.75:
            if agent_id not in self.specializations:
                ...
                # Média móvel do IVM
                spec.avg_ivm = (
                    spec.avg_ivm * (spec.tasks_completed - 1) + ivm
                ) / spec.tasks_completed
                spec.epigenetic_config = self._generate_epigenetic_config(
                    task_type, spec.avg_ivm
                )
                self.epigenetic_memory[agent_id] = spec.epigenetic_config
    ...
    def suggest_specialization(
        self, agent_id: str, available_types: List[str]
    ) -> Optional[str]:
        """
        ...
        if best_type and best_count >= 3:
            logger.info(
                f"🧬 [MITOSE] Sugerindo especialização {best_type} para {agent_id}"
            )
            return best_type
    ...
    async def create_specialist(
        self, base_agent_id: str, specialization: str, specialist_id: str = None
    ) -> str:
        """
    ...
    def get_specialist_pool(self, task_type: str) -> List[str]:
        ...
        for spec in self.specializations.values():
            if spec.specialization == task_type or spec.specialization.startswith(
                task_type[:3]
            ):
    ...
    async def _save_state(self):
        ...
        logger.debug(
            f"🧬 [MITOSE] Estado persistido: {len(self.specializations)} especializações"
        )
