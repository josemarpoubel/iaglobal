    ...
    def audit(
        ...
        patterns = [
            ...
            (
                ...
                "Subprocess pode ser vetor de injeção",
                "high",
            ),
            (r"os\.system", "os.system — execução de comando no shell", "high"),
            (
                r"sqlite3\.execute\(.*['\"]",
                "SQL direto — risco de SQL injection",
                "high",
            ),
            (r"\.format\(.*\{\}", "Formatação insegura de string", "low"),
            (
                r"request\.(GET|POST|REQUEST)\[",
                "Acesso direto a request sem sanitização",
                "medium",
            ),
            (
            ...
            (
                r"ALLOWED_HOSTS\s*=\s*\['\*'\]",
                "CORS/Hosts excessivamente permissivo",
                "medium",
            ),
        ]
        ...
        if knowledge_context:
            logger.info(
                "🔍 [SECURITY-AUDIT] knowledge context available (%d chars)",
                len(knowledge_context),
            )

        if error_context:
            logger.info(
                "🔍 [SECURITY-AUDIT] error context available (%d chars)",
                len(error_context),
            )
            if "eval" in error_context.lower():
                patterns.insert(
                    ...
                    (
                        ...
                        "[REINCIDENCIA] eval/exec ja causou erro antes",
                        "high",
                    ),
                )
            if "sql" in error_context.lower():
                patterns.insert(
                    0,
                    (
                        r"sqlite3\.execute|raw_sql|\.format\(.*['\"]",
                        "[REINCIDENCIA] SQL injection ja ocorreu antes",
                        "high",
                    ),
                )
            if "pickle" in error_context.lower():
                patterns.insert(
                    ...
                    (
                        ...
                        "[REINCIDENCIA] Pickle inseguro ja ocorreu antes",
                        "high",
                    ),
                )
            if (
                "hardcoded" in error_context.lower()
                or "secret" in error_context.lower()
            ):
                patterns.insert(
                    0,
                    (
                        r"(API_KEY|SECRET_KEY|PASSWORD)\s*=.*['\"]",
                        "[REINCIDENCIA] Secret hardcoded ja ocorreu antes",
                        "high",
                    ),
                )
        ...
        if _contem_padrao_regex_perigoso(code):
            logger.error(
                "🚫 [SECURITY-AUDIT] Código contém padrão regex perigoso — potencial DoS"
            )
            issues.append(
                {
        ...
        for pattern, desc, severity in patterns:
            if re.search(pattern, code, re.IGNORECASE):
                issues.append(
                    {"description": desc, "severity": severity, "pattern": pattern}
                )
                severity_count[severity] = severity_count.get(severity, 0) + 1

        for req in security_requirements:
            req_lower = req.lower()
            if "auth" in req_lower and not re.search(
                r"(login|logout|session|token|jwt)", code, re.IGNORECASE
            ):
                issues.append(
                    {
                        "description": f"Requisito não implementado: {req}",
                        "severity": "high",
                        "pattern": "missing_requirement",
                    }
                )
                severity_count["high"] += 1
            if "sanitize" in req_lower and not re.search(
                r"(strip|escape|sanitize|validate)", code, re.IGNORECASE
            ):
                issues.append(
                    {
                        "description": f"Requisito não implementado: {req}",
