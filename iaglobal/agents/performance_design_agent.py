...
class PerformanceIssue:
    ...
    load_profiles: List[LoadProfile] = field(default_factory=lambda: list(LoadProfile))
...
class BackendRules:
    ...
    async def check_n_plus_one(
        ...
        has_orm = _has(
            tokens, "orm", "sqlalchemy", "django", "sequelize", "prisma", "hibernate"
        )
        has_loop_query = _has(
            tokens, "for loop", "foreach", "loop query", "n+1", "n plus 1"
        )
    ...
    async def check_cache_strategy(
        ...
        severity = (
            Severity.CRITICAL
            if profile in (LoadProfile.ENTERPRISE, LoadProfile.HYPERSCALE)
            else Severity.HIGH
        )
        issue = PerformanceIssue(
            id="BE-002",
            ...
            load_profiles=[
                LoadProfile.GROWTH,
                LoadProfile.ENTERPRISE,
                LoadProfile.HYPERSCALE,
            ],
        )
        rec = PerformanceRecommendation(
            issue_id="BE-002",
...
class FrontendRules:
    ...
    async def check_bundle_size(
        ...
        issue = PerformanceIssue(
            ...
            severity=Severity.HIGH,
            impact_areas=[
                ImpactArea.RENDER_PERF,
                ImpactArea.UX,
                ImpactArea.BUNDLE_SIZE,
            ],
            layer="frontend",
    ...
    async def check_css_performance(
        ...
        has_css = _has(
            tokens, "css", "estilo", "animacao", "animation", "transition", "transform"
        )
        has_perf_css = _has(
            tokens,
            "will-change",
...
class PerformanceDesignAgent(AgentBase):
    ...
    async def analyze_async(
        ...
        **kwargs,
        ...
        if knowledge_context:
            logger.info(
                "⚡ [PERF-DESIGN] knowledge_context=%d chars", len(knowledge_context)
            )
        if error_context:
        ...
        logger.info(
            "⚡ [PERF-DESIGN] load_profile=%s | rendering=%s", profile, rendering
        )
        ...
        for result_coro in backend_results:
            await self._collect_rule_result(
                issues, recs, score, await result_coro, profile
            )
        ...
        # ── Quick wins e riscos ──────────────────────────────────────────────
        quick_wins = [rec.title for rec in recs if rec.effort == "low"]
        arch_risks = [
            issue.title for issue in issues if issue.severity == Severity.CRITICAL
        ]

        # ── Sort por prioridade ──────────────────────────────────────────────
        recs.sort(key=lambda r: r.priority)
        issues.sort(
            key=lambda i: ["critical", "high", "medium", "low"].index(i.severity.value)
        )
    ...
    async def _collect_rule_result(
        ...
        if (
            profile is not None
            and issue.load_profiles
            and profile not in issue.load_profiles
        ):
            return
        issues.append(issue)
        recs.append(rec)
    ...
    def _apply_score_deduction(self, score: PerformanceScore, issue: PerformanceIssue):
        ...
        score.deduct(
            issue.layer
            if issue.layer in ("backend", "frontend", "database", "infra")
            else "backend",
            amount,
        )
