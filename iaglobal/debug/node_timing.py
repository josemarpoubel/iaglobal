...
def report(timings: Dict[str, dict], total: float) -> str:
    ...
    lines.append(f"  Mais rápido    : {min(all_times):.2f}s ({min(timings, key=lambda n: timings[n]['elapsed'])})")
    lines.append(f"  Mais lento     : {max(all_times):.2f}s ({max(timings, key=lambda n: timings[n]['elapsed'])})")
    lines.append(f"  Mediana        : {sorted(all_times)[len(all_times)//2]:.2f}s" if all_times else "N/A")
    lines.append("=" * 60)
...
async def main():
    ...
    with open(os.path.join(data_dir, "benchmark_latency.json"), "w") as f:
