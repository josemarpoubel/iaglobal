...
def registrar_erro_no_banco(task: str, erro_excecao: Exception, context: Optional[str] = None):
    """
    ...
    try:
        with conn:
            ...
            tb_str = "".join(traceback.format_exception(None, erro_excecao, erro_excecao.__traceback__))
...
def _sincronizar_registro_seguro():
    ...
    try:
        ...
        DataBridge.sincronizar_sqlite_para_cbor(
            str(DATA_ROOT / "errors.db"), "error_registry", str(DATA_ROOT / "errors.cbor2")
        )
        ...
        logger.warning(f"⚠️ [SANDBOX]: Erro não bloqueante na sincronização: {sync_error}")
...
def preparar_ambiente_sandbox():
    ...
    try:
        ...
        vars_to_sanitize = ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "OPENAI_API_KEY", "DATABASE_URL"]
...
def executar_codigo_sandbox(
    ...
    try:
        ...
        for node in ast.walk(tree):
            if not isinstance(node, PERMITTED_NODES):
                ...
                logger.error(
                    f"🚫 [SECURITY] Node proibido: {type(node).__name__}"
                )
        ...
        if not seguro:
            ...
            logger.error(
                f"🚫 [SECURITY] Código bloqueado: {'; '.join(violacoes)}"
            )
    ...
    # =============================================================
        ...
        logger.warning(
            f"⏳ [SANDBOX] Timeout excedido ({timeout}s)"
        )
        ...
        if tmp_path:
            ...
            try:
                ...
                if os.path.exists(tmp_path):
                    ...
                    logger.info(
                        "🧹 [SANDBOX] Arquivo temporário removido."
                    )

            except Exception as cleanup_error:

                logger.error(
                    f"⚠️ [SANDBOX] Cleanup error: {cleanup_error}"
                )
