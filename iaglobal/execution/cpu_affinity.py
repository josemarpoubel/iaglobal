...
class CpuAffinityManager:
    ...
    async def dispersion_report(self) -> dict:
        async with self.lock:
            ...
            for agent_id in self._agents:
                core = self._last_agent_map.get(
                    agent_id, self._core_from_hash(agent_id)
                )
            ...
            imbalance = (
                (max_load - total_mapped / self._total_cores)
                / (total_mapped / self._total_cores)
                if total_mapped
                else 0
            )
            ...
            if self._agents:
                ivm_medio = sum(m.ivm_atual for m in self._agents.values()) / len(
                    self._agents
                )
                fitness_medio = sum(
                    m.fitness_score for m in self._agents.values()
                ) / len(self._agents)
                agentes_em_sobrevivencia = sum(
                    1 for m in self._agents.values() if m.em_modo_sobrevivencia
                )
            ...
            return {
                ...
                "total_budget_alocado": round(sum(budgets.values()), 3)
                if budgets
                else 0,
            }
    ...
    async def rebalance_if_needed(self, *args, **kwargs) -> bool:
        ...
        async with self.lock:
            ...
            prioridades = [
                (
                    agent_id,
                    self._agents[agent_id].fitness_score
    ...
    async def set_cpu_budget(self, agent_id: str, budget: float) -> None:
        teto = (
            BUDGET_ADRENALINA
            if self._metabolic_state == "ADRENALINE"
            else BUDGET_PADRAO
        )
    ...
    async def entrar_estase(self):
        ...
        logger.info(
            "[CPU] 🌙 Entrando em DEEP SLEEP: Ritmo metabólico reduzido para 5%%."
        )
    ...
    async def disparar_adrenalina(self, agent_id: str, duracao: float = 30.0):
        ...
        logger.warning(
            "[CPU] ⚡ ADRENALINA: Agente %s em Burst Mode (50%% CPU) por %.1fs",
            agent_id,
            duracao,
        )
    ...
    async def atualizar_estado_metabolico(self):
        async with self.lock:
            if (
                self._metabolic_state == "ADRENALINE"
                and time.time() > self._adrenaline_expiry
            ):
                self._metabolic_state = "NORMAL"
                logger.info(
                    "[CPU] 📉 Adrenalina esgotada. Retornando à homeostase normal."
                )
    ...
    async def set_priority_boost(
        self, agent_ids: List[str], boost_percent: int = 50
    ) -> None:
    ...
    async def enter_critical_batch(
        self, agent_ids: List[str], boost_percent: int = 60
    ) -> Dict[str, float]:
        """
    ...
    async def calcular_ivm(
        self,
        agent_id: str,
        produtividade: Optional[float] = None,
        cpu_usage: Optional[float] = None,
        obsidian_notes: Optional[int] = None,
    ) -> float:
        ...
        async with self.lock:
            ...
            notes = (
                obsidian_notes
                if obsidian_notes is not None
                else metrics.obsidian_notes_escritas
            )
    ...
    async def _write_single_fitness(self, agent_id: str, score: float) -> None:
        ...
        try:
            ...
            data["ultima_atualizacao"] = time.strftime(
    ...
    def _gerar_recomendacao(ivm: float, metrics: AgentCpuMetrics) -> str:
        if ivm < IVM_THRESHOLD_CRITICO:
            return (
                f"Agente consumindo {metrics.cpu_usage_atual:.0%} de CPU "
