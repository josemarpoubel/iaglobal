...
class HealthAggregator:
    ...
    async def check_all(self) -> Dict[str, ServiceHealth]:
        ...
        logger.info(
            "[HEALTH] Check completo em %.0fms | serviços=%d",
            elapsed, len(results)
        )
        
        health_map = {
            name: result if isinstance(result, ServiceHealth) else self._error_to_health(name, result)
    ...
    async def _check_service(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    "[HEALTH] Serviço %s retornou status %d",
    ...
    def compute_overall_status(health_map: Dict[str, ServiceHealth]) -> str:
        ...
        # Gateway é crítico
        if health_map.get("gateway", ServiceHealth("gateway", "unknown")).status == "unhealthy":
            return "unhealthy"
    ...
    async def get_metabolic_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if ivm:
                ...
                ivm_medio = sum(r["current_ivm"] for r in ranking) / agents_ativos if agents_ativos else 0
                ...
                return {
                    ...
                    "agents_excelentes": sum(1 for r in ranking if r["current_ivm"] >= 0.8),
                    "agents_criticos": sum(1 for r in ranking if r["current_ivm"] < 0.3),
                }
            else:
                return {
                    "ivm_medio": 0,
    ...
    async def get_immune_state(self) -> Dict[str, Any]:
        ...
        """
        try:
            from iaglobal.observability.entropy_interceptor import get_immune_state as get_entropy_state
            ...
            cache_usage_pct = (cache_size / LRU_CACHE_SIZE * 100) if LRU_CACHE_SIZE > 0 else 0
            
            # Métricas de vacinas DLQ (Mutação 1C)
            now = time.monotonic()
            oldest_vaccine_age_days = 0
            if few_shot_provider._example_cache:
                oldest_timestamp = min(ts for _, ts in few_shot_provider._example_cache.values())
                oldest_vaccine_age_days = (now - oldest_timestamp) / 86400
            
            negative_examples_count = len(few_shot_provider._negative_examples)
            estimated_token_overhead = negative_examples_count * ESTIMATED_TOKENS_PER_EXAMPLE
            ...
            return {
                ...
                "cognitive": {
                    ...
                    "cache_health": "high" if cache_usage_pct > 80 else "normal" if cache_usage_pct > 30 else "low",
                    # Métricas de vacinas DLQ (Mutação 1C)
                    "few_shot": {
                        ...
                        "expiry_status": "ok" if oldest_vaccine_age_days < MAX_VACCINE_AGE_DAYS else "expiry_soon",
                    },
                },
