...
mcp_breaker = pybreaker.CircuitBreaker(
    fail_max=5,
    reset_timeout=60,
    name="mcp_service"
)

ui_breaker = pybreaker.CircuitBreaker(
    fail_max=5,
    reset_timeout=60,
    name="ui_service"
)
...
async def circuit_breaker_middleware(request: Request, call_next):
    ...
    if path.startswith("/mcp"):
        ...
        try:
            ...
            logger.warning("[CIRCUIT_BREAKER] Evolution circuit open - rejeitando request")
            return JSONResponse(
                status_code=503,
                content={
...
async def health_consolidated():
    ...
    response = {
        ...
        "vital_signs": {
            ...
            **{name: health._asdict() if hasattr(health, '_asdict') else health.__dict__ 
    ...
    logger.info(
        ...
        overall_status, len(health_map), elapsed
    )
