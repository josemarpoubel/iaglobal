...
async def alternar_estrategia(modo: str):
    if modo == "deep":
        ...
        raise HTTPException(
            status_code=400, detail="Estratégia inválida. Escolha 'deep' ou 'fast'."
        )
...
async def dashboard_json():
    ...
    short_term = (
        len(list((obsidian_dir / "02_Short_Term").glob("*.md")))
        if (obsidian_dir / "02_Short_Term").exists()
        else 0
    )
    long_term = (
        len(list((obsidian_dir / "03_Long_Term").glob("*.md")))
        if (obsidian_dir / "03_Long_Term").exists()
        else 0
    )
    synapses = (
        len(list((obsidian_dir / "04_Synapses").glob("*.md")))
        if (obsidian_dir / "04_Synapses").exists()
        else 0
    )
...
if __name__ == "__main__":
    ...
    print(
        f"💡 Dica: Para acessar via ASGI Gateway, use http://localhost:8000/evolution/"
    )
