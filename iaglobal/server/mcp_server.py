...
class MCPServer:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning(
                f"AcetylcholineBus não disponível (modo standalone): {e}"
            )
    ...
    def _setup_routes(self):
        ...
        self.app.post("/fix", dependencies=[Depends(self._validate_credentials)])(
    ...
    async def json_rpc_handler(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if method == "initialize":
                ...
                raise HTTPException(
                    status_code=400, detail=f"Method not found: {method}"
                )
    ...
    def _make_auth_checker(self):
        ...
        if not valid_user or not valid_pass:
            self.logger.warning(
                "MCP_USER/MCP_PASSWORD não configurados; endpoints protegidos indisponíveis."
            )
    ...
    async def get_metrics(self) -> PlainTextResponse:
        ...
        metrics = []
        metrics.append(
            "# HELP iaglobal_mcp_audit_score Score da última auditoria (0-1)"
        )
        ...
        for inv_name, status in invariants.items():
            ...
            metrics.append(
                f"# HELP iaglobal_invariant_{safe_name}_status Status do invariante (0=VIOLADA, 1=OK)"
            )
    ...
    def _setup_bus_integration(self):
        ...
        try:
            ...
            self.logger.error(
                f"❌ Falha ao integrar MCP Server ao AcetylcholineBus: {e}"
            )
    ...
    async def _handle_autocorrect_command(self, payload: Dict[str, Any]):
        ...
        try:
            self.logger.info(
                f"⚡ Comando de correção recebido: {payload.get('target')}"
            )
            result = await self.autocorrect.verificar_e_corrigir()
            ...
            await self.bus.publish(
                f"mcp/autocorrect/response/{payload['command_id']}", response
            )
    ...
    async def _handle_invariant_violation(self, payload: Dict[str, Any]):
        ...
        self.logger.warning(
            f"🚨 Violação detectada em {payload['invariant']}: {payload['alert']}"
        )
